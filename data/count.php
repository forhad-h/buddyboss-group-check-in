...
if( ! function_exists( 'BPGCI_count_data_by_user_group_status' ) ) {

  function BPGCI_count_data_by_user_group_status( $wpdb, $user_id, $group_id, $status ) {

    $table_name = BPGCI_ADDON_PLUGIN['table_name'];

    $sql = "SELECT COUNT(*) FROM {$table_name} WHERE user_id='{$user_id}' AND group_id='{$group_id}' AND status='{$status}'";
