 ...
 * @param bgci_bp_habits_Group|null $old_group Group before new details were saved.
 */
function groups_notification_group_updated( $group_id = 0, $old_group = null ) {
	$group = groups_get_group( $group_id );

	if ( $old_group instanceof bgci_bp_habits_Group ) {
		$changed = array();

		if ( $group->name !== $old_group->name ) {
			$changed[] = sprintf(
				__( '* Name changed from "%1$s" to "%2$s".', 'bp-group-check-in' ),
		...
		if ( $group->description !== $old_group->description ) {
			$changed[] = sprintf(
				__( '* Description changed from "%1$s" to "%2$s".', 'bp-group-check-in' ),
		...
		if ( $group->slug !== $old_group->slug ) {
			$changed[] = sprintf(
				__( '* Permalink changed from "%1$s" to "%2$s".', 'bp-group-check-in' ),
	...
	$user_ids = bgci_bp_habits_Member::get_group_member_ids( $group->id );
	 ...
	 */
	do_action( 'bgci_bp_habits_sent_updated_email', $user_ids, '', '', $group_id );
}
...
function groups_notification_promoted_member( $user_id = 0, $group_id = 0 ) {
	...
	if ( groups_is_user_admin( $user_id, $group_id ) ) {
		$promoted_to = __( 'organizer', 'bp-group-check-in' );
		$type        = 'member_promoted_to_admin';
	} else {
		$promoted_to = __( 'moderator', 'bp-group-check-in' );
 ...
 * @since BuddyPress 1.0.0
 *
 * @param bgci_bp_habits_Group  $group           Group object.
 * @param bgci_bp_habits_Member $member          Member object.
...
function groups_notification_group_invites( &$group, &$member, $inviter_user_id ) {
	...
	if ( $member instanceof bgci_bp_habits_Member ) {
	...
	switch ( $action ) {
		case 'new_membership_request':
			...
			if ( (int) $total_items > 1 ) {
				$text              = sprintf( __( '%1$d new membership requests for the group "%2$s"', 'bp-group-check-in' ), (int) $total_items, $group->name );
				$amount            = 'multiple';
				...
				if ( 'string' == $format ) {
					 ...
					 * Complete filter - bgci_bp_habits_multiple_new_membership_requests_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 ...
					 */
					return apply_filters( 'bgci_bp_habits_' . $amount . '_' . $action . 's_notification', '<a href="' . $notification_link . '">' . $text . '</a>', $group_link, $total_items, $group->name, $text, $notification_link );
				} else {
					...
					return apply_filters(
						'bgci_bp_habits_' . $amount . '_' . $action . 's_notification',
						array(
				...
				$text              = sprintf( __( '%1$s requests membership for the group: %2$s', 'bp-group-check-in' ), $user_fullname, $group->name );
				$notification_link = $group_link . 'admin/membership-requests/?n=1';
				...
				if ( 'string' == $format ) {
					 ...
					 *
					 * This is a dynamic filter that is dependent on item count and action.
					 * Complete filter - bgci_bp_habits_single_new_membership_request_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for request.
					 ...
					 * @param string $notification_link The permalink for notification.
					 */
					return apply_filters( 'bgci_bp_habits_' . $amount . '_' . $action . '_notification', '<a href="' . $notification_link . '">' . $text . '</a>', $group_link, $user_fullname, $group->name, $text, $notification_link );
				} else {
					 ...
					 */
					return apply_filters(
						'bgci_bp_habits_' . $amount . '_' . $action . '_notification',
						array(
							'link' => $notification_link,
							'text' => $text,
						),
		...
		case 'membership_request_accepted':
			...
			if ( (int) $total_items > 1 ) {
				$text              = sprintf( __( '%d accepted group membership requests', 'bp-group-check-in' ), (int) $total_items, $group->name );
				$amount            = 'multiple';
				...
				if ( 'string' == $format ) {
					 ...
					 * Complete filter - bgci_bp_habits_multiple_membership_request_accepted_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 ...
					 * @param string $notification_link The permalink for notification.
					 */
					return apply_filters( 'bgci_bp_habits_' . $amount . '_' . $action . '_notification', '<a href="' . $notification_link . '">' . $text . '</a>', $total_items, $group->name, $text, $notification_link );
				} else {
				...
				$text              = sprintf( __( 'Membership for group "%s" accepted', 'bp-group-check-in' ), $group->name );
				$filter            = 'bgci_bp_habits_single_membership_request_accepted_notification';
				...
				if ( 'string' == $format ) {
					 ...
					 * Complete filter - bgci_bp_habits_single_membership_request_accepted_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
					 ...
					 * @param string $notification_link The permalink for notification.
					 */
					return apply_filters( 'bgci_bp_habits_' . $amount . '_' . $action . '_notification', '<a href="' . $notification_link . '">' . $text . '</a>', $group_link, $group->name, $text, $notification_link );
				} else {
		...
		case 'membership_request_rejected':
			...
			if ( (int) $total_items > 1 ) {
				$text              = sprintf( __( '%d rejected group membership requests', 'bp-group-check-in' ), (int) $total_items, $group->name );
				$amount            = 'multiple';
				$notification_link = trailingslashit( bp_loggedin_user_domain() . bp_get_groups_slug() ) . '?n=1';
				...
				if ( 'string' == $format ) {
					 ...
					 * Complete filter - bgci_bp_habits_multiple_membership_request_rejected_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
					 ...
					 * @param string $notification_link The permalink for notification.
					 */
					return apply_filters( 'bgci_bp_habits_' . $amount . '_' . $action . '_notification', '<a href="' . $notification_link . '">' . $text . '</a>', $total_items, $group->name );
				} else {
				...
				$text              = sprintf( __( 'Membership for group "%s" rejected', 'bp-group-check-in' ), $group->name );
				$notification_link = $group_link . '?n=1';
				...
				if ( 'string' == $format ) {
					 ...
					 * Complete filter - bgci_bp_habits_single_membership_request_rejected_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
		...
		case 'member_promoted_to_admin':
			...
			if ( (int) $total_items > 1 ) {
				$text              = sprintf( __( 'You were promoted to the role of %1$s in %2$d groups', 'bp-group-check-in' ), strtolower( get_group_role_label( $group_id, 'organizer_singular_label_name' ) ), (int) $total_items );
				$amount            = 'multiple';
				$notification_link = trailingslashit( bp_loggedin_user_domain() . bp_get_groups_slug() ) . '?n=1';
				...
				if ( 'string' == $format ) {
					 ...
					 * Complete filter - bgci_bp_habits_multiple_member_promoted_to_admin_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
					 ...
					 * @param string $notification_link The permalink for notification.
					 */
					return apply_filters( 'bgci_bp_habits_' . $amount . '_' . $action . '_notification', '<a href="' . $notification_link . '">' . $text . '</a>', $total_items, $text, $notification_link );
				} else {
					/**
				...
				$text              = sprintf( __( 'You were promoted to the role of %1$s in the group "%2$s"', 'bp-group-check-in' ), strtolower( get_group_role_label( $group_id, 'organizer_singular_label_name' ) ), $group->name );
				$notification_link = $group_link . '?n=1';

				if ( 'string' == $format ) {
					/**
					 * Filters single promoted to group admin notification for non-string format.
					 * Complete filter - bgci_bp_habits_single_member_promoted_to_admin_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
		...
		case 'member_promoted_to_mod':
			...
			if ( (int) $total_items > 1 ) {
				$text              = sprintf( __( 'You were promoted to a %1$s in %2$d groups', 'bp-group-check-in' ), strtolower( get_group_role_label( $group_id, 'moderator_singular_label_name' ) ), (int) $total_items );
				$amount            = 'multiple';
				$notification_link = trailingslashit( bp_loggedin_user_domain() . bp_get_groups_slug() ) . '?n=1';
				...
				if ( 'string' == $format ) {
					 ...
					 * Complete filter - bgci_bp_habits_multiple_member_promoted_to_mod_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
				...
				$text              = sprintf( __( 'You were promoted to a %1$s in the group "%2$s"', 'bp-group-check-in' ), strtolower( get_group_role_label( $group_id, 'moderator_singular_label_name' ) ), $group->name );
				$notification_link = $group_link . '?n=1';

				if ( 'string' == $format ) {
					/**
					 * Filters single promoted to group mod notification for string format.
					 * Complete filter - bgci_bp_habits_single_member_promoted_to_mod_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
		...
		case 'group_invite':
			...
			if ( (int) $total_items > 1 ) {
				$text   = sprintf( __( 'You have %d new group invitations', 'bp-group-check-in' ), (int) $total_items );
				$amount = 'multiple';

				if ( 'string' == $format ) {
					/**
					 * Filters multiple group invitation notification for string format.
					 * Complete filter - bgci_bp_habits_multiple_group_invite_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
				...
				$text   = sprintf( __( 'You have an invitation to the group: %s', 'bp-group-check-in' ), $group->name );
				$filter = 'bgci_bp_habits_single_group_invite_notification';

				if ( 'string' == $format ) {
					/**
					 * Filters single group invitation notification for string format.
					 * Complete filter - bgci_bp_habits_single_group_invite_notification.
					 *
					 * @since BuddyPress 1.0.0
					 *
					 * @param string $string            HTML anchor tag for notification.
		...
		default:
			 ...
			 */
			$custom_action_notification = apply_filters( 'bgci_bp_habits_' . $action . '_notification', null, $item_id, $secondary_item_id, $total_items, $format );
 ...
 * @param int $group_id ID of the group.
 */
function bgci_bp_habits_delete_group_delete_all_notifications( $group_id ) {
	if ( bp_is_active( 'notifications' ) ) {
		bp_notifications_delete_all_notifications_by_type( $group_id, buddypress()->groups->id );
	}
}
add_action( 'groups_delete_group', 'bgci_bp_habits_delete_group_delete_all_notifications', 10 );
 ...
 */
function bgci_bp_habits_uninvite_user_delete_group_invite_notification( $group_id = 0, $user_id = 0 ) {
...
add_action( 'groups_uninvite_user', 'bgci_bp_habits_uninvite_user_delete_group_invite_notification', 10, 2 );
 ...
 * @param int $group_id ID of the group.
 */
function bgci_bp_habits_delete_promotion_notifications( $user_id = 0, $group_id = 0 ) {
...
add_action( 'groups_demoted_member', 'bgci_bp_habits_delete_promotion_notifications', 10, 2 );
 ...
 * @param int $group_id ID of the group.
 */
function bgci_bp_habits_accept_invite_mark_notifications( $user_id, $group_id ) {
	if ( bp_is_active( 'notifications' ) ) {
		bp_notifications_mark_notifications_by_item_id( $user_id, $group_id, buddypress()->groups->id, 'group_invite' );
	}
}
add_action( 'groups_accept_invite', 'bgci_bp_habits_accept_invite_mark_notifications', 10, 2 );
add_action( 'groups_reject_invite', 'bgci_bp_habits_accept_invite_mark_notifications', 10, 2 );
add_action( 'groups_delete_invite', 'bgci_bp_habits_accept_invite_mark_notifications', 10, 2 );
 ...
 * @param int $group_id ID of the group.
 */
function bgci_bp_habits_accept_request_mark_notifications( $user_id, $group_id ) {
	if ( bp_is_active( 'notifications' ) ) {
...
add_action( 'groups_membership_accepted', 'bgci_bp_habits_accept_request_mark_notifications', 10, 2 );
add_action( 'groups_membership_rejected', 'bgci_bp_habits_accept_request_mark_notifications', 10, 2 );
 ...
 * @since BuddyPress 1.9.0
 */
function bgci_bp_habits_screen_my_groups_mark_notifications() {
...
add_action( 'groups_screen_my_groups', 'bgci_bp_habits_screen_my_groups_mark_notifications', 10 );
add_action( 'groups_screen_group_home', 'bgci_bp_habits_screen_my_groups_mark_notifications', 10 );
/*
 * Request membership screen in clear read notification and count.
 */
add_action( 'groups_screen_group_request_membership', 'bgci_bp_habits_screen_my_groups_mark_notifications', 10 );
 ...
 * @since BuddyPress 1.9.0
 */
function bgci_bp_habits_screen_invites_mark_notifications() {
	if ( bp_is_active( 'notifications' ) ) {
		bp_notifications_mark_notifications_by_type( bp_loggedin_user_id(), buddypress()->groups->id, 'group_invite' );
	}
}
add_action( 'groups_screen_group_invites', 'bgci_bp_habits_screen_invites_mark_notifications', 10 );
 ...
 * @since BuddyPress 1.9.0
 */
function bgci_bp_habits_screen_group_admin_requests_mark_notifications() {
	if ( bp_is_active( 'notifications' ) ) {
		bp_notifications_mark_notifications_by_type( bp_loggedin_user_id(), buddypress()->groups->id, 'new_membership_request' );
	}
}
add_action( 'groups_screen_group_admin_requests', 'bgci_bp_habits_screen_group_admin_requests_mark_notifications', 10 );
 ...
 * @param int $user_id ID of the user.
 */
function bgci_bp_habits_remove_data_for_user_notifications( $user_id ) {
	if ( bp_is_active( 'notifications' ) ) {
		bp_notifications_delete_notifications_from_user( $user_id, buddypress()->groups->id, 'new_membership_request' );
	}
}
add_action( 'groups_remove_data_for_user', 'bgci_bp_habits_remove_data_for_user_notifications', 10 );
...
function groups_screen_notification_settings() {
	...
	<table class="notification-settings" id="groups-notification-settings">
		<thead>
			<tr>
				<th class="icon"></th>
				<th class="title"><?php _e( 'Social Groups', 'bp-group-check-in' ); ?></th>
				<th class="yes"><?php _e( 'Yes', 'bp-group-check-in' ); ?></th>
				<th class="no"><?php _e( 'No', 'bp-group-check-in' ); ?></th>
			</tr>
		</thead>
		...
		<tbody>
			<tr id="groups-notification-settings-invitation">
				<td></td>
				<td><?php _e( 'A member invites you to join a group', 'bp-group-check-in' ); ?></td>
				<td class="yes">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-invite-yes"><span class="bp-screen-reader-text"><?php _e( 'Yes, send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
				<td class="no">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-invite-no"><span class="bp-screen-reader-text"><?php _e( 'No, do not send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
			</tr>
			<tr id="groups-notification-settings-info-updated">
				<td></td>
				<td><?php _e( 'Group information is updated', 'bp-group-check-in' ); ?></td>
				<td class="yes">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-group-updated-yes"><span class="bp-screen-reader-text"><?php _e( 'Yes, send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
				<td class="no">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-group-updated-no"><span class="bp-screen-reader-text"><?php _e( 'No, do not send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
			</tr>
			<tr id="groups-notification-settings-promoted">
				<td></td>
				<td><?php _e( 'You are promoted to a group organizer or moderator', 'bp-group-check-in' ); ?></td>
				<td class="yes">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-admin-promotion-yes"><span class="bp-screen-reader-text"><?php _e( 'Yes, send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
				<td class="no">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-admin-promotion-no"><span class="bp-screen-reader-text"><?php _e( 'No, do not send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
			</tr>
			<tr id="groups-notification-settings-request">
				<td></td>
				<td><?php _e( 'A member requests to join a private group you organize', 'bp-group-check-in' ); ?></td>
				<td class="yes">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-membership-request-yes"><span class="bp-screen-reader-text"><?php _e( 'Yes, send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
				<td class="no">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-membership-request-no"><span class="bp-screen-reader-text"><?php _e( 'No, do not send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
			</tr>
			<tr id="groups-notification-settings-request-completed">
				<td></td>
				<td><?php _e( 'Your request to join a group has been approved or denied', 'bp-group-check-in' ); ?></td>
				<td class="yes">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-membership-request-completed-yes"><span class="bp-screen-reader-text"><?php _e( 'Yes, send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
				<td class="no">
					<div class="bp-radio-wrap">
						...
						<label for="notification-groups-membership-request-completed-no"><span class="bp-screen-reader-text"><?php _e( 'No, do not send email', 'bp-group-check-in' ); ?></span></label>
					</div>
				</td>
			</tr>
			...
			<?php
				if ( true === bp_disable_group_messages() ) {
					...
					<tr id="groups-notification-settings-request-messages">
						<td></td>
						<td><?php _e( 'A group sends you a new message', 'bp-group-check-in' ); ?></td>
						<td class="yes">
							<div class="bp-radio-wrap">
								...
								<label for="notification-groups-messages-yes"><span class="bp-screen-reader-text"><?php _e( 'Yes, send email', 'bp-group-check-in' ); ?></span></label>
							</div>
						</td>
						<td class="no">
							<div class="bp-radio-wrap">
								...
								<label for="notification-groups-messages-no"><span class="bp-screen-reader-text"><?php _e( 'No, do not send email', 'bp-group-check-in' ); ?></span></label>
							</div>
						</td>
					</tr>
					<?php
