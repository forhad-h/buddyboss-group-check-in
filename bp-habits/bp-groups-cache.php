 ...
 *
 * This function is called in two places in the bgci_bp_habits_Group class:
 ...
 *                                         group ids.
 */
function bgci_bp_habits_update_meta_cache( $group_ids = false ) {
	$bp = buddypress();
	...
	$cache_args = array(
		...
		'cache_key_prefix' => 'bgci_bp_habits_groupmeta',
	);
 ...
 */
function bgci_bp_habits_delete_group_cache( $group_id = 0 ) {
	wp_cache_delete( $group_id, 'bgci_bp_habits' );
}
add_action( 'groups_delete_group', 'bgci_bp_habits_delete_group_cache' );
add_action( 'groups_update_group', 'bgci_bp_habits_delete_group_cache' );
add_action( 'groups_details_updated', 'bgci_bp_habits_delete_group_cache' );
add_action( 'groups_settings_updated', 'bgci_bp_habits_delete_group_cache' );
 ...
 */
function bgci_bp_habits_delete_group_cache_on_metadata_change( $meta_id, $group_id ) {
	wp_cache_delete( $group_id, 'bgci_bp_habits' );
}
add_action( 'updated_group_meta', 'bgci_bp_habits_delete_group_cache_on_metadata_change', 10, 2 );
add_action( 'added_group_meta', 'bgci_bp_habits_delete_group_cache_on_metadata_change', 10, 2 );
 ...
 * @param bgci_bp_habits_Group $group_obj Group object.
 */
function bgci_bp_habits_clear_group_creator_cache( $group_id, $group_obj ) {
	// Clears the 'total groups' for this user.
	groups_clear_group_user_object_cache( $group_obj->id, $group_obj->creator_id );
}
add_action( 'groups_created_group', 'bgci_bp_habits_clear_group_creator_cache', 10, 2 );
 ...
 */
function bgci_bp_habits_clear_group_members_caches( $group_obj, $user_ids ) {
	// Clears the 'total groups' cache for each member in a group.
	foreach ( (array) $user_ids as $user_id ) {
		groups_clear_group_user_object_cache( $group_obj->id, $user_id );
	}
}
add_action( 'bgci_bp_habits_delete_group', 'bgci_bp_habits_clear_group_members_caches', 10, 2 );
 ...
 */
function bgci_bp_habits_clear_invite_count_for_user( $user_id ) {
	wp_cache_delete( $user_id, 'bp_group_invite_count' );
}
add_action( 'groups_accept_invite', 'bgci_bp_habits_clear_invite_count_for_user' );
add_action( 'groups_reject_invite', 'bgci_bp_habits_clear_invite_count_for_user' );
add_action( 'groups_delete_invite', 'bgci_bp_habits_clear_invite_count_for_user' );
 ...
 * @param int $user_id  The user ID.
 */
function bgci_bp_habits_clear_invite_count_on_uninvite( $group_id, $user_id ) {
	bgci_bp_habits_clear_invite_count_for_user( $user_id );
}
add_action( 'groups_uninvite_user', 'bgci_bp_habits_clear_invite_count_on_uninvite', 10, 2 );
 ...
 */
function bgci_bp_habits_clear_invite_count_on_send( $group_id, $invited_users ) {
	foreach ( $invited_users as $user_id ) {
		bgci_bp_habits_clear_invite_count_for_user( $user_id );
	}
}
add_action( 'groups_send_invites', 'bgci_bp_habits_clear_invite_count_on_send', 10, 2 );
 ...
 * using {@link bgci_bp_habits_Member::save()}.  Usually via a plugin.
 *
 * @since BuddyPress 2.1.0
 *
 * @param bgci_bp_habits_Member $member Member object.
 */
function groups_clear_group_administrator_cache_on_member_save( bgci_bp_habits_Member $member ) {
 ...
 * @param int $group_id Group ID.
 */
function bgci_bp_habits_clear_group_administrator_cache_on_member_delete( $user_id, $group_id ) {
	groups_clear_group_administrator_cache( $group_id );
}
add_action( 'bgci_bp_habits_member_after_delete', 'bgci_bp_habits_clear_group_administrator_cache_on_member_delete', 10, 2 );
 ...
 * @since BuddyPress 2.6.0
 *
 * @param bgci_bp_habits_Member $member BP Groups Member instance.
 */
function bgci_bp_habits_clear_user_group_cache_on_membership_save( bgci_bp_habits_Member $member ) {
	wp_cache_delete( $member->user_id, 'bgci_bp_habits_memberships_for_user' );
	wp_cache_delete( $member->id, 'bgci_bp_habits_memberships' );
}
add_action( 'groups_member_before_save', 'bgci_bp_habits_clear_user_group_cache_on_membership_save' );
add_action( 'groups_member_before_remove', 'bgci_bp_habits_clear_user_group_cache_on_membership_save' );
 ...
 */
function bgci_bp_habits_clear_user_group_cache_on_invitation_save( BP_Invitation $invitation ) {
	if ( sanitize_key( 'bgci_bp_habits_Invitation_Manager' ) !== $invitation->class ) {
		return;
	}

	wp_cache_delete( $invitation->id, 'bgci_bp_habits_invitations_as_memberships' );
}
add_action( 'bp_invitation_after_save', 'bgci_bp_habits_clear_user_group_cache_on_invitation_save', 10, 2 );
 ...
 */
function bgci_bp_habits_clear_user_group_cache_on_invitation_change( $args ) {
	$args['fields' ] = 'ids';
	$affected_invitation_ids = groups_get_invites( $args );
	foreach ( $affected_invitation_ids as $invitation_id ) {
		wp_cache_delete( $invitation_id, 'bgci_bp_habits_invitations_as_memberships' );
	}
}
add_action( 'bp_invitation_before_delete', 'bgci_bp_habits_clear_user_group_cache_on_invitation_change' );
add_action( 'bp_invitation_before_update', 'bgci_bp_habits_clear_user_group_cache_on_invitation_change' );
 ...
 */
function bgci_bp_habits_clear_user_group_cache_on_other_events( $user_id, $group_id ) {
	wp_cache_delete( $user_id, 'bgci_bp_habits_memberships_for_user' );

	$membership = new bgci_bp_habits_Member( $user_id, $group_id );
	wp_cache_delete( $membership->id, 'bgci_bp_habits_memberships' );
}
add_action( 'bgci_bp_habits_member_before_delete', 'bgci_bp_habits_clear_user_group_cache_on_other_events', 10, 2 );
 ...
 */
function bgci_bp_habits_reset_cache_incrementor() {
	return bp_core_reset_incrementor( 'bgci_bp_habits' );
}
add_action( 'groups_group_after_save', 'bgci_bp_habits_reset_cache_incrementor' );
add_action( 'bgci_bp_habits_delete_group', 'bgci_bp_habits_reset_cache_incrementor' );
add_action( 'updated_group_meta', 'bgci_bp_habits_reset_cache_incrementor' );
add_action( 'deleted_group_meta', 'bgci_bp_habits_reset_cache_incrementor' );
add_action( 'added_group_meta', 'bgci_bp_habits_reset_cache_incrementor' );
 ...
 * @return bool True on success, false on failure.
 */
function bgci_bp_habits_reset_cache_incrementor_on_group_term_change( $object_id, $terms, $tt_ids, $taxonomy ) {
	$tax_object = get_taxonomy( $taxonomy );
	if ( $tax_object && in_array( 'bp_group', $tax_object->object_type, true ) ) {
		return bgci_bp_habits_reset_cache_incrementor();
	}

	return false;
}
add_action( 'bp_set_object_terms', 'bgci_bp_habits_reset_cache_incrementor_on_group_term_change', 10, 4 );
 ...
 * @return bool True on success, false on failure.
 */
function bgci_bp_habits_reset_cache_incrementor_on_group_term_remove( $object_id, $terms, $taxonomy ) {
	$tax_object = get_taxonomy( $taxonomy );
	if ( $tax_object && in_array( 'bp_group', $tax_object->object_type, true ) ) {
		return bgci_bp_habits_reset_cache_incrementor();
	}
...
add_action( 'bp_remove_object_terms', 'bgci_bp_habits_reset_cache_incrementor_on_group_term_remove', 10, 3 );
