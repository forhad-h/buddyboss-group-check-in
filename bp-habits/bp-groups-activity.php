...
function groups_register_activity_actions() {
	...
	bp_activity_set_action(
		$bp->groups->id,
		'activity_update',
		__( 'Posted a status update', 'bp-group-check-in' ),
		'bgci_bp_habits_format_activity_action_activity_update',
		__( 'Updates', 'bp-group-check-in' ),
	...
	bp_activity_set_action(
		$bp->groups->id,
		'created_group',
		__( 'Created a group', 'bp-group-check-in' ),
		'bgci_bp_habits_format_activity_action_created_group',
		__( 'New Groups', 'bp-group-check-in' ),
	...
	bp_activity_set_action(
		$bp->groups->id,
		'joined_group',
		__( 'Joined a group', 'bp-group-check-in' ),
		'bgci_bp_habits_format_activity_action_joined_group',
		__( 'Group Memberships', 'bp-group-check-in' ),
		array( 'activity', 'group', 'member', 'member_groups' )
	);
	...
	bp_activity_set_action(
		...
		__( 'Group details edited', 'bp-group-check-in' ),
		'bgci_bp_habits_format_activity_action_group_details_updated',
		__( 'Group Updates', 'bp-group-check-in' ),
		array( 'activity', 'group', 'member', 'member_groups' )
	);
 ...
 * @return bgci_bp_habits_Group  The group object the activity belongs to.
 */
function bgci_bp_habits_get_activity_group( $group_id = 0 ) {
 ...
 * @return string
 */
function bgci_bp_habits_format_activity_action_created_group( $action, $activity ) {
	$user_link = bp_core_get_userlink( $activity->user_id );

	$group      = groups_get_group( $activity->item_id );
	$group_link = '<a href="' . esc_url( bp_get_group_permalink( $group ) ) . '">' . esc_html( $group->name ) . '</a>';

	$action = sprintf( __( '%1$s created the group %2$s', 'bp-group-check-in' ), $user_link, $group_link );
 ...
 * @return string
 */
function bgci_bp_habits_format_activity_action_joined_group( $action, $activity ) {
	$user_link = bp_core_get_userlink( $activity->user_id );

	$group      = groups_get_group( $activity->item_id );
	$group_link = '<a href="' . esc_url( bp_get_group_permalink( $group ) ) . '">' . esc_html( $group->name ) . '</a>';

	$action = sprintf( __( '%1$s joined the group %2$s', 'bp-group-check-in' ), $user_link, $group_link );
	 ...
	 * @param object $activity Activity data object.
	 */
	return apply_filters( 'bgci_bp_habits_format_activity_action_joined_group', $action, $activity );
}
 ...
 * @return string
 */
function bgci_bp_habits_format_activity_action_activity_update( $action, $activity ) {
	$user_link = bp_core_get_userlink( $activity->user_id );

	$group      = groups_get_group( $activity->item_id );
	$group_link = '<a href="' . esc_url( bp_get_group_permalink( $group ) ) . '">' . esc_html( $group->name ) . '</a>';

	$action = sprintf( __( '%1$s posted an update in the group %2$s', 'bp-group-check-in' ), $user_link, $group_link );
	 ...
	 * @param object $activity Activity data object.
	 */
	return apply_filters( 'bgci_bp_habits_format_activity_action_activity_update', $action, $activity );
}
 ...
 * @return string
 */
function bgci_bp_habits_format_activity_action_group_details_updated( $action, $activity ) {
	$user_link = bp_core_get_userlink( $activity->user_id );

	$group      = bgci_bp_habits_get_activity_group( $activity->item_id );
	$group_link = '<a href="' . esc_url( bp_get_group_permalink( $group ) ) . '">' . esc_html( $group->name ) . '</a>';

	/*
	 * Changed group details are stored in groupmeta, keyed by the activity
	 * timestamp. See {@link bgci_bp_habits_group_details_updated_add_activity()}.
	 */
	...
	if ( empty( $changed ) ) {
		$action = sprintf( __( '%1$s updated details for the group %2$s', 'bp-group-check-in' ), $user_link, $group_link );
		...
		$action = sprintf( __( '%1$s changed the name and description of the group %2$s', 'bp-group-check-in' ), $user_link, $group_link );

		// Name only.
	} elseif ( ! empty( $changed['name']['old'] ) && ! empty( $changed['name']['new'] ) ) {
		$action = sprintf( __( '%1$s changed the name of the group %2$s from "%3$s" to "%4$s"', 'bp-group-check-in' ), $user_link, $group_link, esc_html( $changed['name']['old'] ), esc_html( $changed['name']['new'] ) );

		// Description only.
	} elseif ( ! empty( $changed['description']['old'] ) && ! empty( $changed['description']['new'] ) ) {
		$action = sprintf( __( '%1$s changed the description of the group %2$s from "%3$s" to "%4$s"', 'bp-group-check-in' ), $user_link, $group_link, esc_html( $changed['description']['old'] ), esc_html( $changed['description']['new'] ) );

	} elseif ( ! empty( $changed['slug']['old'] ) && ! empty( $changed['slug']['new'] ) ) {
		$action = sprintf( __( '%1$s changed the permalink of the group %2$s.', 'bp-group-check-in' ), $user_link, $group_link );
 ...
 * @return array
 */
function bgci_bp_habits_prefetch_activity_object_data( $activities ) {
	$group_ids = array();
	...
	if ( ! empty( $group_ids ) ) {
		...
		foreach ( $group_ids as $group_id ) {
			if ( false === wp_cache_get( $group_id, 'bgci_bp_habits' ) ) {
		...
		if ( ! empty( $uncached_ids ) ) {
			...
			foreach ( $groups as $group ) {
				wp_cache_set( $group->id, $group, 'bgci_bp_habits' );
			}
		}
	}
...
add_filter( 'bp_activity_prefetch_object_data', 'bgci_bp_habits_prefetch_activity_object_data' );
 ...
 * @return array
 */
function bgci_bp_habits_filter_activity_scope( $retval = array(), $filter = array() ) {
...
add_filter( 'bp_activity_set_groups_scope_args', 'bgci_bp_habits_filter_activity_scope', 10, 2 );
...
function groups_record_activity( $args = '' ) {
	...
	if ( ! empty( $args['item_id'] ) ) {
		$group = bgci_bp_habits_get_activity_group( $args['item_id'] );
		...
		if ( ! empty( $activity->id ) ) {
			...
			if ( ! bp_activity_user_can_edit( $activity ) ) {
				if ( 'wp_error' === $args['error_type'] ) {
					return new WP_Error( 'error', __( 'Allowed time for editing this activity is passed already, you can not edit now.', 'bp-group-check-in' ) );
				} else {
					return false;
				}
			}
 ...
 * @return bool
 */
function bgci_bp_habits_filter_activity_can_comment( $retval, $activity = null ) {
...
add_filter( 'bp_activity_can_comment', 'bgci_bp_habits_filter_activity_can_comment', 99, 1 );
 ...
 * @return bool
 */
function bgci_bp_habits_filter_activity_can_comment_reply( $retval, $comment ) {
	...
	return bgci_bp_habits_filter_activity_can_comment( $retval, $parent );
}
add_filter( 'bp_activity_can_comment_reply', 'bgci_bp_habits_filter_activity_can_comment_reply', 99, 2 );
 ...
 * @return false|null False on failure.
 */
function bgci_bp_habits_membership_accepted_add_activity( $user_id, $group_id ) {
	 ...
	 */
	$action = apply_filters_ref_array( 'groups_activity_membership_accepted_action', array( sprintf( __( '%1$s joined the group %2$s', 'bp-group-check-in' ), bp_core_get_userlink( $user_id ), '<a href="' . bp_get_group_permalink( $group ) . '">' . esc_attr( $group->name ) . '</a>' ), $user_id, &$group ) );
...
add_action( 'groups_membership_accepted', 'bgci_bp_habits_membership_accepted_add_activity', 10, 2 );
 ...
 *
 * @param  int             $group_id       ID of the group.
 * @param  bgci_bp_habits_Group $old_group      Group object before the details had been changed.
 ...
 */
function bgci_bp_habits_group_details_updated_add_activity( $group_id, $old_group, $notify_members ) {
...
add_action( 'groups_details_updated', 'bgci_bp_habits_group_details_updated_add_activity', 10, 3 );
 ...
 * @param int $group_id ID of the group.
 */
function bgci_bp_habits_delete_group_delete_all_activity( $group_id ) {
	if ( bp_is_active( 'activity' ) ) {
...
add_action( 'groups_delete_group', 'bgci_bp_habits_delete_group_delete_all_activity', 10 );
 ...
 */
function bgci_bp_habits_leave_group_delete_recent_activity( $group_id, $user_id ) {
...
add_action( 'groups_leave_group', 'bgci_bp_habits_leave_group_delete_recent_activity', 10, 2 );
add_action( 'groups_remove_member', 'bgci_bp_habits_leave_group_delete_recent_activity', 10, 2 );
add_action( 'groups_ban_member', 'bgci_bp_habits_leave_group_delete_recent_activity', 10, 2 );
