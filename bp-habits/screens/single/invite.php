...
function groups_screen_group_invite() {
	...
	if ( bp_is_action_variable( 'send', 0 ) ) {
		...
		bp_core_add_message( __( 'Group invites sent.', 'bp-group-check-in' ) );
		...
		add_action( 'bp_template_content', 'bgci_bp_habits_send_invite_screen_content' );
...
function bgci_bp_habits_send_invite_screen_content() {
...
function groups_remove_group_invite() {
	...
	$message   = __( 'Invite successfully removed', 'bp-group-check-in' );
	$redirect  = wp_get_referer();
	$error     = false;

	if ( ! bgci_bp_habits_user_can_send_invites( $group_id ) ) {
		$message = __( 'You are not allowed to send or remove invites', 'bp-group-check-in' );
		$error   = 'error';
	} elseif ( groups_check_for_membership_request( $friend_id, $group_id ) ) {
		$message = __( 'The member requested to join the group', 'bp-group-check-in' );
		$error   = 'error';
	} elseif ( ! groups_uninvite_user( $friend_id, $group_id ) ) {
		$message = __( 'There was an error removing the invite', 'bp-group-check-in' );
		$error   = 'error';
	}
