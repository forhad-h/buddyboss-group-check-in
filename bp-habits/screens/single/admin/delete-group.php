...
function groups_screen_group_admin_delete_group() {
	...
	if ( isset( $_REQUEST['delete-group-button'] ) && isset( $_REQUEST['delete-group-understand'] ) ) {
		...
		if ( ! groups_delete_group( $bp->groups->current_group->id ) ) {
			bp_core_add_message( __( 'There was an error deleting the group. Please try again.', 'bp-group-check-in' ), 'error' );
		} else {
			bp_core_add_message( __( 'The group was deleted successfully.', 'bp-group-check-in' ) );
