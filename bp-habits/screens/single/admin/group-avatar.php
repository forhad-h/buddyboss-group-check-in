...
function groups_screen_group_admin_avatar() {
	...
	if ( bp_is_action_variable( 'delete', 1 ) ) {
		...
		if ( bp_core_delete_existing_avatar(
			...
			bp_core_add_message( __( 'The group profile photo was deleted successfully!', 'bp-group-check-in' ) );
		} else {
			bp_core_add_message( __( 'There was a problem deleting the group profile photo. Please try again.', 'bp-group-check-in' ), 'error' );
		}
	}
	...
	if ( isset( $_POST['avatar-crop-submit'] ) ) {
		...
		if ( ! bp_core_avatar_handle_crop( $args ) ) {
			bp_core_add_message( __( 'There was a problem cropping the group profile photo.', 'bp-group-check-in' ), 'error' );
		} else {
			/**
			...
			bp_core_add_message( __( 'The new group profile photo was uploaded successfully.', 'bp-group-check-in' ) );
		}
	}
