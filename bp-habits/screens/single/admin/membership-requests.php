	...
	if ( $request_action && $user_id && $group_id ) {
		if ( 'accept' === $request_action ) {
			...
			if ( ! groups_accept_membership_request( false, $user_id, $group_id ) ) {
				bp_core_add_message( __( 'There was an error accepting the membership request. Please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				bp_core_add_message( __( 'Group membership request accepted', 'bp-group-check-in' ) );
			}
			...
			if ( ! groups_reject_membership_request( false, $user_id, $group_id ) ) {
				bp_core_add_message( __( 'There was an error rejecting the membership request. Please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				bp_core_add_message( __( 'Group membership request rejected', 'bp-group-check-in' ) );
			}
		}
