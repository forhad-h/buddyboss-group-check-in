	...
	if ( bp_is_item_admin() ) {
		...
		if ( isset( $_POST['save'] ) ) {
			...
			if ( empty( $_POST['group-name'] ) ) {
				bp_core_add_message( __( 'Groups must have a name and a description. Please try again.', 'bp-group-check-in' ), 'error' );
				...
				bp_core_add_message( __( 'There was an error updating group details. Please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				bp_core_add_message( __( 'Group details were successfully updated.', 'bp-group-check-in' ) );
			}
