	...
	if ( bp_action_variable( 1 ) && bp_action_variable( 2 ) && bp_action_variable( 3 ) ) {
		if ( bp_is_action_variable( 'promote', 1 ) && ( bp_is_action_variable( 'mod', 2 ) || bp_is_action_variable( 'admin', 2 ) ) && is_numeric( bp_action_variable( 3 ) ) ) {
			...
			if ( ! groups_promote_member( $user_id, $bp->groups->current_group->id, $status ) ) {
				bp_core_add_message( __( 'There was an error when promoting that user. Please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				bp_core_add_message( __( 'User promoted successfully', 'bp-group-check-in' ) );
			}
	...
	if ( bp_action_variable( 1 ) && bp_action_variable( 2 ) ) {
		if ( bp_is_action_variable( 'demote', 1 ) && is_numeric( bp_action_variable( 2 ) ) ) {
			...
			elseif ( ! groups_demote_member( $user_id, $bp->groups->current_group->id ) ) {
				bp_core_add_message( __( 'There was an error when demoting that user. Please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				bp_core_add_message( __( 'User demoted successfully', 'bp-group-check-in' ) );
			}
		...
		if ( bp_is_action_variable( 'ban', 1 ) && is_numeric( bp_action_variable( 2 ) ) ) {
			...
			if ( ! groups_ban_member( $user_id, $bp->groups->current_group->id ) ) {
				bp_core_add_message( __( 'There was an error when banning that user. Please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				bp_core_add_message( __( 'User banned successfully', 'bp-group-check-in' ) );
			}
		...
		if ( bp_is_action_variable( 'unban', 1 ) && is_numeric( bp_action_variable( 2 ) ) ) {
			...
			if ( ! groups_unban_member( $user_id, $bp->groups->current_group->id ) ) {
				bp_core_add_message( __( 'There was an error when unbanning that user. Please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				bp_core_add_message( __( 'User ban removed successfully', 'bp-group-check-in' ) );
			}
		...
		if ( bp_is_action_variable( 'remove', 1 ) && is_numeric( bp_action_variable( 2 ) ) ) {
			...
			if ( ! groups_remove_member( $user_id, $bp->groups->current_group->id ) ) {
				bp_core_add_message( __( 'There was an error removing that user from the group. Please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				bp_core_add_message( __( 'User removed successfully', 'bp-group-check-in' ) );
			}
