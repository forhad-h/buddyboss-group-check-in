...
function groups_screen_group_admin_settings() {
	...
	if ( isset( $_POST['save'] ) ) {
		 ...
		 */
		$current_types = bgci_bp_habits_get_group_type( bp_get_current_group_id(), false );
		...
		if ( isset( $_POST['group-types'] ) ) {
			...
			// Set group types.
			bgci_bp_habits_set_group_type( bp_get_current_group_id(), $current_types );
		...
		if ( ! groups_edit_group_settings( $_POST['group-id'], $enable_forum, $status, $invite_status, $activity_feed_status, $parent_id, $media_status, $document_status, $album_status, $message_status ) ) {
			bp_core_add_message( __( 'There was an error updating group settings. Please try again.', 'bp-group-check-in' ), 'error' );
		} else {
			bp_core_add_message( __( 'Group settings were successfully updated.', 'bp-group-check-in' ) );
		}
