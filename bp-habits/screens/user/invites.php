...
function groups_screen_group_invites() {
	...
	if ( bp_is_action_variable( 'accept' ) && is_numeric( $group_id ) ) {
		...
		if ( ! groups_accept_invite( bp_loggedin_user_id(), $group_id ) ) {
			bp_core_add_message( __( 'Group invite could not be accepted', 'bp-group-check-in' ), 'error' );
		} else {
			// Record this in activity feeds.
			$group = groups_get_group( $group_id );

			bp_core_add_message( sprintf( __( 'Group invite accepted. Visit %s.', 'bp-group-check-in' ), bp_get_group_link( $group ) ) );
		...
		if ( ! groups_reject_invite( bp_loggedin_user_id(), $group_id ) ) {
			bp_core_add_message( __( 'Group invite could not be rejected', 'bp-group-check-in' ), 'error' );
		} else {
			bp_core_add_message( __( 'Group invite rejected', 'bp-group-check-in' ) );
		}
