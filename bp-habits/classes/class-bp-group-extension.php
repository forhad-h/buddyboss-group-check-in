...
class BP_Group_Extension {
	...
	protected function get_default_screens() {
		...
		$screens = array(
			...
			'edit'   => array(
				'submit_text' => __( 'Save Changes', 'bp-group-check-in' ),
			),
			'admin'  => array(
	...
	/**
	 * Filter the access check in bgci_bp_habits_group_access_protection() for this extension.
	 *
	...
	public function group_access_protection( $user_can_visit, &$no_access_args ) {
		...
		if ( ! $user_can_visit && is_user_logged_in() ) {
			...
			$no_access_args['message']  = __( 'You do not have access to this content.', 'bp-group-check-in' );
	...
	protected function setup_admin_hooks() {
		...
		add_action( 'bgci_bp_habits_admin_meta_box_content_' . $this->slug, array( $this, 'call_admin_screen' ) );

		// Initialize the metabox.
		add_action( 'bgci_bp_habits_admin_meta_boxes', array( $this, '_meta_box_display_callback' ) );
	...
	public function _meta_box_display_callback() {
		...
		$callback       = function() use ( $extension_slug, $group_id ) {
			do_action( 'bgci_bp_habits_admin_meta_box_content_' . $extension_slug, $group_id );
		};
