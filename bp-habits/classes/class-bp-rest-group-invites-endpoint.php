...
class BP_REST_Group_Invites_Endpoint extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					'invite_id' => array(
						'description' => __( 'A unique numeric ID for the group invitation.', 'bp-group-check-in' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! $user_id ) {
			$retval = new WP_Error(
				...
				__( 'Sorry, you need to be logged in to see the group invitations.', 'bp-group-check-in' ),
				array(
		...
		if ( true === $retval && $request['group_id'] && ! $group instanceof bgci_bp_habits_Group ) {
			$retval = new WP_Error(
		...
		if ( true === $retval && $user_id_arg && ! $user instanceof WP_User ) {
			$retval = new WP_Error(
				...
				__( 'Invalid member ID.', 'bp-group-check-in' ),
				array(
					'status' => 404,
				)
			);
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( true === $retval && ! $invite ) {
			$retval = new WP_Error(
				...
				__( 'Invalid group invitation ID.', 'bp-group-check-in' ),
				array(
					'status' => 404,
				)
			);
	...
	public function create_item( $request ) {
		...
		if ( ! $invite_id ) {
			return new WP_Error(
				...
				__( 'Could not invite member to the group.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to create an invitation.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( true === $retval && ! bp_current_user_can( 'bp_moderate' ) && bp_loggedin_user_id() !== $inviter_id_arg ) {
			$retval = new WP_Error(
				'bp_rest_group_invite_cannot_create_item',
				__( 'Sorry, you are not allowed to create the invitation as requested.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function create_multiple_item_permissions_check( $request ) {
		...
		if ( true === $retval && ( empty( $user_ids ) || in_array( $inviter->ID, $user_ids, true ) ) ) {
			$retval = new WP_Error(
				'bp_rest_member_invalid_id',
				__( 'Invalid members ID.', 'bp-group-check-in' ),
				array(
					'status' => 404,
				)
			);
	...
	public function update_item( $request ) {
		...
		if ( ! $accept ) {
			return new WP_Error(
				...
				__( 'Could not accept group invitation.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
		...
		$accepted_member = new bgci_bp_habits_Member( $invite->user_id, $invite->item_id );
		 ...
		 *
		 * @param bgci_bp_habits_Member $accepted_member Accepted group member.
		 * @param bgci_bp_habits_Group  $group           The group object.
		 * @param WP_REST_Response $response        The response data.
		 * @param WP_REST_Request  $request         The request sent to the API.
		 *
	...
	public function update_item_permissions_check( $request ) {
		...
		if (
			...
			$retval = new WP_Error(
				'bp_rest_group_invite_cannot_update_item',
				__( 'Sorry, you are not allowed to accept the invitation as requested.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function delete_item( $request ) {
		...
		if ( ! $deleted ) {
			return new WP_Error(
				...
				__( 'Could not delete group invitation.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function delete_item_permissions_check( $request ) {
		...
		if (
			...
			$retval = new WP_Error(
				'bp_rest_group_invite_cannot_delete_item',
				__( 'Sorry, you are not allowed to delete the invitation as requested.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function get_endpoint_args_for_item_schema( $method = WP_REST_Server::CREATABLE, $multiple = false ) {
		...
		if ( WP_REST_Server::CREATABLE === $method || WP_REST_Server::EDITABLE === $method ) {
			...
			$args['message']['description'] = __( 'The optional message to send to the invited user.', 'bp-group-check-in' );
			...
			$args['send_invite']['description'] = __( 'Whether the invite should be sent to the invitee.', 'bp-group-check-in' );
		...
		if ( WP_REST_Server::CREATABLE === $method && true === $multiple ) {
			$args['user_id'] = array(
				'description'       => __( 'Return only invitations extended to those users.', 'bp-group-check-in' ),
				'type'              => 'array',
		...
		$schema = array(
			...
			'properties' => array(
				'id'            => array(
					...
					'description' => __( 'A unique numeric ID for the BP Invitation object.', 'bp-group-check-in' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
				'user_id'       => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'The ID of the user who is invited to join the Group.', 'bp-group-check-in' ),
					'type'        => 'integer',
				),
				'invite_sent'   => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'Whether the invite has been sent to the invitee.', 'bp-group-check-in' ),
					'type'        => 'boolean',
				),
				'inviter_id'    => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'The ID of the user who made the invite.', 'bp-group-check-in' ),
					'type'        => 'integer',
				),
				'group_id'      => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'The ID of the group to which the user has been invited.', 'bp-group-check-in' ),
					'type'        => 'integer',
				),
				'date_modified' => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( "The date the object was created or last updated, in the site's timezone.", 'bp-group-check-in' ),
					'type'        => 'string',
					'format'      => 'date-time',
				),
				'type'          => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'Invitation or request.', 'bp-group-check-in' ),
					'type'        => 'string',
				...
				'message'       => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'The raw and rendered versions for the content of the message.', 'bp-group-check-in' ),
					'type'        => 'object',
					'arg_options' => array(
					...
					'properties'  => array(
						'raw'      => array(
							'description' => __( 'Content for the object, as it exists in the database.', 'bp-group-check-in' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'rendered' => array(
							'description' => __( 'HTML content for the object, transformed for display.', 'bp-group-check-in' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
	...
	public function get_collection_params() {
		...
		$params['group_id'] = array(
			'description'       => __( 'ID of the group to limit results to.', 'bp-group-check-in' ),
			'required'          => false,
			'default'           => 0,
			'type'              => 'integer',
		...
		$params['user_id'] = array(
			'description'       => __( 'Return only invitations extended to this user.', 'bp-group-check-in' ),
			'required'          => false,
			'default'           => 0,
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
		...
		$params['inviter_id'] = array(
			'description'       => __( 'Return only invitations extended by this user.', 'bp-group-check-in' ),
			'required'          => false,
			'default'           => 0,
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
		...
		$params['invite_sent'] = array(
			'description'       => __( 'Limit result set to invites that have been sent, not sent, or include all.', 'bp-group-check-in' ),
			'default'           => 'sent',
			'type'              => 'string',
