 ...
 * @since BuddyPress 1.7.0
 */
class bgci_bp_habits_List_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		if ( bgci_bp_habits_get_group_types() ) {
			...
			add_filter( 'bgci_bp_habits_list_table_get_columns', array( $this, 'add_type_column' ) );
			add_filter( 'bgci_bp_habits_admin_get_group_custom_column', array( $this, 'column_content_group_type' ), 10, 3 );
			// Add the bulk change select.
			add_action( 'bgci_bp_habits_list_table_after_bulk_actions', array( $this, 'add_group_type_bulk_change_select' ) );
		}
	}
	...
	public function prepare_items() {
		...
		$this->group_type_ids = bgci_bp_habits_Group::get_group_type_ids();
		...
		// Group types
		$group_type = false;
		if ( isset( $_GET['bp-group-type'] ) && null !== bgci_bp_habits_get_group_type_object( $_GET['bp-group-type'] ) ) {
	...
	public function no_items() {
		_e( 'No groups found.', 'bp-group-check-in' );
	}
	...
	public function display() {
		...
		<?php
			...
			_e( 'Groups list', 'bp-group-check-in' );
		?>
		</h2>
	...
	protected function extra_tablenav( $which ) {
		 ...
		 */
		do_action( 'bgci_bp_habits_list_table_after_bulk_actions', $which );
	}
	...
	public function single_row( $item = array() ) {
		 ...
		 */
		$row_classes = apply_filters( 'bgci_bp_habits_admin_row_class', $row_classes, $item['id'] );
	...
	public function get_views() {
		...
		<?php
			/* translators: accessibility text */
			_e( 'Filter groups list', 'bp-group-check-in' );
		?>
		</h2>
		...
		<ul class="subsubsub">
			<li class="all"><a href="<?php echo esc_url( $url_base ); ?>" class="
												...
												if ( 'all' == $this->view ) {
													echo 'current';}
												?>
			"><?php _e( 'All', 'bp-group-check-in' ); ?></a> |</li>
			<li class="public"><a href="<?php echo esc_url( add_query_arg( 'group_status', 'public', $url_base ) ); ?>" class="
												   <?php
													if ( 'public' == $this->view ) {
														echo 'current';}
													?>
			"><?php printf( _n( 'Public <span class="count">(%s)</span>', 'Public <span class="count">(%s)</span>', $this->group_counts['public'], 'bp-group-check-in' ), number_format_i18n( $this->group_counts['public'] ) ); ?></a> |</li>
			<li class="private"><a href="<?php echo esc_url( add_query_arg( 'group_status', 'private', $url_base ) ); ?>" class="
													<?php
													if ( 'private' == $this->view ) {
														echo 'current';}
													?>
			"><?php printf( _n( 'Private <span class="count">(%s)</span>', 'Private <span class="count">(%s)</span>', $this->group_counts['private'], 'bp-group-check-in' ), number_format_i18n( $this->group_counts['private'] ) ); ?></a> |</li>
			<li class="hidden"><a href="<?php echo esc_url( add_query_arg( 'group_status', 'hidden', $url_base ) ); ?>" class="
												   <?php
													if ( 'hidden' == $this->view ) {
														echo 'current';}
													?>
			"><?php printf( _n( 'Hidden <span class="count">(%s)</span>', 'Hidden <span class="count">(%s)</span>', $this->group_counts['hidden'], 'bp-group-check-in' ), number_format_i18n( $this->group_counts['hidden'] ) ); ?></a></li>
			 ...
			 */
			do_action( 'bgci_bp_habits_list_table_get_views', $url_base, $this->view );
			?>
		</ul>
		<?php
	}
	...
	public function get_bulk_actions() {
		...
		return apply_filters(
			'bgci_bp_habits_list_table_get_bulk_actions',
			array(
				'delete' => __( 'Delete', 'bp-group-check-in' ),
			)
		);
	}
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters(
			'bgci_bp_habits_list_table_get_columns',
			array(
				'cb'          => '<input name type="checkbox" />',
				'comment'     => __( 'Name', 'bp-group-check-in' ),
				'description' => __( 'Description', 'bp-group-check-in' ),
				'status'      => __( 'Status', 'bp-group-check-in' ),
				'members'     => __( 'Members', 'bp-group-check-in' ),
				'last_active' => __( 'Last Active', 'bp-group-check-in' ),
			)
		);
	}
	...
	public function column_cb( $item = array() ) {
		/* translators: accessibility text */
		printf( '<label class="screen-reader-text" for="gid-%1$d">' . __( 'Select group %1$d', 'bp-group-check-in' ) . '</label><input type="checkbox" name="gid[]" value="%1$d" id="gid-%1$d" />', $item['id'] );
	}
	...
	public function column_comment( $item = array() ) {
		...
		// Rollover actions.
		// Edit.
		$actions['edit'] = sprintf( '<a href="%s">%s</a>', esc_url( $edit_url ), __( 'Edit', 'bp-group-check-in' ) );

		// Delete.
		$actions['delete'] = sprintf( '<a href="%s">%s</a>', esc_url( $delete_url ), __( 'Delete', 'bp-group-check-in' ) );

		// Visit.
		$actions['view'] = sprintf( '<a href="%s">%s</a>', esc_url( $view_url ), __( 'View', 'bp-group-check-in' ) );
		 ...
		 */
		$actions = apply_filters( 'bgci_bp_habits_admin_comment_row_actions', array_filter( $actions ), $item );
		...
		if ( buddypress()->avatar->show_avatars ) {
			$avatar = bp_core_fetch_avatar(
				array(
					...
					'alt'        => sprintf( __( 'Group logo of %s', 'bp-group-check-in' ), $group_name ),
					'width'      => '32',
					'height'     => '32',
					'title'      => $group_name,
				)
	...
	public function column_status( $item = array() ) {
		...
		switch ( $status ) {
			case 'public':
				$status_desc = __( 'Public', 'bp-group-check-in' );
				break;
			case 'private':
				$status_desc = __( 'Private', 'bp-group-check-in' );
				break;
			case 'hidden':
				$status_desc = __( 'Hidden', 'bp-group-check-in' );
				break;
		}
		 ...
		 */
		echo apply_filters_ref_array( 'bgci_bp_habits_admin_get_group_status', array( $status_desc, $item ) );
	}
	...
	public function column_members( $item = array() ) {
		 ...
		 * @parma array $item  The current group item in the loop.
		 */
		echo apply_filters_ref_array( 'bgci_bp_habits_admin_get_group_member_count', array( (int) $count, $item ) );
	}
	...
	public function column_last_active( $item = array() ) {
		 ...
		 * @parma array  $item        The current group item in the loop.
		 */
		echo apply_filters_ref_array( 'bgci_bp_habits_admin_get_group_last_active', array( $last_active, $item ) );
	}
	...
	public function column_default( $item = array(), $column_name = '' ) {
		 ...
		 * @param array  $item        The current group item in the loop.
		 */
		return apply_filters( 'bgci_bp_habits_admin_get_group_custom_column', '', $column_name, $item );
	}
	...
	public function add_type_column( $columns = array() ) {
		$columns['bp_group_type'] = __( 'Group Type', 'bp-group-check-in' );
	...
	public function column_content_group_type( $retval = '', $column_name, $item ) {
		 ...
		 * @parma array  $item   The current group item in the loop.
		 */
		echo apply_filters_ref_array( 'bgci_bp_habits_admin_get_group_type_column', array( $retval, $item ) );
	}
	...
	public function add_group_type_bulk_change_select( $which ) {
		...
		<div class="alignleft actions">
			<label class="screen-reader-text" for="<?php echo $id_name; ?>"><?php _e( 'Change group type to&hellip;', 'bp-group-check-in' ); ?></label>
			<select name="<?php echo $id_name; ?>" id="<?php echo $id_name; ?>" style="display:inline-block;float:none;">
				<option value=""><?php _e( 'Change group type to&hellip;', 'bp-group-check-in' ); ?></option>
				...
				<option value="remove_group_type"><?php _e( 'No Group Type', 'bp-group-check-in' ); ?></option>

			</select>
			<?php
			wp_nonce_field( 'bp-bulk-groups-change-type-' . bp_loggedin_user_id(), 'bp-bulk-groups-change-type-nonce' );
			submit_button( __( 'Change', 'bp-group-check-in' ), 'button', 'bp_change_group_type', false );
			?>
		</div>
		<?php
	}
