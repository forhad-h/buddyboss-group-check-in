...
class bgci_bp_habits_Template {
	 ...
	 * Constructor method.
	 *
	 * @see bgci_bp_habits_Group::get() for an in-depth description of arguments.
	 *
	 * @param array $args {
	 *     Array of arguments. Accepts all arguments accepted by
	 *     {@link bgci_bp_habits_Group::get()}. In cases where the default
	...
	function __construct( $args = array() ) {
		...
		if ( 'invites' == $type ) {
			...
			if ( groups_get_current_group() ) {
				...
				$group = groups_get_group( bgci_bp_habits_Group::get_id_from_slug( $r['slug'] ) );
			}

			// Backwards compatibility - the 'group_id' variable is not part of the
			// bgci_bp_habits_Group object, but we add it here for devs doing checks against it
			//
		...
		if ( (int) $this->total_group_count && (int) $this->pag_num ) {
			...
			$this->pag_links = paginate_links(
				array(
					...
					'prev_text' => __( '&larr;', 'bp-group-check-in' ),
					'next_text' => __( '&rarr;', 'bp-group-check-in' ),
					'mid_size'  => 1,
					'add_args'  => $add_args,
				)
			);
	 ...
	 *
	 * This method is used by {@link bgci_bp_habits()} as part of the while loop
