...
class BP_REST_Groups_Details_Endpoint extends WP_REST_Controller {
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( true === $retval && ! is_user_logged_in() && ( 'private' === bp_get_group_status( $group ) || 'hidden' === bp_get_group_status( $group ) ) ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you are not allowed to view group tabs.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		$schema = array(
			...
			'properties' => array(
				'tabs'          => array(
					...
					'description' => __( 'Groups directory tabs.', 'bp-group-check-in' ),
					'type'        => 'object',
					'readonly'    => true,
					'items'       => array(
						'type' => 'array',
				...
				'order_options' => array(
					'context'     => array( 'embed', 'view' ),
					'description' => __( 'Groups order by options.', 'bp-group-check-in' ),
					'type'        => 'array',
					'readonly'    => true,
				),
			),
	...
	public function get_collection_params() {
		$params = array(
			'type' => array(
				'description'       => __( 'Filter by.. active(Last Active), popular(Most Members), newest(Newly Created), alphabetical(Alphabetical)', 'bp-group-check-in' ),
				'type'              => 'string',
				'default'           => 'active',
	...
	public function bp_rest_legacy_get_groups_directory_nav_items() {
		...
		$nav_items['all'] = array(
			'text'     => __( 'All Groups', 'bp-group-check-in' ),
			'slug'     => 'all',
		...
		if ( is_user_logged_in() ) {
			...
			if ( $my_groups_count ) {
				$nav_items['personal'] = array(
					'text'     => __( 'My Groups', 'bp-group-check-in' ),
			...
			if ( bp_user_can_create_groups() ) {
				$nav_items['create'] = array(
					'text'     => __( 'Create a Group', 'bp-group-check-in' ),
	 ...
	 *
	 * @param bgci_bp_habits_Group $group Optional. Group object. Default: current group in loop.
	...
	public function bp_rest_legacy_get_group_component_filters() {
		...
		$filters_data['active']       = __( 'Last Active', 'bp-group-check-in' );
		$filters_data['popular']      = __( 'Most Members', 'bp-group-check-in' );
		$filters_data['newest']       = __( 'Newly Created', 'bp-group-check-in' );
		$filters_data['alphabetical'] = __( 'Alphabetical', 'bp-group-check-in' );
