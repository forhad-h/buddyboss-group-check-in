 ...
 * @since BuddyPress 1.5.0
 */
class bgci_bp_habits_Component extends BP_Component {
	 ...
	 * Group types.
	 *
	 * @see bgci_bp_habits_register_group_type()
	 *
	...
	public function __construct() {
		parent::start(
			'groups',
			__( 'Social Groups', 'bp-group-check-in' ),
	...
	public function late_includes() {
		...
		if ( bp_is_groups_component() ) {
			...
			// Theme compatibility.
			new bgci_bp_habits_Theme_Compat();
		}
	}
	 ...
	 *
	 * The bgci_bp_habits_SLUG constant is deprecated, and only used here for
	...
	public function setup_globals( $args = array() ) {
		...
		if ( ! defined( 'bgci_bp_habits_SLUG' ) ) {
			define( 'bgci_bp_habits_SLUG', $this->id );
		}

		// Global tables for groups component.
		$global_tables = array(
			'table_name'           => $bp->table_prefix . 'bgci_bp_habits',
			'table_name_members'   => $bp->table_prefix . 'bgci_bp_habits_members',
			'table_name_groupmeta' => $bp->table_prefix . 'bgci_bp_habits_groupmeta',
		);

		// Metadata tables for groups component.
		$meta_tables = array(
			'group' => $bp->table_prefix . 'bgci_bp_habits_groupmeta',
		);
		...
		$args = array(
			'slug'                  => bgci_bp_habits_SLUG,
			'root_slug'             => isset( $bp->pages->groups->slug ) ? $bp->pages->groups->slug : bgci_bp_habits_SLUG,
			'has_directory'         => true,
			...
			'search_string'         => __( 'Search Groups&hellip;', 'bp-group-check-in' ),
		...
		if ( bp_is_groups_component()
			&& ( ( $group_id = bgci_bp_habits_Group::group_exists( bp_current_action() ) )
				|| ( $group_id = bgci_bp_habits_Group::get_id_by_previous_slug( bp_current_action() ) ) )
			) {
			 ...
			 */
			$current_group_class = apply_filters( 'bgci_bp_habits_current_group_class', 'bgci_bp_habits_Group' );

			if ( $current_group_class == 'bgci_bp_habits_Group' ) {
				 ...
				 */
				$this->current_group = apply_filters( 'bgci_bp_habits_current_group_object', new $current_group_class( $group_id ) );
			}
			...
			$this->current_group->front_template = bgci_bp_habits_get_front_template( $this->current_group );
		...
		if ( bp_is_groups_directory() && bp_is_current_action( bp_get_groups_group_type_base() ) && bp_action_variable() ) {
			$matched_types = bgci_bp_habits_get_group_types(
				array(
					'has_directory'  => true,
		...
		$this->group_creation_steps = apply_filters(
			...
			array(
				'group-details'  => array(
					'name'     => __( 'Details', 'bp-group-check-in' ),
					'position' => 0,
				),
				'group-settings' => array(
					'name'     => __( 'Settings', 'bp-group-check-in' ),
					'position' => 10,
				),
			)
		);
		...
		if ( ! $disabled_avatar_uploads && $bp->avatar->show_avatars ) {
			$this->group_creation_steps['group-avatar'] = array(
				'name'     => __( 'Photo', 'bp-group-check-in' ),
				'position' => 20,
			);
		}
		...
		if ( bp_group_use_cover_image_header() ) {
			$this->group_creation_steps['group-cover-image'] = array(
				'name'     => __( 'Cover Photo', 'bp-group-check-in' ),
				'position' => 25,
			);
		}
		...
		if ( bp_is_active( 'friends' ) ) {
			$this->group_creation_steps['group-invites'] = array(
				'name'     => __( 'Invites', 'bp-group-check-in' ),
				'position' => 30,
			);
		}
	...
	public function setup_canonical_stack() {
		 ...
		 * @since BuddyPress 1.6.0
		 *
		 * @param string $value bgci_bp_habits_DEFAULT_EXTENSION constant if defined,
		 *                      else 'members'.
		 */
		$default_tab = ( '' === $default_tab ) ? 'members' : $default_tab;
		$this->default_extension = apply_filters( 'bgci_bp_habits_default_extension', defined( 'bgci_bp_habits_DEFAULT_EXTENSION' ) ? bgci_bp_habits_DEFAULT_EXTENSION : $default_tab );
	...
	public function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		if ( bp_is_user() ) {
			...
			$nav_name = __( 'Groups', 'bp-group-check-in' );
			$nav_name .= sprintf(
				' <span class="%s">%s</span>',
				esc_attr( $class ),
		...
		if ( ! empty( $user_domain ) ) {
			...
			$sub_nav[] = array(
				'name'            => __( 'My Groups', 'bp-group-check-in' ),
				'slug'            => 'my-groups',
			...
			$sub_nav[] = array(
				'name'            => __( 'Invitations', 'bp-group-check-in' ),
				'slug'            => 'invites',
				'parent_url'      => $groups_link,
				'parent_slug'     => $slug,
		...
		if ( bp_is_groups_component() && bp_is_single_item() ) {
			...
			$sub_nav[] = array(
				'name'            => sprintf( apply_filters( 'group_single_members_label', __( 'Members', 'bp-group-check-in' ) ) . __( ' %s', 'bp-group-check-in' ), '<span>' . number_format( $this->current_group->total_member_count ) . '</span>' ),
				'slug'            => 'members',
			...
			// 'name'              => __( 'All Members', 'bp-group-check-in' ),
			// 'slug'              => 'all-members',
			// 'position'          => 0,
			// ), $default_params );
			//
			// $sub_nav[] = array_merge( array(
			// 'name'              => __( 'Group Leaders', 'bp-group-check-in' ),
			// 'slug'              => 'leaders',
			// 'position'          => 10,
			// ), $default_params );
			...
			if ( bp_is_active( 'activity' ) ) {
				$sub_nav[] = array(
					'name'            => __( 'Feed', 'bp-group-check-in' ),
					'slug'            => 'activity',
					'parent_url'      => $group_link,
					'parent_slug'     => $this->current_group->slug,
			...
			if ( bp_enable_group_hierarchies() ) {
				...
				if ( $total_descendant = count( $descendant_groups ) ) {
					$sub_nav[] = array(
						'name'            => sprintf( __( 'Subgroups', 'bp-group-check-in' ), '<span>' . number_format( $total_descendant ) . '</span>' ),
						'slug'            => 'subgroups',
						'parent_url'      => $group_link,
						'parent_slug'     => $this->current_group->slug,
			...
			if ( groups_check_for_membership_request( bp_loggedin_user_id(), $this->current_group->id ) || bp_current_user_can( 'groups_request_membership', array( 'group_id' => $this->current_group->id ) ) ) {
				...
				if ( isset( $parent_group_id ) && $parent_group_id > 0 && true === bp_enable_group_hierarchies() && true === bp_enable_group_restrict_invites() ) {
					...
					if ( false !== $is_member ) {
						$sub_nav[] = array(
							'name'            => __( 'Request Access', 'bp-group-check-in' ),
			...
			if ( bp_is_active( 'friends' ) && bgci_bp_habits_user_can_send_invites() ) {
				$sub_nav[] = array(
					'name'            => __( 'Send Invites', 'bp-group-check-in' ),
					'slug'            => 'invite',
					'parent_url'      => $group_link,
					'parent_slug'     => $this->current_group->slug,
				...
				$sub_nav[] = array_merge(
					array(
						'name'     => __( 'Pending Invites', 'bp-group-check-in' ),
			...
			if ( bp_is_active( 'media' ) && bp_is_group_media_support_enabled() ) {
				$sub_nav[] = array(
					'name'            => __( 'Photos', 'bp-group-check-in' ),
					'slug'            => 'photos',
					'parent_url'      => $group_link,
					'parent_slug'     => $this->current_group->slug,
				...
				if ( bp_is_group_albums_support_enabled() ) {
					$sub_nav[] = array(
						'name'            => __( 'Albums', 'bp-group-check-in' ),
						'slug'            => 'albums',
						'parent_url'      => $group_link,
						'parent_slug'     => $this->current_group->slug,
			...
			if ( bp_is_active( 'media' ) && bp_is_group_document_support_enabled() ) {
				$sub_nav[] = array(
					'name'            => __( 'Documents', 'bp-group-check-in' ),
					'slug'            => 'documents',
					'parent_url'      => $group_link,
					'parent_slug'     => $this->current_group->slug,
			...
			if ( true === bp_disable_group_messages() && bp_is_active( 'messages' ) && $show ) {
				$sub_nav[] = array(
					'name'            => __( 'Send Messages', 'bp-group-check-in' ),
					'slug'            => 'messages',
					'parent_url'      => $group_link,
					'parent_slug'     => $this->current_group->slug,
			...
			if ( bp_is_item_admin() ) {
				$sub_nav[] = array(
					'name'            => __( 'Manage', 'bp-group-check-in' ),
					'slug'            => 'admin',
					'parent_url'      => $group_link,
					'parent_slug'     => $this->current_group->slug,
				...
				$sub_nav[] = array_merge(
					array(
						'name'     => __( 'Members', 'bp-group-check-in' ),
						'slug'     => 'manage-members',
						'position' => 30,
					),
					$default_params
				...
				if ( 'private' == $this->current_group->status ) {
					$sub_nav[] = array_merge(
						array(
							'name'     => __( 'Requests', 'bp-group-check-in' ),
				...
				$sub_nav[] = array_merge(
					array(
						'name'     => __( 'Delete', 'bp-group-check-in' ),
						'slug'     => 'delete-group',
						'position' => 1000,
					),
					$default_params
	...
	public function setup_admin_bar( $wp_admin_nav = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			$title   = __( 'Groups', 'bp-group-check-in' );
			$pending = __( 'No Pending Invites', 'bp-group-check-in' );

			if ( ! empty( $count ) ) {
				$title = sprintf(
					/* translators: %s: Group invitation count for the current user */
					__( 'Groups %s', 'bp-group-check-in' ),
					'<span class="count">' . bp_core_number_format( $count ) . '</span>'
				);

				$pending = sprintf(
					/* translators: %s: Group invitation count for the current user */
					__( 'Pending Invites %s', 'bp-group-check-in' ),
					'<span class="count">' . bp_core_number_format( $count ) . '</span>'
				);
			}
			...
			$wp_admin_nav[] = array(
				...
				'title'    => __( 'My Groups', 'bp-group-check-in' ),
				'href'     => $groups_link,
				'position' => 10,
			);
			...
			if ( bp_user_can_create_groups() ) {
				$wp_admin_nav[] = array(
					...
					'title'    => __( 'Create Group', 'bp-group-check-in' ),
	...
	public function setup_title() {
		...
		if ( bp_is_groups_component() ) {
			...
			if ( bp_is_my_profile() && ! bp_is_single_item() ) {
				$bp->bp_options_title = __( 'My Groups', 'bp-group-check-in' );
				...
				$bp->bp_options_avatar = bp_core_fetch_avatar(
					array(
						...
						'type'    => 'thumb',
						'alt'     => sprintf( __( 'Profile photo of %s', 'bp-group-check-in' ), bp_get_displayed_user_fullname() ),
					)
				);
				...
				$bp->bp_options_avatar = bp_core_fetch_avatar(
					array(
						...
						'alt'        => __( 'Group Profile Photo', 'bp-group-check-in' ),
					)
				);

				if ( empty( $bp->bp_options_avatar ) ) {
					$bp->bp_options_avatar = '<img src="' . esc_url( bp_core_avatar_default_thumb() ) . '" alt="' . esc_attr__( 'No Group Profile Photo', 'bp-group-check-in' ) . '" class="avatar" />';
				}
			}
		}
	...
	public function setup_cache_groups() {
		...
		wp_cache_add_global_groups(
			array(
				...
				'group_meta',
				'bgci_bp_habits_memberships',
				'bgci_bp_habits_memberships_for_user',
			)
		);
