...
class BP_REST_Attachments_Group_Cover_Endpoint extends WP_REST_Controller {
	...
	public function get_item( $request ) {
		...
		if ( empty( $cover_url ) ) {
			return new WP_Error(
				...
				__( 'Sorry, there was a problem fetching this group cover.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( true === $retval && bp_disable_group_cover_image_uploads() ) {
			$retval = new WP_Error(
				...
				__( 'Sorry, group cover upload is disabled.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function delete_item( $request ) {
		...
		if ( ! $deleted ) {
			return new WP_Error(
				...
				__( 'Sorry, there was a problem deleting this group cover.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( true === $retval && ! is_user_logged_in() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to perform this action.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'warning' => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Warning while uploading the cover photo.', 'bp-group-check-in' ),
					'type'        => 'string',
					'readonly'    => true,
				),
			),
