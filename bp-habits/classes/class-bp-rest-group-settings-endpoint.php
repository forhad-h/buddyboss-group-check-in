...
class BP_REST_Group_Settings_Endpoint extends WP_REST_Controller {
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					...
					'nav' => array(
						'description'       => __( 'Navigation item slug.', 'bp-group-check-in' ),
						'type'              => 'string',
						'required'          => true,
						'enum'              => $this->nav,
	...
	public function get_item( $request ) {
		...
		if ( empty( $fields ) ) {
			return new WP_Error(
				...
				__( 'Sorry, you are not allowed to see the group settings options.', 'bp-group-check-in' ),
				array(
					'status' => 400,
				)
			);
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				...
				__( 'Sorry, you are not allowed to see the group settings.', 'bp-group-check-in' ),
				array(
		...
		if ( true === $retval && ! bp_is_active( 'groups' ) ) {
			$retval = new WP_Error(
				...
				__( 'Sorry, Groups component was not enabled.', 'bp-group-check-in' ),
				array(
					'status' => '404',
				)
			);
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you are not allowed to update the group settings.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( WP_REST_Server::EDITABLE === $method ) {
			...
			$args = array(
				...
				'fields' => array(
					...
					'description' => __( 'The list of fields Objects to update with name and value of the field.', 'bp-group-check-in' ),
					'type'        => 'object',
					'required'    => true,
				),
			);
	...
	public function get_item_schema() {
		$schema = array(
			...
			'title'      => 'bgci_bp_habits_details',
			'type'       => 'object',
			'properties' => array(
				'label'       => array(
					'context'     => array( 'embed', 'view' ),
					'description' => __( 'Label for the setting.', 'bp-group-check-in' ),
					'type'        => 'object',
					'readonly'    => true,
					'items'       => array(
						'type' => 'array',
				...
				'name'        => array(
					'context'     => array( 'embed', 'view' ),
					'description' => __( 'Setting field name.', 'bp-group-check-in' ),
					'type'        => 'array',
					'readonly'    => true,
				),
				'description' => array(
					'context'     => array( 'embed', 'view' ),
					'description' => __( 'Setting field description.', 'bp-group-check-in' ),
					'type'        => 'array',
					'readonly'    => true,
				),
				'type'        => array(
					'context'     => array( 'embed', 'view' ),
					'description' => __( 'Field type for the setting.', 'bp-group-check-in' ),
					'type'        => 'array',
					'readonly'    => true,
				),
				'value'       => array(
					'context'     => array( 'embed', 'view' ),
					'description' => __( 'Selected value for the setting.', 'bp-group-check-in' ),
					'type'        => 'array',
					'readonly'    => true,
				),
				'options'     => array(
					'context'     => array( 'embed', 'view' ),
					'description' => __( 'Available options for the setting.', 'bp-group-check-in' ),
					'type'        => 'array',
					'readonly'    => true,
				),
			),
	...
	protected function get_settings_fields( $group_id ) {
		...
		$fields[] = array(
			'label'       => esc_html__( 'Privacy Options', 'bp-group-check-in' ),
			'name'        => 'group-status',
			'description' => '',
			'field'       => 'radio',
			...
			'options'     => array(
				array(
					'label'             => esc_html__( 'This is a public group', 'bp-group-check-in' ),
					'value'             => 'public',
					'description'       => '<ul id="public-group-description">' .
											'<li>' . esc_html__( 'Any site member can join this group.', 'bp-group-check-in' ) . '</li>' .
											'<li>' . esc_html__( 'This group will be listed in the groups directory and in search results.', 'bp-group-check-in' ) . '</li>' .
											'<li>' . esc_html__( 'Group content and activity will be visible to any site member.', 'bp-group-check-in' ) . '</li>' .
										'</ul>',
					'is_default_option' => 'public' === bp_get_new_group_status() || ! bp_get_new_group_status(),
				),
				array(
					'label'             => esc_html__( 'This is a private group', 'bp-group-check-in' ),
					'value'             => 'private',
					'description'       => '<ul id="public-group-description">' .
											'<li>' . esc_html__( 'Only people who request membership and are accepted can join the group.', 'bp-group-check-in' ) . '</li>' .
											'<li>' . esc_html__( 'This group will be listed in the groups directory and in search results.', 'bp-group-check-in' ) . '</li>' .
											'<li>' . esc_html__( 'Group content and activity will only be visible to members of the group.', 'bp-group-check-in' ) . '</li>' .
										'</ul>',
					'is_default_option' => 'private' === bp_get_new_group_status(),
				),
				array(
					'label'             => esc_html__( 'This is a hidden group', 'bp-group-check-in' ),
					'value'             => 'hidden',
					'description'       => '<ul id="public-group-description">' .
											'<li>' . esc_html__( 'Only people who are invited can join the group.', 'bp-group-check-in' ) . '</li>' .
											'<li>' . esc_html__( 'This group will not be listed in the groups directory or search results', 'bp-group-check-in' ) . '</li>' .
											'<li>' . esc_html__( 'Group content and activity will only be visible to members of the group.', 'bp-group-check-in' ) . '</li>' .
										'</ul>',
					'is_default_option' => 'private' === bp_get_new_group_status(),
				),
		...
		$fields[] = array(
			'label'       => esc_html__( 'Group Invitations', 'bp-group-check-in' ),
			'name'        => 'group-invite-status',
			'description' => esc_html__( 'Which members of this group are allowed to invite others?', 'bp-group-check-in' ),
			'field'       => 'radio',
			'value'       => bp_group_get_invite_status( $group_id ),
			'options'     => array(
				array(
					'label'             => esc_html__( 'All group members', 'bp-group-check-in' ),
					'value'             => 'members',
					'description'       => '',
					'is_default_option' => 'members' === bp_group_get_invite_status( $group_id ),
				),
				array(
					'label'             => esc_html__( 'Organizers and Moderators only', 'bp-group-check-in' ),
					'value'             => 'mods',
					'description'       => '',
					'is_default_option' => 'mods' === bp_group_get_invite_status( $group_id ),
				),
				array(
					'label'             => esc_html__( 'Organizers only', 'bp-group-check-in' ),
					'value'             => 'admins',
					'description'       => '',
		...
		$fields[] = array(
			'label'       => esc_html__( 'Activity Feeds', 'bp-group-check-in' ),
			'name'        => 'group-activity-feed-status',
			'description' => esc_html__( 'Which members of this group are allowed to post into the activity feed?', 'bp-group-check-in' ),
			'field'       => 'radio',
		...
		if ( bp_is_active( 'media' ) && bp_is_group_media_support_enabled() ) {
			$fields[] = array(
				'label'       => esc_html__( 'Group Photos', 'bp-group-check-in' ),
				'name'        => 'group-media-status',
				'description' => esc_html__( 'Which members of this group are allowed to manage photos?', 'bp-group-check-in' ),
				'field'       => 'radio',
		...
		if ( bp_is_active( 'media' ) && bp_is_group_media_support_enabled() ) {
			$fields[] = array(
				'label'       => esc_html__( 'Group Albums', 'bp-group-check-in' ),
				'name'        => 'group-album-status',
				'description' => esc_html__( 'Which members of this group are allowed to manage albums?', 'bp-group-check-in' ),
				'field'       => 'radio',
		...
		if ( bp_is_active( 'media' ) && bp_is_group_document_support_enabled() ) {
			$fields[] = array(
				'label'       => esc_html__( 'Group Documents', 'bp-group-check-in' ),
				'name'        => 'group-document-status',
				'description' => esc_html__( 'Which members of this group are allowed to manage documents?', 'bp-group-check-in' ),
				'field'       => 'radio',
		...
		if ( bp_is_active( 'messages' ) && function_exists( 'bp_disable_group_messages' ) && true === bp_disable_group_messages() ) {
			$fields[] = array(
				'label'       => esc_html__( 'Group Messages', 'bp-group-check-in' ),
				'name'        => 'group-message-status',
				'description' => esc_html__( 'Which members of this group are allowed to send group messages?', 'bp-group-check-in' ),
				'field'       => 'radio',
		...
		// Group Types.
		$group_types = bgci_bp_habits_get_group_types( array( 'show_in_create_screen' => true ), 'objects' );
		...
		if ( $group_types ) {
			$current_types    = bgci_bp_habits_get_group_type( $group_id, false );
			$group_type_field = array(
				'label'       => esc_html__( 'Group Type', 'bp-group-check-in' ),
				'name'        => 'group-types',
				'description' => esc_html__( 'What type of group is this? (optional)', 'bp-group-check-in' ),
				'field'       => 'select',
			...
			$group_hierarchies['options'][] = array(
				'label'             => __( 'Select Group Type', 'bp-group-check-in' ),
				'value'             => '',
				'description'       => '',
				'is_default_option' => '',
			);
							...
							if ( isset( $include_group_type ) && ! empty( $include_group_type ) ) {
								if ( in_array( $type->name, $include_group_type, true ) ) {
									$group_type_field['options'][] = array(
										...
										'value'       => $type->name,
										'description' => '',
										'is_default_option' => ( ( ( true === bgci_bp_habits_has_group_type( $group_id, $type->name ) ) ? $type->name : '' ) === $type->name ),
									);
								}
							} else {
								$group_type_field['options'][] = array(
		...
		if ( function_exists( 'bp_enable_group_hierarchies' ) && bp_enable_group_hierarchies() ) {
			...
			$group_hierarchies = array(
				'label'       => esc_html__( 'Group Parent', 'bp-group-check-in' ),
				'name'        => 'bp-groups-parent',
				'description' => esc_html__( 'Which group should be the parent of this group? (optional)', 'bp-group-check-in' ),
				'field'       => 'select',
			...
			if ( $possible_parent_groups ) {
				$group_hierarchies['options'][] = array(
					'label'             => __( 'Select Parent', 'bp-group-check-in' ),
					'value'             => 0,
					'description'       => '',
	...
	protected function update_settings_fields( $request ) {
		 ...
		 */
		$current_types = bgci_bp_habits_get_group_type( $group_id, false );
		$current_types = array_intersect( bgci_bp_habits_get_group_types( array( 'show_in_create_screen' => false ) ), (array) $current_types );
		if ( isset( $post_fields['group-types'] ) ) {
			$current_types = array_merge( $current_types, (array) $post_fields['group-types'] );

			// Set group types.
			bgci_bp_habits_set_group_type( $group_id, $current_types );
		...
		if ( ! groups_edit_group_settings( $group_id, $enable_forum, $status, $invite_status, $activity_feed_status, $parent_id, $media_status, $document_status, $album_status, $message_status ) ) {
			$error = __( 'There was an error updating group settings. Please try again.', 'bp-group-check-in' );
		} else {
			$notice = __( 'Group settings were successfully updated.', 'bp-group-check-in' );
		}
	...
	protected function get_forum_fields( $group_id ) {
		...
		if ( ! bp_is_active( 'forums' ) || ! function_exists( 'bbp_is_group_forums_active' ) || ! bbp_is_group_forums_active() ) {
			return new WP_Error(
				'bp_rest_invalid_group_setting_nav',
				__( 'Sorry, you are not allowed to see the forum group settings options.', 'bp-group-check-in' ),
				array(
					'status' => 400,
				)
			);
		...
		$fields[] = array(
			'label'       => esc_html__( 'Group Forum Settings', 'bp-group-check-in' ),
			'name'        => '',
			'description' => esc_html__( 'Create a discussion forum to allow members of this group to communicate in a structured, bulletin-board style fashion.', 'bp-group-check-in' ),
			'field'       => 'heading',
			'value'       => '',
			'options'     => array(),
		);
		...
		$fields[] = array(
			'label'       => esc_html__( 'Yes. I want this group to have a discussion forum.', 'bp-group-check-in' ),
			'name'        => 'bbp-edit-group-forum',
			'description' => esc_html__( 'Saying no will not delete existing forum content.', 'bp-group-check-in' ),
			'field'       => 'checkbox',
			'value'       => $checked,
			'options'     => array(),
		);
		...
		if ( bbp_is_user_keymaster() ) {
			$forum_field = array(
				'label'       => esc_html__( 'Group Forum:', 'bp-group-check-in' ),
				'name'        => 'bbp_group_forum_id',
				'description' => esc_html__( 'Only site administrators can reconfigure which forum belongs to this group.', 'bp-group-check-in' ),
				'field'       => 'select',
				'value'       => $forum_id,
				'options'     => array(),
			);
			...
			if ( ! empty( $forums ) ) {
				$forum_field['options'][] = array(
					'label'             => esc_html__( '(No Forum)', 'bp-group-check-in' ),
					'value'             => '',
					'description'       => '',
				...
				foreach ( $forums as $forum ) {
					...
					if ( '' === $title ) {
						...
						$title = sprintf( __( '#%d (no title)', 'bp-group-check-in' ), $forum->ID );
					}
					$forum_field['options'][] = array(
						'label'             => $title,
						'value'             => $forum->ID,
	...
	protected function update_forum_fields( $request ) {
		...
		if ( ! bp_is_active( 'forums' ) || ! function_exists( 'bbp_is_group_forums_active' ) || ! bbp_is_group_forums_active() || ! class_exists( 'BBP_Forums_Group_Extension' ) ) {
			return new WP_Error(
				'bp_rest_invalid_group_setting_nav',
				__( 'Sorry, you are not allowed to update the forum group settings options.', 'bp-group-check-in' ),
				array(
					'status' => 400,
				)
			);
	...
	protected function get_courses_fields( $group_id ) {
		...
		if ( ! function_exists( 'bp_ld_sync' ) || '1' !== bp_ld_sync( 'settings' )->get( 'buddypress.enabled', true ) ) {
			return new WP_Error(
				'bp_rest_invalid_group_setting_nav',
				__( 'Sorry, you are not allowed to see the courses group settings options.', 'bp-group-check-in' ),
				array(
					'status' => 400,
				)
			);
		...
		$fields[] = array(
			'label'       => esc_html__( 'Group Courses Settings', 'bp-group-check-in' ),
			'name'        => '',
			'description' => esc_html__( 'Create and associate to a LearnDash group, allowing courses and reports to be managed within the group.', 'bp-group-check-in' ),
			'field'       => 'heading',
			'value'       => '',
			'options'     => array(),
		);
		...
		$fields[] = array(
			'label'       => esc_html__( 'Yes. I want this group to sync with a LearnDash group.', 'bp-group-check-in' ),
	...
	protected function update_courses_fields( $request ) {
		...
		if ( ! function_exists( 'bp_ld_sync' ) || '1' !== bp_ld_sync( 'settings' )->get( 'buddypress.enabled', true ) ) {
			return new WP_Error(
				'bp_rest_invalid_group_setting_nav',
				__( 'Sorry, you are not allowed to update the courses group settings options.', 'bp-group-check-in' ),
				array(
					'status' => 400,
				)
			);
