 ...
 * @since BuddyPress 1.1.0
 */
class bgci_bp_habits_Invite_Template {
	...
	/**
	 * bgci_bp_habits_Invite_Template constructor.
	 *
	...
	public function invites() {
		...
		if ( $tick < $this->invite_count ) {
			 ...
			 *
			 * @param bgci_bp_habits_Invite_Template $this Instance of the current Invites template.
			 */
	...
	public function the_invite() {
		...
		$this->invite->user->avatar_mini  = bp_core_fetch_avatar(
			array(
				'item_id' => $user_id,
				'type'    => 'thumb',
				'alt'     => sprintf( __( 'Profile photo of %s', 'bp-group-check-in' ), $this->invite->user->fullname ),
				'width'   => 30,
				'height'  => 30,
			)
		);
		...
		$this->invite->user->last_active  = bp_core_get_last_activity( $this->invite->user->last_activity, __( 'active %s', 'bp-group-check-in' ) );

		if ( bp_is_active( 'groups' ) ) {
			$total_groups                     = bgci_bp_habits_Member::total_group_count( $user_id );
			$this->invite->user->total_groups = sprintf( _n( '%d group', '%d groups', $total_groups, 'bp-group-check-in' ), $total_groups );
		}
