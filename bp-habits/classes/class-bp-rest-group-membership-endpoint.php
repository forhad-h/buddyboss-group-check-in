...
class BP_REST_Group_Membership_Endpoint extends WP_REST_Controller {
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					...
					'user_id'  => array(
						'description' => __( 'A unique numeric ID for the Group Member.', 'bp-group-check-in' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function get_items( $request ) {
		 ...
		 * @param bgci_bp_habits_Group $group The group object.
	...
	public function create_item( $request ) {
		...
		if ( ! $request['context'] || 'view' === $request['context'] ) {
			if ( ! groups_join_group( $group->id, $user->ID ) ) {
				return new WP_Error(
					...
					__( 'Could not join the group.', 'bp-group-check-in' ),
					array(
						'status' => 500,
					)
				);
			...
			$group_member = new bgci_bp_habits_Member( $user->ID, $group->id );
		} else {
			$role         = $request['role'];
			$group_id     = $group->id;
			$group_member = new bgci_bp_habits_Member( $user->ID, $group_id );
			...
			if ( ! $saved ) {
				return new WP_Error(
					'bp_rest_group_member_failed_to_join',
					__( 'Could not add member to the group.', 'bp-group-check-in' ),
					array(
						'status' => 500,
					)
				);
		 ...
		 * @param bgci_bp_habits_Member $group_member The group member object.
		 * @param bgci_bp_habits_Group $group The group object.
		 * @param WP_REST_Response $response The response data.
		 * @param WP_REST_Request $request The request sent to the API.
		 *
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				...
				__( 'Sorry, you need to be logged in to join a group.', 'bp-group-check-in' ),
				array(
		...
		if ( true === $retval && ! $user instanceof WP_User ) {
			$retval = new WP_Error(
				...
				__( 'Invalid group member ID.', 'bp-group-check-in' ),
				array(
					'status' => 404,
				)
			);
		...
		$group = $this->groups_endpoint->get_group_object( $request['group_id'] );
		if ( true === $retval && ! $group instanceof bgci_bp_habits_Group ) {
			$retval = new WP_Error(
		...
		if ( 'promote' === $action ) {
			if ( ! $group_member->promote( $role ) ) {
				return new WP_Error(
					...
					__( 'Could not promote member.', 'bp-group-check-in' ),
					array(
						'status' => 500,
					)
				);
			...
			if ( ! $group_member->promote( $role ) ) {
				return new WP_Error(
					...
					__( 'Could not demote member.', 'bp-group-check-in' ),
					array(
						'status' => 500,
					)
				);
			...
			if ( ! $group_member->$action() ) {
				$messages = array(
					'demote' => __( 'Could not demote member from the group.', 'bp-group-check-in' ),
					'ban'    => __( 'Could not ban member from the group.', 'bp-group-check-in' ),
					'unban'  => __( 'Could not unban member from the group.', 'bp-group-check-in' ),
				);
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to make an update.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( true === $retval && bp_current_user_can( 'bp_moderate' ) ) {
			...
			if ( true === $retval && in_array( $request['action'], array( 'ban', 'unban', 'promote', 'demote' ), true ) ) {
				if ( ! groups_is_user_admin( $loggedin_user_id, $group->id ) && ! groups_is_user_mod( $loggedin_user_id, $group->id ) ) {
					$messages = array(
						'ban'     => __( 'Sorry, you are not allowed to ban this group member.', 'bp-group-check-in' ),
						'unban'   => __( 'Sorry, you are not allowed to unban this group member.', 'bp-group-check-in' ),
						'promote' => __( 'Sorry, you are not allowed to promote this group member.', 'bp-group-check-in' ),
						'demote'  => __( 'Sorry, you are not allowed to demote this group member.', 'bp-group-check-in' ),
					);
	...
	public function delete_item( $request ) {
		...
		$member   = new bgci_bp_habits_Member( $request['user_id'], $request['group_id'] );
		...
		if ( ! $member->remove() ) {
			return new WP_Error(
				...
				__( 'Could not remove member from this group.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
		 ...
		 *
		 * @param WP_User $user The updated member.
		 * @param bgci_bp_habits_Member $member The group member object.
		 * @param bgci_bp_habits_Group $group The group object.
		 * @param WP_REST_Response $response The response data.
		 * @param WP_REST_Request $request The request sent to the API.
		 *
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to view a group membership.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	 ...
	 *
	 * @param bgci_bp_habits_Member $group_member Group member object.
	 * @param WP_REST_Request  $request Full details about the request.
	 *
	...
	public function prepare_item_for_response( $group_member, $request ) {
		...
		if ( function_exists( 'bp_get_user_group_role_title' ) && ! empty( $request['group_id'] ) ) {
			...
			// BuddyPress support.
		} elseif ( function_exists( 'bgci_bp_habits_get_group_roles' ) && ! empty( $request['group_id'] ) ) {
	...
	public function get_endpoint_args_for_method( $method = WP_REST_Server::CREATABLE ) {
		...
		if ( WP_REST_Server::CREATABLE === $method || WP_REST_Server::EDITABLE === $method ) {
			$group_roles = array_diff( array_keys( bgci_bp_habits_get_group_roles() ), array( 'banned' ) );

			$args['role'] = array(
				'description'       => __( 'Group role to assign the user to.', 'bp-group-check-in' ),
				'default'           => 'member',
				'type'              => 'string',
				'enum'              => $group_roles,
			...
			if ( WP_REST_Server::CREATABLE === $method ) {
				...
				$args['user_id'] = array_merge(
					...
					array(
						'description' => __( 'A unique numeric ID for the Member to add to the Group.', 'bp-group-check-in' ),
			...
			if ( WP_REST_Server::EDITABLE === $method ) {
				$key            = 'update_item';
				$args['action'] = array(
					'description'       => __( 'Action used to update a group member.', 'bp-group-check-in' ),
					'default'           => 'promote',
					'type'              => 'string',
	...
	public function get_item_schema() {
		...
		$schema['properties']['is_mod'] = array(
			...
			'description' => __( 'Whether the member is a group moderator.', 'bp-group-check-in' ),
			'type'        => 'boolean',
		);

		$schema['properties']['is_banned'] = array(
			'context'     => array( 'view', 'edit' ),
			'description' => __( 'Whether the member has been banned from the group.', 'bp-group-check-in' ),
			'type'        => 'boolean',
		);

		$schema['properties']['is_admin'] = array(
			'context'     => array( 'view', 'edit' ),
			'description' => __( 'Whether the member is a group administrator.', 'bp-group-check-in' ),
			'type'        => 'boolean',
		);

		$schema['properties']['is_confirmed'] = array(
			'context'     => array( 'view', 'edit' ),
			'description' => __( 'Whether the membership of this user has been confirmed.', 'bp-group-check-in' ),
			'type'        => 'boolean',
		);

		$schema['properties']['date_modified'] = array(
			'context'     => array( 'view', 'edit' ),
			'description' => __( "The date of the last time the membership of this user was modified, in the site's timezone.", 'bp-group-check-in' ),
			'type'        => 'string',
			'format'      => 'date-time',
		);
	...
	public function get_collection_params() {
		...
		$params['status'] = array(
			'description'       => __( 'Sort the order of results by the status of the group members.', 'bp-group-check-in' ),
			'default'           => 'last_joined',
			'type'              => 'string',
			'enum'              => $statuses,
			'sanitize_callback' => 'sanitize_key',
		...
		$params['roles'] = array(
			'description'       => __( 'Ensure result set includes specific group roles.', 'bp-group-check-in' ),
			'default'           => array(),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
				'enum' => array_keys( bgci_bp_habits_get_group_roles() ),
			),
		...
		$params['exclude'] = array(
			'description'       => __( 'Ensure result set excludes specific member IDs.', 'bp-group-check-in' ),
			'default'           => array(),
			'type'              => 'array',
		...
		$params['exclude_admins'] = array(
			'description'       => __( 'Whether results should exclude group admins and mods.', 'bp-group-check-in' ),
			'default'           => true,
			'type'              => 'boolean',
		...
		$params['exclude_banned'] = array(
			'description'       => __( 'Whether results should exclude banned group members.', 'bp-group-check-in' ),
			'default'           => true,
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'validate_callback' => 'rest_validate_request_arg',
	...
	public function bp_rest_get_group_potential_invites( $group, $request ) {
		...
		if ( 'invite' === $request['scope'] ) {
			...
			$group_type = bgci_bp_habits_get_group_type( $args['group_id'] );
					...
					if ( $parent_group_id > 0 ) {
						...
						if ( empty( $args['include'] ) ) {
							return new WP_Error(
								...
								__( 'No members found in parent group.', 'bp-group-check-in' ),
								array(
									'status' => 202,
								)
							);
		...
		if ( ! bgci_bp_habits_user_can_send_invites( $args['group_id'] ) ) {
			return new WP_Error(
				'bp_rest_group_invites_cannot_get_items',
				__( 'You are not authorized to send invites to other users.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( 'invited' === $request['scope'] ) {
			...
			if ( ! bp_group_has_invites(
				...
				if ( isset( $args ) && isset( $args['search_terms'] ) && '' !== $args['search_terms'] ) {
					...
					return new WP_Error(
						'bp_rest_group_invites_cannot_get_items',
						__( 'All members already received invitations.', 'bp-group-check-in' ),
						array(
							'status' => 202,
						)
					);
					...
					return new WP_Error(
						'bp_rest_group_invites_cannot_get_items',
						__( 'No pending group invitations found.', 'bp-group-check-in' ),
						array(
							'status' => 202,
						)
					);
