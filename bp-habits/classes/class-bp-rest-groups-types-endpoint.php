...
class BP_REST_Groups_Types_Endpoint extends WP_REST_Controller {
	...
	public function get_items( $request ) {
		$registered_types = bgci_bp_habits_get_group_types( array(), 'objects' );
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( function_exists( 'bp_disable_group_type_creation' ) && false === bp_disable_group_type_creation() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, Group Type is disabled from setting.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'title'      => 'bgci_bp_habits_types',
			'type'       => 'object',
			'properties' => array(
				'labels'                => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Labels to use in various parts of the interface.', 'bp-group-check-in' ),
					'type'        => 'object',
					'arg_options' => array(
					...
					'properties'  => array(
						'name'          => array(
							'description' => __( 'Default name. Should typically be plural.', 'bp-group-check-in' ),
							'type'        => 'string',
							'context'     => array( 'embed', 'view', 'edit' ),
							'readonly'    => true,
						),
						'singular_name' => array(
							'description' => __( 'Singular name.', 'bp-group-check-in' ),
							'type'        => 'string',
							'context'     => array( 'embed', 'view', 'edit' ),
							'readonly'    => true,
						),
				...
				'name'                  => array(
					'description' => __( 'Slug of the group type.', 'bp-group-check-in' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'description'           => array(
					'description' => __( 'Description of the group type.', 'bp-group-check-in' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'directory_slug'        => array(
					'description' => __( 'Directory slug of the group type.', 'bp-group-check-in' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'has_directory'         => array(
					'description' => __( 'Whether the group type should have its own type-specific directory.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'show_in_create_screen' => array(
					'description' => __( 'Whether this group type is allowed to be selected on the group creation page.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'show_in_list'          => array(
					'description' => __( 'Whether this group type should be shown in lists.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'create_screen_checked' => array(
					'description' => __( 'Whether we should have our group type checkbox checked by default on group create.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
		...
		if ( ! empty( $post_types ) && in_array( 'bp-group-type', $post_types, true ) ) {
			$schema['properties']['enable_filter'] = array(
				'description' => __( 'Display this group type in "Types" filter in Groups Directory.', 'bp-group-check-in' ),
				'type'        => 'boolean',
				'context'     => array( 'embed', 'view', 'edit' ),
				'readonly'    => true,
			);
			...
			$schema['properties']['enable_remove'] = array(
				'description' => __( 'Hide all groups of this type from Groups Directory.', 'bp-group-check-in' ),
				'type'        => 'boolean',
				'context'     => array( 'embed', 'view', 'edit' ),
				'readonly'    => true,
			);
			...
			$schema['properties']['restrict-invites-user-same-group-type'] = array(
				'description' => __( 'If a member is already in a group of this type, they cannot be sent an invite to join another group of this type.', 'bp-group-check-in' ),
				'type'        => 'boolean',
				'context'     => array( 'embed', 'view', 'edit' ),
				'readonly'    => true,
			);
			...
			$schema['properties']['member_type_group_invites'] = array(
				'description' => __( 'Only members of the selected profile types may be sent requests to join this group.', 'bp-group-check-in' ),
				'type'        => 'object',
				'context'     => array( 'embed', 'view', 'edit' ),
				'readonly'    => true,
			);
			...
			$schema['properties']['member_type_join'] = array(
				'description' => __( 'Members of the selected profile types can always join groups of this type, even if the group is private.', 'bp-group-check-in' ),
				'type'        => 'object',
				'context'     => array( 'embed', 'view', 'edit' ),
				'readonly'    => true,
			);
			...
			$schema['properties']['group_type_role_labels'] = array(
				'description' => __( 'Rename the group member roles for groups of this type.', 'bp-group-check-in' ),
				'type'        => 'object',
				'context'     => array( 'embed', 'view', 'edit' ),
				'readonly'    => true,
			);
