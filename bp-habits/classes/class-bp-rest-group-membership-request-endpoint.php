...
class BP_REST_Group_Membership_Request_Endpoint extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					'request_id' => array(
						'description' => __( 'A unique numeric ID for the group membership request.', 'bp-group-check-in' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! $user_id ) {
			$retval = new WP_Error(
				...
				__( 'Sorry, you need to be logged in to view membership requests.', 'bp-group-check-in' ),
				array(
		...
		if (
			...
			$retval = new WP_Error(
				...
				__( 'Sorry, you are not allowed to view membership requests.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! $user_id ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to get a membership.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( true === $retval && ! $group_request ) {
			$retval = new WP_Error(
				...
				__( 'Invalid group membership request ID.', 'bp-group-check-in' ),
				array(
					'status' => 404,
				)
			);
		...
		if (
			...
			$retval = new WP_Error(
				...
				__( 'Sorry, you are not allowed to view a membership request.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function create_item( $request ) {
		...
		if ( groups_check_for_membership_request( $user->ID, $group->id ) ) {
			return new WP_Error(
				...
				__( 'There is already a request to this member.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
		...
		if ( ! $request_id ) {
			return new WP_Error(
				...
				__( 'Could not send membership request to this group.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to create a membership request.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if (
			...
			$retval = new WP_Error(
				'bp_rest_group_membership_requests_cannot_create_item',
				__( 'User may not extend requests on behalf of another user.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function update_item( $request ) {
		...
		if ( ! $success ) {
			return new WP_Error(
				...
				__( 'There was an error accepting the membership request.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
		...
		$g_member = new bgci_bp_habits_Member( $group_request->user_id, $group_request->item_id );
		 ...
		 *
		 * @param bgci_bp_habits_Member $g_member The groups member object.
		 * @param bgci_bp_habits_Group $group The group object.
		 * @param WP_REST_Response $response The response data.
		 * @param WP_REST_Request $request The request sent to the API.
		 *
	...
	public function update_item_permissions_check( $request ) {
		...
		if (
			...
			$retval = new WP_Error(
				'bp_rest_group_member_request_cannot_update_item',
				__( 'User is not allowed to approve membership requests to this group.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function delete_item( $request ) {
		...
		if ( ! $success ) {
			return new WP_Error(
				...
				__( 'There was an error rejecting the membership request.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! $user_id ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to delete a request.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if (
			...
			$retval = new WP_Error(
				'bp_rest_group_membership_requests_cannot_delete_item',
				__( 'User is not allowed to delete this membership request.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function get_item_schema() {
		...
		$schema['properties']['user_id']['description']  = __( 'The ID of the user who requested a Group membership.', 'bp-group-check-in' );
		$schema['properties']['group_id']['description'] = __( 'The ID of the group the user requested a membership for.', 'bp-group-check-in' );
	...
	public function get_collection_params() {
		...
		// Adapt some item schema property descriptions to this endpoint.
		$params['user_id']['description']  = __( 'Return only Membership requests made by a specific user.', 'bp-group-check-in' );
		$params['group_id']['description'] = __( 'The ID of the group the user requested a membership for.', 'bp-group-check-in' );
