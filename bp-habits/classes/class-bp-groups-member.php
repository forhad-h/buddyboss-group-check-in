...
class bgci_bp_habits_Member {
	...
	public function save() {
		 ...
		 *
		 * @param bgci_bp_habits_Member $this Current instance of the group membership item being saved. Passed by reference.
		 */
		 ...
		 * @since BuddyPress 1.0.0
		 *
		 * @param bgci_bp_habits_Member $this Current instance of the group membership item has been saved. Passed by reference.
		 */
	...
	public function promote( $status = 'mod' ) {
		if ( 'mod' == $status ) {
			$this->is_admin   = 0;
			$this->is_mod     = 1;
			$this->user_title = __( 'Group Moderator', 'bp-group-check-in' );
		}

		if ( 'admin' == $status ) {
			$this->is_admin   = 1;
			$this->is_mod     = 0;
			$this->user_title = __( 'Group Organizer', 'bp-group-check-in' );
		}
	...
	public function remove() {
		 ...
		 *
		 * @param bgci_bp_habits_Member $this Current group membership object.
		 */
	...
	public static function refresh_total_member_count_for_group( $group_id ) {
		return groups_update_groupmeta( $group_id, 'total_member_count', (int) bgci_bp_habits_Group::get_total_member_count( $group_id ) );
	}
	...
	public static function delete( $user_id, $group_id ) {
		 ...
		 * @param int $group_id ID of the group.
		 */
		do_action( 'bgci_bp_habits_member_before_delete', $user_id, $group_id );
		 ...
		 * @param int $group_id ID of the group.
		 */
		do_action( 'bgci_bp_habits_member_after_delete', $user_id, $group_id );
	...
	public static function total_group_count( $user_id = 0 ) {
		 ...
		 */
		$sql['where'] = apply_filters( 'bgci_bp_habits_get_where_count_conditions', $sql['where'], array() );
		 ...
		 */
		$sql['from'] = apply_filters( 'bgci_bp_habits_get_join_count_sql', $sql['from'], array() );
	...
	public static function delete_invite( $user_id, $group_id, $inviter_id = false ) {
		 ...
		 */
		do_action( 'bgci_bp_habits_member_before_delete_invite', $user_id, $group_id, $inviter_id );
	...
	public static function delete_all_for_user( $user_id ) {
		$group_ids = bgci_bp_habits_Member::get_group_ids( $user_id );
			...
			if ( groups_is_user_admin( $user_id, $group_id ) ) {
				...
				if ( count( groups_get_group_admins( $group_id ) ) < 2 ) {
					...
					if ( ! empty( $admin ) ) {
						...
						$member = new bgci_bp_habits_Member( $admin[0], $group_id );
						$member->promote( 'admin' );
					}
				}
			}

			bgci_bp_habits_Member::delete( $user_id, $group_id );
		}
