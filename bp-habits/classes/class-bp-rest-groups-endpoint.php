...
class BP_REST_Groups_Endpoint extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					'id' => array(
						'description' => __( 'A unique numeric ID for the Group.', 'bp-group-check-in' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function get_items( $request ) {
		...
		if (
			...
			&& function_exists( 'bgci_bp_habits_get_excluded_group_ids_by_type' )
			&& ! empty( bgci_bp_habits_get_excluded_group_ids_by_type() )
		) {
			$args['exclude'] = array_unique( bgci_bp_habits_get_excluded_group_ids_by_type() );
		}
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( function_exists( 'bp_enable_private_network' ) && true !== bp_enable_private_network() && ! is_user_logged_in() ) {
			$retval = new WP_Error(
				...
				__( 'Sorry, Restrict access to only logged-in members.', 'bp-group-check-in' ),
				array(
	...
	public function get_item( $request ) {
		 ...
		 *
		 * @param bgci_bp_habits_Group  $group    Fetched group.
		 * @param WP_REST_Response $response The response data.
		 * @param WP_REST_Request  $request  The request sent to the API.
		 *
		 * @since 0.1.0
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( true === $retval && empty( $group->id ) ) {
			$retval = new WP_Error(
				...
				__( 'Invalid group ID.', 'bp-group-check-in' ),
				array(
					'status' => 404,
				)
			);
		...
		if ( true === $retval && ! $this->can_see( $group ) ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you cannot view the group.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function create_item( $request ) {
		...
		if ( empty( $request['name'] ) ) {
			return new WP_Error(
				...
				__( 'Please, enter the name of group.', 'bp-group-check-in' ),
				array(
					'status' => 400,
				)
			);
		...
		if ( ! is_numeric( $group_id ) ) {
			return new WP_Error(
				...
				__( 'Cannot create new group.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
		...
		if ( ! empty( $request['types'] ) ) {
			bgci_bp_habits_set_group_type( $group_id, $request['types'] );
		}
		 ...
		 *
		 * @param bgci_bp_habits_Group  $group    The created group.
		 * @param WP_REST_Response $response The response data.
		 * @param WP_REST_Request  $request  The request sent to the API.
		 *
		 * @since 0.1.0
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! ( is_user_logged_in() && bp_user_can_create_groups() ) ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you are not allowed to create groups.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function update_item( $request ) {
		...
		if ( ! is_numeric( $group_id ) ) {
			return new WP_Error(
				...
				__( 'Cannot update existing group.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
		 ...
		 *
		 * @param bgci_bp_habits_Group  $group    The updated group.
		 * @param WP_REST_Response $response The response data.
		 * @param WP_REST_Request  $request  The request sent to the API.
		 *
		 * @since 0.1.0
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to update this group.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( true === $retval && ! $this->can_user_delete_or_update( $group ) ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you are not allowed to update this group.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function delete_item( $request ) {
		...
		if ( ! groups_delete_group( $group->id ) ) {
			return new WP_Error(
				...
				__( 'Could not delete the group.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you need to be logged in to delete this group.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
		...
		if ( true === $retval && ! $this->can_user_delete_or_update( $group ) ) {
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you are not allowed to delete this group.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	 ...
	 *
	 * @param bgci_bp_habits_Group $item    Group object.
	 * @param WP_REST_Request $request Full details about the request.
	 *
	...
	public function prepare_item_for_response( $item, $request ) {
		$data = array(
			...
			'types'              => bgci_bp_habits_get_group_type( $item->id, false ),
		...
		if ( function_exists( 'bp_get_user_group_role_title' ) && bp_loggedin_user_id() ) {
			...
			// BuddyPress support.
		} elseif ( function_exists( 'bgci_bp_habits_get_group_roles' ) && bp_loggedin_user_id() ) {
			$group_role = bgci_bp_habits_get_group_roles();
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $prepared_group->group_id ) && isset( $request['types'] ) ) {
			bgci_bp_habits_set_group_type( $prepared_group->group_id, $request['types'], false );
		}
		...
		if ( isset( $prepared_group->group_id ) && isset( $request['remove_types'] ) ) {
			array_map(
				function( $type ) use ( $prepared_group ) {
					bgci_bp_habits_remove_group_type( $prepared_group->group_id, $type );
				},
		...
		if ( isset( $prepared_group->group_id ) && isset( $request['append_types'] ) ) {
			bgci_bp_habits_set_group_type( $prepared_group->group_id, $request['append_types'], true );
		}
	 ...
	 *
	 * @param bgci_bp_habits_Group $group Group item.
	 *
	 * @return bool
	 * @since 0.1.0
	 */
	 ...
	 * @param WP_REST_Request $request Full details about the request.
	 *
	 * @return bool|bgci_bp_habits_Group
	 * @since 0.1.0
	 */
	...
	public function get_endpoint_args_for_item_schema( $method = WP_REST_Server::CREATABLE ) {
		...
		if ( WP_REST_Server::CREATABLE === $method || WP_REST_Server::EDITABLE === $method ) {
			...
			// Add group types.
			$args['types'] = array(
				'description'       => __( 'Set type(s) for a group.', 'bp-group-check-in' ),
				'type'              => 'array',
				'enum'              => bgci_bp_habits_get_group_types(),
			...
			if ( WP_REST_Server::EDITABLE === $method ) {
				...
				$args['append_types'] = array(
					'description'       => __( 'Append type(s) for a group.', 'bp-group-check-in' ),
					'type'              => 'array',
					'enum'              => bgci_bp_habits_get_group_types(),
					'sanitize_callback' => 'bp_rest_sanitize_group_types',
					'validate_callback' => 'bp_rest_validate_group_types',
				...
				$args['remove_types'] = array(
					'description'       => __( 'Remove type(s) for a group.', 'bp-group-check-in' ),
					'type'              => 'array',
					'enum'              => bgci_bp_habits_get_group_types(),
					'sanitize_callback' => 'bp_rest_sanitize_group_types',
					'validate_callback' => 'bp_rest_validate_group_types',
			...
			if ( bp_is_active( 'forums' ) ) {
				$args['delete_group_forum'] = array(
					'description'       => __( 'Delete the Group forum if exist.', 'bp-group-check-in' ),
					'type'              => 'boolean',
					'default'           => false,
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'creator_id'         => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The ID of the user who created the Group.', 'bp-group-check-in' ),
					'type'        => 'integer',
					'default'     => bp_loggedin_user_id(),
				),
				'name'               => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The name of the Group.', 'bp-group-check-in' ),
					'type'        => 'string',
					'required'    => true,
					'arg_options' => array(
				...
				'slug'               => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The URL-friendly slug for the Group.', 'bp-group-check-in' ),
					'type'        => 'string',
					'arg_options' => array(
				...
				'link'               => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The permalink to the Group on the site.', 'bp-group-check-in' ),
					'type'        => 'string',
					'format'      => 'uri',
					'readonly'    => true,
				),
				'description'        => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The description of the Group.', 'bp-group-check-in' ),
					'type'        => 'object',
					'required'    => true,
					'arg_options' => array(
						'sanitize_callback' => null,
					...
					'properties'  => array(
						'raw'      => array(
							'description' => __( 'Content for the description of the Group, as it exists in the database.', 'bp-group-check-in' ),
							'type'        => 'string',
							'context'     => array( 'embed', 'view', 'edit' ),
						),
						'rendered' => array(
							'description' => __( 'HTML content for the description of the Group, transformed for display.', 'bp-group-check-in' ),
							'type'        => 'string',
							'context'     => array( 'embed', 'view', 'edit' ),
							'readonly'    => true,
						),
				...
				'status'             => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The status of the Group.', 'bp-group-check-in' ),
					'type'        => 'string',
				...
				'enable_forum'       => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Whether the Group has a forum enabled or not.', 'bp-group-check-in' ),
					'type'        => 'boolean',
				),
				'parent_id'          => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'ID of the parent Group.', 'bp-group-check-in' ),
					'type'        => 'integer',
				),
				'date_created'       => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( "The date the Group was created, in the site's timezone.", 'bp-group-check-in' ),
					'readonly'    => true,
					'type'        => 'string',
					'format'      => 'date-time',
				),
				'types'              => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The type(s) of the Group.', 'bp-group-check-in' ),
					'readonly'    => true,
					'enum'        => bgci_bp_habits_get_group_types(),
					'type'        => 'array',
					'items'       => array(
				...
				'subgroups_id'       => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Sub Groups id if having a sub groups.', 'bp-group-check-in' ),
					'readonly'    => true,
					'type'        => 'array',
					'items'       => array(
						'type' => 'integer',
				...
				'admins'             => array(
					'context'     => array( 'edit' ),
					'description' => __( 'Group administrators.', 'bp-group-check-in' ),
					'readonly'    => true,
					'type'        => 'array',
					'items'       => array(
						'type' => 'object',
				...
				'mods'               => array(
					'context'     => array( 'edit' ),
					'description' => __( 'Group moderators.', 'bp-group-check-in' ),
					'readonly'    => true,
					'type'        => 'array',
					'items'       => array(
						'type' => 'object',
				...
				'total_member_count' => array(
					'context'     => array( 'edit' ),
					'description' => __( 'Count of all Group members.', 'bp-group-check-in' ),
					'readonly'    => true,
					'type'        => 'integer',
				),
				'last_activity'      => array(
					'context'     => array( 'edit' ),
					'description' => __( "The date the Group was last active, in the site's timezone.", 'bp-group-check-in' ),
					'type'        => 'string',
					'readonly'    => true,
					'format'      => 'date-time',
				),
				...
				'is_member'          => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The current user is member of a group or not.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'readonly'    => true,
				),
				'invite_id'          => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Return\'s invite ID if current user is invited for a group or not.', 'bp-group-check-in' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
				'request_id'         => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Return\'s request ID if invitation is pending for a group or not.', 'bp-group-check-in' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
				'is_admin'           => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The current user is admin of a group or not.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'readonly'    => true,
				),
				'is_mod'             => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'The current user is moderator of a group or not.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'readonly'    => true,
				),
				'members_count'      => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Members count of the group.', 'bp-group-check-in' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
				'role'               => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Current member\'s role label in the group.', 'bp-group-check-in' ),
					'type'        => 'string',
					'readonly'    => true,
				),
				'plural_role'        => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Current member\'s role label in the plural form in the group', 'bp-group-check-in' ),
					'type'        => 'string',
					'readonly'    => true,
				),
				'can_join'           => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'Check current user can join or request access.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'readonly'    => true,
				),
				'can_post'           => array(
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'Check current user can post activity or not.', 'bp-group-check-in' ),
					'type'        => 'boolean',
					'readonly'    => true,
				),
				'forum'              => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Forum id of the group.', 'bp-group-check-in' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
			),
		...
		if ( ! bp_disable_group_avatar_uploads() ) {
			...
			$avatar_properties['full'] = array(
				...
				'description' => sprintf( __( 'Avatar URL with full image size (%1$d x %2$d pixels).', 'bp-group-check-in' ), number_format_i18n( bp_core_avatar_full_width() ), number_format_i18n( bp_core_avatar_full_height() ) ),
				'type'        => 'string',
				'format'      => 'uri',
				'context'     => array( 'embed', 'view', 'edit' ),
			);
			...
			$avatar_properties['thumb'] = array(
				...
				'description' => sprintf( __( 'Avatar URL with thumb image size (%1$d x %2$d pixels).', 'bp-group-check-in' ), number_format_i18n( bp_core_avatar_thumb_width() ), number_format_i18n( bp_core_avatar_thumb_height() ) ),
				'type'        => 'string',
				'format'      => 'uri',
				'context'     => array( 'embed', 'view', 'edit' ),
			);
			...
			$schema['properties']['avatar_urls'] = array(
				'description' => __( 'Avatar URLs for the group.', 'bp-group-check-in' ),
				'type'        => 'object',
				'context'     => array( 'embed', 'view', 'edit' ),
				'readonly'    => true,
		...
		if ( ! bp_disable_group_cover_image_uploads() ) {
			$schema['properties']['cover_url'] = array(
				'description' => __( 'Cover Image URLs for the group.', 'bp-group-check-in' ),
				'type'        => 'string',
				'format'      => 'uri',
				'context'     => array( 'embed', 'view', 'edit' ),
				'readonly'    => true,
	...
	public function get_collection_params() {
		...
		$params['type'] = array(
			'description'       => __( 'Shorthand for certain orderby/order combinations.', 'bp-group-check-in' ),
			'default'           => 'active',
			'type'              => 'string',
		...
		$params['order'] = array(
			'description'       => __( 'Order sort attribute ascending or descending.', 'bp-group-check-in' ),
			'default'           => 'desc',
			'type'              => 'string',
			'enum'              => array( 'asc', 'desc' ),
			'sanitize_callback' => 'sanitize_key',
		...
		$params['orderby'] = array(
			'description'       => __( 'Order Groups by which attribute.', 'bp-group-check-in' ),
		...
		$params['status'] = array(
			'description'       => __( 'Group statuses to limit results to.', 'bp-group-check-in' ),
			'default'           => array(),
			'type'              => 'array',
			'items'             => array(
				'enum' => buddypress()->groups->valid_status,
		...
		$params['user_id'] = array(
			'description'       => __( 'Pass a user_id to limit to only Groups that this user is a member of.', 'bp-group-check-in' ),
			'default'           => 0,
			'type'              => 'integer',
		...
		$params['parent_id'] = array(
			'description'       => __( 'Get Groups that are children of the specified Group(s) IDs.', 'bp-group-check-in' ),
			'default'           => array(),
			'type'              => 'array',
		...
		$params['meta'] = array(
			'description'       => __( 'Get Groups based on their meta data information.', 'bp-group-check-in' ),
			'default'           => array(),
			'type'              => 'array',
		...
		$params['include'] = array(
			'description'       => __( 'Ensure result set includes Groups with specific IDs.', 'bp-group-check-in' ),
			'default'           => array(),
			'type'              => 'array',
			'items'             => array( 'type' => 'integer' ),
			'sanitize_callback' => 'wp_parse_id_list',
		...
		$params['exclude'] = array(
			'description'       => __( 'Ensure result set excludes Groups with specific IDs', 'bp-group-check-in' ),
			'default'           => array(),
			'type'              => 'array',
			'items'             => array( 'type' => 'integer' ),
			'sanitize_callback' => 'wp_parse_id_list',
		...
		$params['group_type'] = array(
			'description'       => __( 'Limit results set to a certain Group type.', 'bp-group-check-in' ),
			'default'           => '',
			'type'              => 'string',
			'enum'              => bgci_bp_habits_get_group_types(),
			'sanitize_callback' => 'bp_rest_sanitize_group_types',
		...
		$params['show_hidden'] = array(
			'description'       => __( 'Whether results should include hidden Groups.', 'bp-group-check-in' ),
			'default'           => false,
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'validate_callback' => 'rest_validate_request_arg',
		...
		$params['scope'] = array(
			'description'       => __( 'Limit result set to items with a specific scope.', 'bp-group-check-in' ),
			'type'              => 'string',
			'default'           => 'all',
		...
		if ( 'private' === bp_get_group_status( $item ) ) {
			if ( $item->is_invited ) {
				...
				if (
					...
					$group_type = bgci_bp_habits_get_group_type( $item->id );
