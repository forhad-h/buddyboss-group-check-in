...
class BP_REST_Attachments_Group_Avatar_Endpoint extends WP_REST_Controller {
	...
	public function get_item( $request ) {
		...
		if ( ! $avatar->full && ! $avatar->thumb ) {
			return new WP_Error(
				...
				__( 'Sorry, there was a problem fetching this group avatar.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function create_item( $request ) {
		...
		if ( empty( $files ) ) {
			return new WP_Error(
				...
				__( 'Sorry, you need an image file to upload.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( true === $retval && ( bp_disable_group_avatar_uploads() || ! buddypress()->avatar->show_avatars ) ) {
			$retval = new WP_Error(
				...
				__( 'Sorry, group avatar upload is disabled.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
		...
		if (
			...
			$retval = new WP_Error(
				'bp_rest_authorization_required',
				__( 'Sorry, you are not authorized to perform this action.', 'bp-group-check-in' ),
				array(
					'status' => rest_authorization_required_code(),
				)
			);
	...
	public function delete_item( $request ) {
		...
		if ( ! bp_get_group_has_avatar( $group_id ) ) {
			return new WP_Error(
				...
				__( 'Sorry, there are no uploaded avatars for this group on this site.', 'bp-group-check-in' ),
				array(
					'status' => 404,
				)
			);
		...
		if ( ! $deleted ) {
			return new WP_Error(
				...
				__( 'Sorry, there was a problem deleting this group avatar.', 'bp-group-check-in' ),
				array(
					'status' => 500,
				)
			);
		...
		$schema = array(
			...
			'properties' => array(
				'full'  => array(
					...
					'description' => __( 'Full size of the image file.', 'bp-group-check-in' ),
					'type'        => 'string',
					'readonly'    => true,
				),
				'thumb' => array(
					'context'     => array( 'embed', 'view', 'edit' ),
					'description' => __( 'Thumb size of the image file.', 'bp-group-check-in' ),
					'type'        => 'string',
					'readonly'    => true,
				),
			),
	...
	public function get_item_collection_params() {
		...
		$params['html'] = array(
			'description'       => __( 'Whether to return an <img> HTML element, vs a raw URL to a group avatar.', 'bp-group-check-in' ),
			'default'           => false,
			'type'              => 'boolean',
		...
		$params['alt'] = array(
			'description'       => __( 'The alt attribute for the <img> element.', 'bp-group-check-in' ),
			'default'           => '',
			'type'              => 'string',
