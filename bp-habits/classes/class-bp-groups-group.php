...
class bgci_bp_habits_Group {
	...
	public function populate() {
		 ...
		 */
		$validate = apply_filters( 'bgci_bp_habits_group_pre_validate', true, $group );
	...
	public function save() {
		 ...
		 * @since BuddyPress 1.0.0
		 *
		 * @param bgci_bp_habits_Group $this Current instance of the group item being saved. Passed by reference.
		 */
		 ...
		 * @since BuddyPress 1.0.0
		 *
		 * @param bgci_bp_habits_Group $this Current instance of the group item that was saved. Passed by reference.
		 */
		do_action_ref_array( 'groups_group_after_save', array( &$this ) );

		wp_cache_delete( $this->id, 'bgci_bp_habits' );
	...
	public function delete() {
		...
		$user_ids    = bgci_bp_habits_Member::get_group_member_ids( $this->id );
		...
		bgci_bp_habits_Member::delete_all( $this->id );
		 ...
		 * @since BuddyPress 1.2.0
		 *
		 * @param bgci_bp_habits_Group $this     Current instance of the group item being deleted. Passed by reference.
		 * @param array           $user_ids Array of user IDs that were members of the group.
		 */
		do_action_ref_array( 'bgci_bp_habits_delete_group', array( &$this, $user_ids ) );
	...
	protected function set_up_admins_and_mods() {
		$admin_ids         = bgci_bp_habits_Member::get_group_administrator_ids( $this->id );
		$admin_ids_plucked = wp_list_pluck( $admin_ids, 'user_id' );

		$mod_ids         = bgci_bp_habits_Member::get_group_moderator_ids( $this->id );
	 ...
	 *
	 * Alias of {@link bgci_bp_habits_Group::group_exists()}.
	 *
	 * @since BuddyPress 1.6.0
	 *
	 * @param string $slug See {@link bgci_bp_habits_Group::group_exists()}.
	 * @return int|null See {@link bgci_bp_habits_Group::group_exists()}.
	 */
	 ...
	 *                                            Default: 'all' (return bgci_bp_habits_Group objects).
	...
	public static function get( $args = array() ) {
		...
		if ( ! is_array( $args ) || count( $function_args ) > 1 ) {
			_deprecated_argument( __METHOD__, '1.7', sprintf( __( 'Arguments passed to %1$s should be in an associative array. See the inline documentation at %2$s for more details.', 'bp-group-check-in' ), __METHOD__, __FILE__ ) );
		...
		$r = bp_parse_args( $args, $defaults, 'bgci_bp_habits_group_get' );
		...
		if ( ! empty( $r['type'] ) ) {
			 ...
			 */
			$order_orderby = apply_filters( 'bgci_bp_habits_get_orderby', self::convert_type_to_order_orderby( $r['type'] ), $r['type'] );
		 ...
		 * @param string $value   Parsed 'type' value for the get method.
		 */
		$orderby = apply_filters( 'bgci_bp_habits_get_orderby_converted_by_term', self::convert_orderby_to_order_by_term( $orderby ), $orderby, $r['type'] );
		 ...
		 */
		$where_conditions = apply_filters( 'bgci_bp_habits_get_where_conditions', $where_conditions, $r );
		 ...
		 */
		$sql['from'] = apply_filters( 'bgci_bp_habits_get_join_sql', $sql['from'], $r );
		 ...
		 * @param array  $r     Array of parsed arguments for the get method.
		 */
		$paged_groups_sql = apply_filters( 'bgci_bp_habits_get_paged_groups_sql', $paged_groups_sql, $sql, $r );

		$cached = bp_core_get_incremented_cache( $paged_groups_sql, 'bgci_bp_habits' );
		if ( false === $cached ) {
			$paged_group_ids = $wpdb->get_col( $paged_groups_sql );
			bp_core_set_incremented_cache( $paged_groups_sql, 'bgci_bp_habits', $paged_group_ids );
		} else {
		...
		if ( 'ids' === $r['fields'] ) {
			...
			$uncached_group_ids = bp_get_non_cached_ids( $paged_group_ids, 'bgci_bp_habits' );
			if ( $uncached_group_ids ) {
				...
				foreach ( $group_data_objects as $group_data_object ) {
					wp_cache_set( $group_data_object->id, $group_data_object, 'bgci_bp_habits' );
				}
			}

			$paged_groups = array();
			foreach ( $paged_group_ids as $paged_group_id ) {
				$paged_groups[] = new bgci_bp_habits_Group( $paged_group_id );
			}
			...
			if ( ! empty( $r['update_meta_cache'] ) ) {
				bgci_bp_habits_update_meta_cache( $group_ids );
			}

			// Prefetch all administrator IDs, if requested.
			if ( $r['update_admin_cache'] ) {
				bgci_bp_habits_Member::prime_group_admins_mods_cache( $group_ids );
			}
		 ...
		 * @param array  $r         Array of parsed arguments for the get method.
		 */
		$total_groups_sql = apply_filters( 'bgci_bp_habits_get_total_groups_sql', $total_groups_sql, $sql, $r );

		$cached = bp_core_get_incremented_cache( $total_groups_sql, 'bgci_bp_habits' );
		if ( false === $cached ) {
			$total_groups = (int) $wpdb->get_var( $total_groups_sql );
			bp_core_set_incremented_cache( $total_groups_sql, 'bgci_bp_habits', $total_groups );
		} else {
	 ...
	 *
	 * Use bgci_bp_habits_Group::get() with 'type' = 'random' instead.
	 *
	 * @since BuddyPress 1.6.0
	 *
	 * @param int|null          $limit           Optional. The max number of results to return.
	...
	protected static function get_sql_clause_for_group_types( $group_types, $operator ) {
		...
		foreach ( $group_types as $gt ) {
			if ( bgci_bp_habits_get_group_type_object( $gt ) ) {
				$types[] = $gt;
			}
		}
