 ...
 */
function bgci_bp_habits_has_directory() {
	$bp = buddypress();
 ...
 * of instantiating bgci_bp_habits_Group directly, so that you will inherit cache
 ...
 * @return bgci_bp_habits_Group $group The group object.
 */
function groups_get_group( $group_id ) {
	...
	$group = new bgci_bp_habits_Group( $group_id );
	 ...
	 * @since BuddyPress 1.2.0
	 *
	 * @param bgci_bp_habits_Group $group Single group object.
	 */
...
function groups_create_group( $args = '' ) {
	...
	if ( ! empty( $group_id ) ) {
		...
		$group = new bgci_bp_habits_Group();
	...
	if ( empty( $group_id ) ) {
		$member                = new bgci_bp_habits_Member();
		$member->group_id      = $group->id;
		$member->user_id       = $group->creator_id;
		$member->is_admin      = 1;
		$member->user_title    = __( 'Group Organizer', 'bp-group-check-in' );
		 ...
		 * @param bgci_bp_habits_Member $member Instance of the member who is assigned
		 *                                 as group creator.
		 * @param bgci_bp_habits_Group  $group  Instance of the group being created.
		 */
		 ...
		 * @param bgci_bp_habits_Group $group Instance of the group being updated.
		 */
...
function groups_edit_base_group_details( $args = array() ) {
	...
	if ( ! is_array( $args ) || count( $function_args ) > 1 ) {
		_deprecated_argument( __METHOD__, '2.9.0', sprintf( __( 'Arguments passed to %1$s should be in an associative array. See the inline documentation at %2$s for more details.', 'bp-group-check-in' ), __METHOD__, __FILE__ ) );
	 ...
	 * @param bgci_bp_habits_Group $old_group      Group object, before being modified.
...
function groups_check_slug( $slug ) {
	...
	if ( bgci_bp_habits_Group::check_slug( $slug ) ) {
		do {
			$slug = $slug . '-' . rand();
		} while ( bgci_bp_habits_Group::check_slug( $slug ) );
	}
...
function groups_get_id( $group_slug ) {
	return bgci_bp_habits_Group::group_exists( $group_slug );
}
...
function groups_get_id_by_previous_slug( $group_slug ) {
	return bgci_bp_habits_Group::get_id_by_previous_slug( $group_slug );
}
...
function groups_leave_group( $group_id, $user_id = 0 ) {
	...
	if ( count( groups_get_group_admins( $group_id ) ) < 2 ) {
		if ( groups_is_user_admin( $user_id, $group_id ) ) {
			bp_core_add_message( __( 'This group must have at least one organizer.', 'bp-group-check-in' ), 'error' );
			return false;
		}
	}

	if ( ! bgci_bp_habits_Member::delete( $user_id, $group_id ) ) {
		return false;
	}

	bp_core_add_message( __( 'You successfully left the group.', 'bp-group-check-in' ) );
...
function groups_join_group( $group_id, $user_id = 0 ) {
	...
	$new_member                = new bgci_bp_habits_Member();
...
function groups_get_group_admins( $group_id ) {
	return bgci_bp_habits_Member::get_group_administrator_ids( $group_id );
}
...
function groups_get_group_mods( $group_id ) {
	return bgci_bp_habits_Member::get_group_moderator_ids( $group_id );
}
 ...
 * Previously called bgci_bp_habits_Member::get_all_for_group().
 *
...
function groups_get_group_members( $args = array() ) {
	...
	// Backward compatibility with old method of passing arguments.
	if ( ! is_array( $args ) || count( $function_args ) > 1 ) {
		_deprecated_argument( __METHOD__, '2.0.0', sprintf( __( 'Arguments passed to %1$s should be in an associative array. See the inline documentation at %2$s for more details.', 'bp-group-check-in' ), __METHOD__, __FILE__ ) );
	...
	// For legacy users. Use of bgci_bp_habits_Member::get_all_for_group() is deprecated.
	if ( apply_filters( 'bp_use_legacy_group_member_query', false, __FUNCTION__, $function_args ) ) {
		$retval = bgci_bp_habits_Member::get_all_for_group( $r['group_id'], $r['per_page'], $r['page'], $r['exclude_admins_mods'], $r['exclude_banned'], $r['exclude'] );
	} else {
...
function groups_get_total_member_count( $group_id ) {
	return bgci_bp_habits_Group::get_total_member_count( $group_id );
}
 ...
 *     {@link bgci_bp_habits_Group::get()}. Where the default values differ, they
 *     have been described here.
 *     @type int $per_page Default: 20.
 *     @type int $page Default: 1.
 * }
 * @return array See {@link bgci_bp_habits_Group::get()}.
 */
function groups_get_groups( $args = '' ) {
	...
	$defaults = array(
		...
		'fields'             => 'all',          // Return bgci_bp_habits_Group objects or a list of ids.
		'moderation_query'   => 'false',        // Remove moderation query.
	);

	$r = bp_parse_args( $args, $defaults, 'groups_get_groups' );

	$groups = bgci_bp_habits_Group::get(
		array(
			'type'               => $r['type'],
			'user_id'            => $r['user_id'],
	 ...
	 * @since BuddyPress 1.2.0
	 *
	 * @param bgci_bp_habits_Group $groups Object of found groups based on parameters.
	 *                                Passed by reference.
...
function groups_get_user_groups( $user_id = 0, $pag_num = 0, $pag_page = 0 ) {
	...
	return bgci_bp_habits_Member::get_group_ids( $user_id, $pag_num, $pag_page );
}
...
function bp_get_user_groups( $user_id, $args = array() ) {
	...
	$membership_ids = wp_cache_get( $user_id, 'bgci_bp_habits_memberships_for_user' );
	if ( false === $membership_ids ) {
		$membership_ids = bgci_bp_habits_Member::get_membership_ids_for_user( $user_id );
		wp_cache_set( $user_id, $membership_ids, 'bgci_bp_habits_memberships_for_user' );
	}

	// Prime the membership cache.
	$uncached_membership_ids = bp_get_non_cached_ids( $membership_ids, 'bgci_bp_habits_memberships' );
	if ( ! empty( $uncached_membership_ids ) ) {
		$uncached_memberships = bgci_bp_habits_Member::get_memberships_by_id( $uncached_membership_ids );

		foreach ( $uncached_memberships as $uncached_membership ) {
			wp_cache_set( $uncached_membership->id, $uncached_membership, 'bgci_bp_habits_memberships' );
		}
	}
	...
	if ( true !== $r['is_confirmed'] || false !== $r['invite_sent'] ) {
		...
		$uncached_invitation_ids = bp_get_non_cached_ids( $invitation_ids, 'bgci_bp_habits_invitations_as_memberships' );
		if ( $uncached_invitation_ids ) {
			...
			foreach ( $uncached_invitations as $uncached_invitation ) {
				...
				wp_cache_set( $uncached_invitation->id, $invitation, 'bgci_bp_habits_invitations_as_memberships' );
			}
		}
	}
	...
	foreach ( $membership_ids as $membership_id ) {
		$membership = wp_cache_get( $membership_id, 'bgci_bp_habits_memberships' );
	...
	foreach ( $invitation_ids as $invitation_id ) {
		$invitation = wp_cache_get( $invitation_id, 'bgci_bp_habits_invitations_as_memberships' );
...
function groups_total_groups_for_user( $user_id = 0 ) {
	...
	if ( false === $count ) {
		$count = bgci_bp_habits_Member::total_group_count( $user_id );
...
/**
 * Get the bgci_bp_habits_Group object corresponding to the current group.
 *
 * @since BuddyPress 1.5.0
 *
 * @return bgci_bp_habits_Group The current group object.
 */
function groups_get_current_group() {
	...
	/**
	 * Filters the bgci_bp_habits_Group object corresponding to the current group.
	 *
	 * @since BuddyPress 1.5.0
	 *
	 * @param bgci_bp_habits_Group $current_group Current bgci_bp_habits_Group object.
	 */
...
function bgci_bp_habits_get_group_roles() {
	return array(
		'admin' => (object) array(
			'id'           => 'admin',
			'name'         => __( 'Organizer', 'bp-group-check-in' ),
			'is_admin'     => true,
			'is_banned'    => false,
			'is_confirmed' => true,
			'is_mod'       => false,
		...
		'mod' => (object) array(
			'id'           => 'mod',
			'name'         => __( 'Moderator', 'bp-group-check-in' ),
			'is_admin'     => false,
			'is_banned'    => false,
			'is_confirmed' => true,
			'is_mod'       => true,
		...
		'member' => (object) array(
			'id'           => 'member',
			'name'         => __( 'Member', 'bp-group-check-in' ),
			'is_admin'     => false,
			'is_banned'    => false,
			'is_confirmed' => true,
			'is_mod'       => false,
		...
		'banned' => (object) array(
			'id'           => 'banned',
			'name'         => __( 'Banned', 'bp-group-check-in' ),
			'is_admin'     => false,
			'is_banned'    => true,
			'is_confirmed' => true,
			'is_mod'       => false,
...
function groups_is_user_pending( $user_id, $group_id ) {
	...
	$invites_class = new bgci_bp_habits_Invitation_Manager();
...
function groups_is_user_creator( $user_id, $group_id ) {
	return bgci_bp_habits_Member::check_is_creator( $user_id, $group_id );
}
...
function groups_post_update( $args = '' ) {
	...
	// Record this in activity feeds.
	$activity_action  = sprintf( __( '%1$s posted an update in the group %2$s', 'bp-group-check-in' ), bp_core_get_userlink( $user_id ), '<a href="' . bp_get_group_permalink( $bp->groups->current_group ) . '">' . esc_attr( $bp->groups->current_group->name ) . '</a>' );
	 ...
	 */
	do_action( 'bgci_bp_habits_posted_update', $content, $user_id, $group_id, $activity_id );
...
function groups_send_invites( $args = array() ) {
	...
	// Backward compatibility with old method of passing arguments.
	if ( ! is_array( $args ) || count( $function_args ) > 1 ) {
		_deprecated_argument( __METHOD__, '1.3.5', sprintf( __( 'Arguments passed to %1$s should be in an associative array. See the inline documentation at %2$s for more details.', 'bp-group-check-in' ), __METHOD__, __FILE__ ) );
...
function groups_get_invites_for_group( $user_id, $group_id, $sent = null ) {
	return bgci_bp_habits_Group::get_invites( $user_id, $group_id, $sent );
}
...
function groups_delete_all_group_invites( $group_id ) {
	return bgci_bp_habits_Group::delete_all_invites( $group_id );
}
...
function groups_promote_member( $user_id, $group_id, $status ) {
	...
	$member = new bgci_bp_habits_Member( $user_id, $group_id );
...
function groups_send_membership_request( $args = array() ) {
	...
	if ( groups_check_user_has_invite( $inv_args['user_id'], $inv_args['item_id'] ) ) {
		...
		if ( true === bp_member_type_enable_disable() && true === bp_disable_group_type_creation() ) {
			...
			$group_type = bgci_bp_habits_get_group_type( $inv_args['item_id'] );
...
function groups_accept_membership_request( $membership_id, $user_id = 0, $group_id = 0 ) {
	...
	if ( ! empty( $membership_id ) ) {
		_deprecated_argument( __METHOD__, '1.3.5', sprintf( __( 'Argument `membership_id` passed to %1$s  is deprecated. See the inline documentation at %2$s for more details.', 'bp-group-check-in' ), __METHOD__, __FILE__ ) );
	}
 ...
 *                           requested. Provide this value along with $user_id to
 *                           override $membership_id.
 * @return false|bgci_bp_habits_Member True on success, false on failure.
 */
...
function groups_remove_data_for_user( $user_id ) {
	bgci_bp_habits_Member::delete_all_for_user( $user_id );
 ...
 *
 * @param bgci_bp_habits_Group $group Instance of the group item being deleted.
 */
function bgci_bp_habits_update_orphaned_groups_on_group_delete( $group ) {
...
add_action( 'bgci_bp_habits_delete_group', 'bgci_bp_habits_update_orphaned_groups_on_group_delete', 10, 2 );

/** Group Types ***************************************************************/

/**
 * Fire the 'bgci_bp_habits_register_group_types' action.
 *
 * @since BuddyPress 2.6.0
 */
function bgci_bp_habits_register_group_types() {
	/**
	 * Fires when it's appropriate to register group types.
	 *
	 * @since BuddyPress 2.6.0
	 */
	do_action( 'bgci_bp_habits_register_group_types' );
}
add_action( 'bp_register_taxonomies', 'bgci_bp_habits_register_group_types' );
 ...
 */
function bgci_bp_habits_register_group_type( $group_type, $args = array() ) {
	$bp = buddypress();

	if ( isset( $bp->groups->types[ $group_type ] ) ) {
		return new WP_Error( 'bp_group_type_exists', __( 'Group type already exists.', 'bp-group-check-in' ), $group_type );
	}
	...
	if ( in_array( $group_type, $illegal_names, true ) ) {
		return new WP_Error( 'bp_group_type_illegal_name', __( 'You may not register a group type with this name.', 'bp-group-check-in' ), $group_type );
	}
	 ...
	 */
	do_action( 'bgci_bp_habits_register_group_type', $group_type, $type );
 ...
 * @since BuddyPress 2.6.0
 *
 * @see bgci_bp_habits_register_group_type() for accepted arguments.
 *
 ...
 */
function bgci_bp_habits_get_group_types( $args = array(), $output = 'names', $operator = 'and' ) {
	 ...
	 */
	$types = apply_filters( 'bgci_bp_habits_get_group_types', $types, $args, $operator );
 ...
 */
function bgci_bp_habits_get_group_type_object( $group_type ) {
 ...
 */
function bgci_bp_habits_set_group_type( $group_id, $group_type, $append = false ) {
	// Pass an empty group type to remove group's type.
	if ( ! empty( $group_type ) && is_string( $group_type ) && ! bgci_bp_habits_get_group_type_object( $group_type ) ) {
		return false;
	}
	...
	foreach ( $group_type as $type ) {
		...
		if ( is_null( bgci_bp_habits_get_group_type_object( $type ) ) ) {
	...
	if ( ! is_wp_error( $retval ) ) {
		wp_cache_delete( $group_id, 'bgci_bp_habits_group_type' );
		 ...
		 */
		do_action( 'bgci_bp_habits_set_group_type', $group_id, $group_type, $append );
	}
 ...
 */
function bgci_bp_habits_get_group_type( $group_id, $single = true ) {
	$types = wp_cache_get( $group_id, 'bgci_bp_habits_group_type' );
	...
	if ( false === $types ) {
		...
		if ( ! is_wp_error( $raw_types ) ) {
			...
			foreach ( $raw_types as $gtype ) {
				if ( bgci_bp_habits_get_group_type_object( $gtype->name ) ) {
					$types[] = $gtype->name;
				}
			}

			wp_cache_set( $group_id, $types, 'bgci_bp_habits_group_type' );
		}
	}
	 ...
	 */
	return apply_filters( 'bgci_bp_habits_get_group_type', $type, $group_id, $single );
}
 ...
 */
function bgci_bp_habits_remove_group_type( $group_id, $group_type ) {
	if ( empty( $group_type ) || ! bgci_bp_habits_get_group_type_object( $group_type ) ) {
		return false;
	}

	// No need to continue if the group doesn't have the type.
	$existing_types = bgci_bp_habits_get_group_type( $group_id, false );
	...
	if ( ! is_wp_error( $deleted ) ) {
		 ...
		 * @param string $group_type Group type.
		 */
		do_action( 'bgci_bp_habits_remove_group_type', $group_id, $group_type );
	}
 ...
 */
function bgci_bp_habits_has_group_type( $group_id, $group_type ) {
	if ( empty( $group_type ) || ! bgci_bp_habits_get_group_type_object( $group_type ) ) {
		return false;
	}

	// Get all group's group types.
	$types = bgci_bp_habits_get_group_type( $group_id, false );
 ...
 *
 * @param  int $group_id ID of the group.
 * @return array|null $value    See {@see bgci_bp_habits_set_group_type()}.
 */
function bp_remove_group_type_on_group_delete( $group_id = 0 ) {
	bgci_bp_habits_set_group_type( $group_id, '' );
}
 ...
 * @return array
 */
function bgci_bp_habits_get_invited_by( $user_id = false, $group_id = false ) {
	...
	//$member = new bgci_bp_habits_Member( $user_id, $group->id );
	$inviter = array(
	...
	return apply_filters( 'bgci_bp_habits_get_invited_by', $inviter, $group->id );
}
 ...
 *
 */
function bgci_bp_habits_get_invite_messsage_for_user( $user_id, $group_id ) {
	global $groups_template;
	 ...
	 * @param int    $group_id group id.
	 */
	return apply_filters( 'bgci_bp_habits_get_invite_messsage_for_user', $message, $group_id );
}
 ...
 * @param int $group_id The group ID.
 */
function bgci_bp_habits_clear_invite_message_for_user( $user_id, $group_id ) {
	delete_user_meta( $user_id, 'bp_group_invite_message_' . $group_id );
}
add_action( 'groups_accept_invite', 'bgci_bp_habits_clear_invite_message_for_user', 10, 2 );
add_action( 'groups_reject_invite', 'bgci_bp_habits_clear_invite_message_for_user', 10, 2 );
add_action( 'groups_delete_invite', 'bgci_bp_habits_clear_invite_message_for_user', 10, 2 );
 ...
 * @param int $user_id The user ID.
 */
function bgci_bp_habits_clear_invite_message_on_uninvite( $group_id, $user_id ) {
	bgci_bp_habits_clear_invite_message_for_user( $user_id, $group_id );
}
add_action( 'groups_uninvite_user', 'bgci_bp_habits_clear_invite_message_on_uninvite', 10, 2 );
add_action( 'groups_remove_member', 'bgci_bp_habits_clear_invite_message_on_uninvite', 10, 2 );
 ...
 */
function bgci_bp_habits_group_type_post_type() {
	echo bgci_bp_habits_get_group_type_post_type();
}
 ...
 */
function bgci_bp_habits_get_group_type_post_type() {
	 ...
	 */
	return apply_filters( 'bgci_bp_habits_get_group_type_post_type', buddypress()->group_type_post_type );
}
 ...
 * @return array
 */
function bgci_bp_habits_get_group_type_post_type_labels() {
	 ...
	 */
	return apply_filters(
		'bgci_bp_habits_get_group_type_post_type_labels',
		array(
			'add_new_item'       => __( 'New Group Type', 'bp-group-check-in' ),
			'all_items'          => __( 'Group Types', 'bp-group-check-in' ),
			'edit_item'          => __( 'Edit Group Type', 'bp-group-check-in' ),
			'menu_name'          => __( 'Social Groups', 'bp-group-check-in' ),
			'name'               => __( 'Group Types', 'bp-group-check-in' ),
			'new_item'           => __( 'New Group Type', 'bp-group-check-in' ),
			'not_found'          => __( 'No Group Types found', 'bp-group-check-in' ),
			'not_found_in_trash' => __( 'No Group Types found in trash', 'bp-group-check-in' ),
			'search_items'       => __( 'Search Group Types', 'bp-group-check-in' ),
			'singular_name'      => __( 'Group Type', 'bp-group-check-in' ),
		)
	);
}
 ...
 * @return array
 */
function bgci_bp_habits_get_group_type_post_type_supports() {
	 ...
	 */
	return apply_filters(
		'bgci_bp_habits_get_group_type_post_type_supports',
		array(
			'page-attributes',
			'title',
		)
...
function bp_get_active_group_types( $args = array() ) {
	...
	$args = bp_parse_args( $args, array(
		'posts_per_page' => - 1,
		'post_type'      => bgci_bp_habits_get_group_type_post_type(),
		'orderby'        => 'menu_order',
		'order'          => 'ASC',
		'fields'         => 'ids'
	), 'group_types' );
...
if ( true === bp_disable_group_type_creation() ) {
	...
	add_action( 'bgci_bp_habits_register_group_types', 'bp_register_active_group_types' );
	...
	if ( ! empty( $group_type_ids ) ) {
		...
		foreach ( $group_type_ids as $group_type_id ) {
			...
			if ( ! in_array( $singular_name, $group_types, true ) ) {
				...
				// Register group type.
				bgci_bp_habits_register_group_type( $key, $temp );
			}
		}
	}
}
 ...
 * @return bool|string
 */
function bgci_bp_habits_exclude_group_type( $qs = false, $object = false ) {
 ...
 * @return array
 */
function bgci_bp_habits_get_excluded_group_ids_by_type() {
 ...
 * @return array
 */
function bgci_bp_habits_get_excluded_group_types() {
...
function bp_get_group_ids_by_group_types( $group_type = '', $taxonomy = 'bp_group_type' ) {
	...
	$groups             = $bp->table_prefix . 'bgci_bp_habits';
	$group_meta         = $bp->table_prefix . 'bgci_bp_habits_groupmeta';
...
function get_group_role_label( $group_id, $label_name ) {
	...
	$get_group_type = bgci_bp_habits_get_group_type( $group_id );

	if ( ! $get_group_type ) {

		if ( 'organizer_plural_label_name' === $label_name ) {
			$label = __( 'Organizers', 'bp-group-check-in' );
		} elseif ( 'moderator_plural_label_name' === $label_name ) {
			$label = __( 'Moderators', 'bp-group-check-in' );
		} elseif ( 'member_plural_label_name' === $label_name ) {
			$label = __( 'Members', 'bp-group-check-in' );
		} elseif ( 'organizer_singular_label_name' === $label_name ) {
			$label = __( 'Organizer', 'bp-group-check-in' );
		} elseif ( 'moderator_singular_label_name' === $label_name ) {
			$label = __( 'Moderator', 'bp-group-check-in' );
		} elseif ( 'member_singular_label_name' === $label_name ) {
			$label = __( 'Member', 'bp-group-check-in' );
		}
	} else {
		...
		if ( 'organizer_plural_label_name' === $label_name ) {
			$label = ( isset( $group_type_roles['organizer_plural_label_name'] ) && $group_type_roles['organizer_plural_label_name'] ) ? $group_type_roles['organizer_plural_label_name'] : __( 'Organizers', 'bp-group-check-in' );
		} elseif ( 'moderator_plural_label_name' === $label_name ) {
			$label = ( isset( $group_type_roles['moderator_plural_label_name'] ) && $group_type_roles['moderator_plural_label_name'] ) ? $group_type_roles['moderator_plural_label_name'] : __( 'Moderators', 'bp-group-check-in' );
		} elseif ( 'member_plural_label_name' === $label_name ) {
			$label = ( isset( $group_type_roles['member_plural_label_name'] ) && $group_type_roles['member_plural_label_name'] ) ? $group_type_roles['member_plural_label_name'] : __( 'Members', 'bp-group-check-in' );
		} elseif ( 'organizer_singular_label_name' === $label_name ) {
			$label = ( isset( $group_type_roles['organizer_singular_label_name'] ) && $group_type_roles['organizer_singular_label_name'] ) ? $group_type_roles['organizer_singular_label_name'] : __( 'Organizer', 'bp-group-check-in' );
		} elseif ( 'moderator_singular_label_name' === $label_name ) {
			$label = ( isset( $group_type_roles['moderator_singular_label_name'] ) && $group_type_roles['moderator_singular_label_name'] ) ? $group_type_roles['moderator_singular_label_name'] : __( 'Moderator', 'bp-group-check-in' );
		} elseif ( 'member_singular_label_name' === $label_name ) {
			$label = ( isset( $group_type_roles['member_singular_label_name'] ) && $group_type_roles['member_singular_label_name'] ) ? $group_type_roles['member_singular_label_name'] : __( 'Member', 'bp-group-check-in' );
		}
	}
 ...
 */
function bgci_bp_habits_prime_mentions_results() {
...
add_action( 'bp_activity_mentions_prime_results', 'bgci_bp_habits_prime_mentions_results' );
add_action( 'bbp_forums_mentions_prime_results', 'bgci_bp_habits_prime_mentions_results' );
 ...
 * @since BuddyPress 5.0.0
 */
function bgci_bp_habits_migrate_invitations() {
	global $wpdb;
	$bp = buddypress();
	...
	foreach ( $records as $record ) {
		$values[] = $wpdb->prepare(
			...
			'',
			'bgci_bp_habits_invitation_manager',
			(int) $record->group_id,
			0,
 ...
 * @param int $group_id Group ID.
 */
function bgci_bp_habits_reset_cover_image_position( $group_id ) {
	if ( ! empty( (int) $group_id ) ) {
		groups_delete_groupmeta( $group_id, 'bp_cover_position' );
	}
}

add_action( 'groups_cover_image_uploaded', 'bgci_bp_habits_reset_cover_image_position', 10, 1 );
add_action( 'groups_cover_image_deleted', 'bgci_bp_habits_reset_cover_image_position', 10, 1 );
