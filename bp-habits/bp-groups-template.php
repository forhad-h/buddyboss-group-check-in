 ...
 * @since BuddyPress 1.5.0
 */
function bgci_bp_habits_root_slug() {
 ...
 */
function bgci_bp_habits_group_type_base() {
...
function bp_get_groups_group_type_base() {
	 ...
	 */
	return apply_filters( 'bgci_bp_habits_group_type_base', _x( 'type', 'Group type URL base slug', 'bp-group-check-in' ) );
}
 ...
 * @since BuddyPress 1.5.0
 */
function bgci_bp_habits_directory_permalink() {
...
function bp_get_group_type_directory_permalink( $group_type = '' ) {
	...
	$type = bgci_bp_habits_get_group_type_object( $_group_type );
 ...
 * @since BuddyPress 2.7.0
 *
 * @param string $group_type Unique group type identifier as used in bgci_bp_habits_register_group_type().
 */
...
function bp_get_group_type_directory_link( $group_type = '' ) {
	...
	return sprintf( '<a href="%s">%s</a>', esc_url( bp_get_group_type_directory_permalink( $group_type ) ), bgci_bp_habits_get_group_type_object( $group_type )->labels['singular_name'] );
}
...
function bp_group_type_list( $group_id = 0, $r = array() ) {
	 ...
	 *                                  true. See bgci_bp_habits_register_group_type() for more info.
	 * }
	 * @return string
	 */
function bp_get_group_type_list( $group_id = 0, $r = array() ) {
	...
	$r = bp_parse_args(
		...
		array(
			...
			'label'          => __( 'Group Types:', 'bp-group-check-in' ),
	...
	if ( $types = bgci_bp_habits_get_group_type( $group_id, false ) ) {
		// Make sure we can show the type in the list.
		if ( false === $r['show_all'] ) {
			$types = array_intersect( bgci_bp_habits_get_group_types( array( 'show_in_list' => true ) ), $types );
			if ( empty( $types ) ) {
				return $retval;
			}
		}
...
function bp_has_groups( $args = '' ) {
	...
	if (
		...
		$bp_group_type_ids = bgci_bp_habits_get_excluded_group_types();
	...
	$groups_template = new bgci_bp_habits_Template( $args );
	 ...
	 * @param bgci_bp_habits_Template $groups_template bgci_bp_habits_Template object based on parsed arguments.
 ...
 * In BP 2.9, a property was added to the bgci_bp_habits_Group class,
 ...
 * @since BuddyPress 1.0.0
 *
 * @param bgci_bp_habits_Group|null $group Optional. Group object. Default: current group in loop.
 * @return bool
 */
...
function bp_get_group_class( $classes = array() ) {
	...
	if ( $group_types = bgci_bp_habits_get_group_type( bp_get_group_id(), false ) ) {
...
function bp_get_group_type( $group = false ) {
	...
	if ( true === bp_disable_group_type_creation() ) {
		...
		$group_type = bgci_bp_habits_get_group_type( $group->id );
		$group_type = bgci_bp_habits_get_group_type_object( $group_type );
		$group_type = isset( $group_type->labels['singular_name'] ) ? $group_type->labels['singular_name'] : '';

		if ( 'public' == $group->status ) {

			$group_visibility = __( 'Public', 'bp-group-check-in' );
			$type             = ! empty( $group_type ) ? '<span class="group-visibility public">' . $group_visibility . '</span> <span class="type-separator">/</span> <span class="group-type">' . $group_type . '</span>' : '<span class="group-visibility public">' . __( 'Public', 'bp-group-check-in' ) . ' <span class="type-separator">/</span> <span class="group-type">' . __( 'Group', 'bp-group-check-in' ) . '</span>';

		} elseif ( 'hidden' == $group->status ) {

			$group_visibility = __( 'Hidden', 'bp-group-check-in' );
			$type             = ! empty( $group_type ) ? '<span class="group-visibility hidden">' . $group_visibility . '</span> <span class="type-separator">/</span> <span class="group-type">' . $group_type . '</span>' : '<span class="group-visibility hidden">' . __( 'Hidden', 'bp-group-check-in' ) . ' <span class="type-separator">/</span> <span class="group-type">' . __( 'Group', 'bp-group-check-in' ) . '</span>';

		} elseif ( 'private' == $group->status ) {

			$group_visibility = __( 'Private', 'bp-group-check-in' );
			$type             = ! empty( $group_type ) ? '<span class="group-visibility private">' . $group_visibility . '</span> <span class="type-separator">/</span> <span class="group-type">' . $group_type . '</span>' : '<span class="group-visibility private">' . __( 'Private', 'bp-group-check-in' ) . ' <span class="type-separator">/</span> <span class="group-type">' . __( 'Group', 'bp-group-check-in' ) . '</span>';

		} else {
			$type = ucwords( $group->status ) . ' ' . __( 'Group', 'bp-group-check-in' );
		}
	} else {

		if ( 'public' == $group->status ) {
			$type = '<span class="group-visibility public">' . __( 'Public Group', 'bp-group-check-in' ) . '</span>';
		} elseif ( 'hidden' == $group->status ) {
			$type = '<span class="group-visibility hidden">' . __( 'Hidden Group', 'bp-group-check-in' ) . '</span>';
		} elseif ( 'private' == $group->status ) {
			$type = '<span class="group-visibility private">' . __( 'Private Group', 'bp-group-check-in' ) . '</span>';
		} else {
			$type = ucwords( $group->status ) . ' ' . __( 'Group', 'bp-group-check-in' );
		}
	}
...
function bp_get_group_status_description( $group = false ) {
	...
	if ( 'public' == $group->status ) {
		$description = __( 'This group\'s content, including its members and activity, are visible to any site member.', 'bp-group-check-in' );
	} elseif ( 'hidden' == $group->status ) {
		$description = __( 'This group and its content are only visible to members of the group.', 'bp-group-check-in' );
	} elseif ( 'private' == $group->status ) {
		$description = __( 'This group\'s content is only visible to members of the group.', 'bp-group-check-in' );
	} else {
		$description = ucwords( $group->status ) . ' ' . __( 'Group', 'bp-group-check-in' );
	}
...
function bp_get_group_avatar( $args = '' ) {
	...
	$r = bp_parse_args(
		...
		array(
			...
			'class'  => 'avatar',
			'id'     => false,
			'alt'    => sprintf( __( 'Group logo of %s', 'bp-group-check-in' ), $groups_template->group->name ),
		)
	);
...
function bp_get_group_last_active( $group = false, $args = array() ) {
	...
	if ( empty( $last_active ) ) {
		return __( 'not yet active', 'bp-group-check-in' );
	} else {
 ...
 *
 * @param bgci_bp_habits_Group|null $group Optional. Group object.
 *                                    Default: current group in loop.
 */
...
function bp_get_group_link( $group = null ) {
	 ...
	 * @param bgci_bp_habits_Group $group The current group object.
	 */
...
function bp_get_group_public_status( $group = false ) {
	...
	if ( $group->is_public ) {
		return __( 'Public', 'bp-group-check-in' );
	} else {
		return __( 'Private', 'bp-group-check-in' );
	}
}
...
function bp_get_group_creator_avatar( $group = false, $args = array() ) {
	...
	$r = bp_parse_args(
		...
		array(
			...
			'class'  => 'avatar',
			'id'     => false,
			'alt'    => sprintf( __( 'Group organizer profile photo of %s', 'bp-group-check-in' ), bp_core_get_user_displayname( $group->creator_id ) ),
		),
...
function bp_group_list_admins( $group = false ) {
	...
	<?php } else { ?>
		<span class="activity"><?php _e( 'No Organizers', 'bp-group-check-in' ); ?></span>
	<?php } ?>
	<?php
}
...
function bp_group_list_parents( $group = false ) {
	...
	if ( ! empty( $group->parent_id ) ) {
		...
		$group_type        = bgci_bp_habits_get_group_type( $group->parent_id );
		$group_type_object = bgci_bp_habits_get_group_type_object( $group_type );
		$group_type        = ( isset( $group_type_object ) && isset( $group_type_object->labels['singular_name'] ) ) ?: esc_html__( 'Subgroup of', 'bp-group-check-in' );
		?>
					...
					<li>
						...
						echo bp_core_fetch_avatar(
							array(
								...
								'object'  => 'group',
								'alt'     => sprintf( __( 'Group photo of %s', 'bp-group-check-in' ), bp_get_group_name( $parent_group ) ),
							)
						)
						?>
						</a>
...
function bp_group_list_mods( $group = false ) {
	...
	<?php else : ?>
		...
		<span class="activity"><?php _e( 'No Moderators', 'bp-group-check-in' ); ?></span>
 ...
 * @since BuddyPress 1.5.0
 *
 * @param bgci_bp_habits_Group|bool $group     Optional. The group being queried. Defaults
 ...
 * @since BuddyPress 1.5.0
 *
 * @param bgci_bp_habits_Group|bool $group   Optional. The group being queried.
...
function bp_group_search_form() {
	...
	$label  = __( 'Filter Groups', 'bp-group-check-in' );
 ...
 */
function bgci_bp_habits_pagination_links() {
 ...
 * @since BuddyPress 1.2.0
 */
function bgci_bp_habits_pagination_count() {
...
function bp_get_groups_pagination_count() {
	...
	$message = sprintf( _n( 'Viewing 1 group', 'Viewing %1$s - %2$s of %3$s groups', $groups_template->total_group_count, 'bp-group-check-in' ), $from_num, $to_num, $total );
 ...
 * @return bool
 */
function bgci_bp_habits_auto_join() {
	 ...
	 */
	return apply_filters( 'bgci_bp_habits_auto_join', (bool) buddypress()->groups->auto_join );
}
...
function bp_get_group_member_count() {
	...
	$count_string = sprintf( _n( '%s member', '%s members', $count, 'bp-group-check-in' ), bp_core_number_format( $count ) );
 ...
 * @since BuddyBoss 1.0.0
 *
 * @param object|bool $group   bgci_bp_habits_Group object to check.
 * @param int|bool    $user_id ID of a user to check group visibility for.
...
function bp_include_group_by_context( $group = false, $user_id = false, $context = 'normal' ) {
	 ...
	 * @param bgci_bp_habits_Group $group   The group object in question.
 ...
 * @return bool
 */
function bgci_bp_habits_user_can_send_invites( $group_id = 0, $user_id = 0 ) {
	 ...
	 */
	return apply_filters( 'bgci_bp_habits_user_can_send_invites', $can_send_invites, $group_id, $invite_status, $user_id );
}
...
function bp_group_admin_memberlist( $admin_list = false, $group = false ) {
		...
		<?php foreach ( (array) $admins as $admin ) { ?>
			...
			<li>
				...
				<h5>
					...
					<span class="small">
						<a class="button confirm admin-demote-to-member" href="<?php bp_group_member_demote_link( $admin->user_id ); ?>"><?php _e( 'Demote to member', 'bp-group-check-in' ); ?></a>
					</span>
				</h5>
			</li>
			...
			<li>
				...
				<span class="activity">
					<?php echo bp_core_get_last_activity( strtotime( $admin->date_modified ), __( 'joined %s', 'bp-group-check-in' ) ); ?>
				</span>
	...
	<?php else : ?>
		...
		<div id="message" class="info">
			<p><?php _e( 'This group has no organizers', 'bp-group-check-in' ); ?></p>
		</div>
...
function bp_group_mod_memberlist( $admin_list = false, $group = false ) {
		...
		<?php foreach ( (array) $group_mods as $mod ) { ?>
			...
			<li>
				...
				<h5>
					...
					<span class="small">
						<a href="<?php bp_group_member_promote_admin_link( array( 'user_id' => $mod->user_id ) ); ?>" class="button confirm mod-promote-to-admin"><?php _e( 'Promote to co-organizer', 'bp-group-check-in' ); ?></a>
						<a class="button confirm mod-demote-to-member" href="<?php bp_group_member_demote_link( $mod->user_id ); ?>"><?php _e( 'Demote to member', 'bp-group-check-in' ); ?></a>
					</span>
				</h5>
			</li>
			...
			<li>
				...
				<span class="activity"><?php echo bp_core_get_last_activity( strtotime( $mod->date_modified ), __( 'joined %s', 'bp-group-check-in' ) ); ?></span>
	...
	<?php } else { ?>
		...
		<div id="message" class="info">
			<p><?php _e( 'This group has no moderators', 'bp-group-check-in' ); ?></p>
		</div>
...
function bp_group_admin_tabs_backcompat( $subnav_output = '', $subnav_item = '', $selected_item = '' ) {
	...
	if ( ! empty( $admin_tabs_backcompat ) ) {
		_doing_it_wrong( "do_action( 'groups_admin_tabs' )", __( 'This action should not be used directly. Please use the BuddyPress Group Extension API to generate Manage tabs.', 'bp-group-check-in' ), '2.2.0' );
...
function bp_get_group_total_for_member() {
	 ...
	 */
	return apply_filters( 'bp_get_group_total_for_member', bgci_bp_habits_Member::total_group_count() );
}
 ...
 * @since BuddyPress 1.5.0
 *
 * @global bgci_bp_habits_Template $groups_template Group template loop object.
 *
 * @param bgci_bp_habits_Group|bool $group   Group to check if user is banned.
 * @param int                  $user_id The user ID to check.
...
function bp_group_is_user_banned( $group = false, $user_id = 0 ) {
	...
	// @see bgci_bp_habits_Group::get_group_extras().
	...
	function bp_get_group_join_button( $group = false ) {
		...
		if ( ! empty( $group->is_member ) ) {
			...
			if ( groups_is_user_admin( bp_loggedin_user_id(), $group->id ) ) {
				$button_text = apply_filters( 'bp_group_organizer_label_text', sprintf( __( 'You\'re an %s', 'bp-group-check-in' ), get_group_role_label( $group->id, 'organizer_singular_label_name' ) ), $group->id, get_group_role_label( $group->id, 'organizer_singular_label_name' ) );
			} elseif ( groups_is_user_mod( bp_loggedin_user_id(), $group->id ) ) {
				$button_text = apply_filters( 'bp_group_moderator_label_text', sprintf( __( 'You\'re a %s', 'bp-group-check-in' ), get_group_role_label( $group->id, 'moderator_singular_label_name' ) ), $group->id, get_group_role_label( $group->id, 'moderator_singular_label_name' ) );
			} else {
				$button_text = apply_filters( 'bp_group_member_label_text', sprintf( __( 'You\'re a %s', 'bp-group-check-in' ), get_group_role_label( $group->id, 'member_singular_label_name' ) ), $group->id, get_group_role_label( $group->id, 'member_singular_label_name' ) );
			}
			...
			$button = array(
				...
				'button_attr'       => array(
					'data-title'           => __( 'Leave group', 'bp-group-check-in' ),
			...
			switch ( $group->status ) {
				...
				case 'public':
					$button = array(
						...
						'link_text'         => __( 'Join group', 'bp-group-check-in' ),
				...
				case 'private':
					...
					if ( $group->is_invited ) {
						$button = array(
							...
							'link_text'         => __( 'Accept Invitation', 'bp-group-check-in' ),
						...
						$button = array(
							...
							'link_text'         => __( 'Request Sent', 'bp-group-check-in' ),
						...
						if ( true === bp_member_type_enable_disable() && true === bp_disable_group_type_creation() ) {
							...
							if ( in_array( $get_requesting_user_member_type, $get_selected_member_type_join ) ) {
								...
								$button = array(
									...
									'wrapper_id'        => 'groupbutton-' . $group->id,
									'link_href'         => wp_nonce_url( trailingslashit( bp_get_group_permalink( $group ) . 'request-membership' ), 'groups_request_membership' ),
									'link_text'         => __( 'Request Access', 'bp-group-check-in' ),
									'link_class'        => 'group-button request-membership',
								);
							}
						} else {
...
function bp_get_group_create_button() {
	...
	$button_args = array(
		'id'         => 'create_group',
		'component'  => 'groups',
		'link_text'  => __( 'Create a Group', 'bp-group-check-in' ),
...
function bp_group_backcompat_create_nav_item() {
	...
	if ( ! has_filter( 'bgci_bp_habits_directory_header' ) ) {
		return;
	}

	bp_group_create_nav_item();
}
add_action( 'bgci_bp_habits_directory_group_filter', 'bp_group_backcompat_create_nav_item', 1000 );
 ...
 * @since BuddyPress 1.0.0
 *
 * @global bgci_bp_habits_Template $groups_template Groups template object.
 *
...
function bp_group_status_message( $group = null ) {
	...
	if ( empty( $group->status ) ) {
		$message = __( 'This group is not currently accessible.', 'bp-group-check-in' );
		...
		switch ( $group->status ) {
				...
				if ( ! bp_group_has_requested_membership( $group ) ) {
					if ( is_user_logged_in() ) {
						if ( bp_group_is_invited( $group ) ) {
							$message = __( 'You must accept your pending invitation before you can access this private group.', 'bp-group-check-in' );
						} else {
							$message = __( 'This is a private group and you must request group membership in order to join.', 'bp-group-check-in' );
						}
					} else {
						$message = __( 'This is a private group. To join you must be a registered site member and request group membership.', 'bp-group-check-in' );
					}
				} else {
					$message = __( 'This is a private group. Your membership request is awaiting approval from the group organizer.', 'bp-group-check-in' );
				}
			...
			case 'hidden':
			default:
				$message = __( 'This is a hidden group. You must be invited to join.', 'bp-group-check-in' );
				break;
		}
	}
...
function bp_group_has_members( $args = '' ) {
	...
	$members_template = new bgci_bp_habits_Group_Members_Template( $r );
	 ...
	 * @param bgci_bp_habits_Group_Members_Template $members_template Object holding the member query results.
	 */
		...
		?>
			<li class="item-entry item-entry-header">
				...
				if ( groups_is_user_admin( $user_id, $group_id ) ) {
					esc_html_e( get_group_role_label( $group_id, 'organizer_plural_label_name' ), 'bp-group-check-in' );
				} elseif ( groups_is_user_mod( $user_id, $group_id ) ) {
					esc_html_e( get_group_role_label( $group_id, 'moderator_plural_label_name' ), 'bp-group-check-in' );
				} elseif ( groups_is_user_member( $user_id, $group_id ) ) {
					esc_html_e( get_group_role_label( $group_id, 'member_plural_label_name' ), 'bp-group-check-in' );
				}
				?>
...
function bp_get_group_member_avatar( $args = '' ) {
	...
	$r = bp_parse_args(
		...
		array(
			...
			'alt'     => sprintf( __( 'Profile photo of %s', 'bp-group-check-in' ), $members_template->member->display_name ),
		)
	);
...
function bp_get_group_member_joined_since( $args = array() ) {
	 ...
	 */
	return apply_filters( 'bp_get_group_member_joined_since', bp_core_get_last_activity( $members_template->member->date_modified, __( 'joined %s', 'bp-group-check-in' ) ) );
}
...
function bp_group_member_pagination() {
	...
	wp_nonce_field( 'bgci_bp_habits_member_list', '_member_pag_nonce' );
}
...
function bp_get_group_member_pagination_count() {
	...
	$message = sprintf( _n( 'Viewing 1 member', 'Viewing %1$s - %2$s of %3$s members', $members_template->total_member_count, 'bp-group-check-in' ), $from_num, $to_num, $total );
...
function bp_group_member_admin_pagination() {
	...
	wp_nonce_field( 'bgci_bp_habits_member_admin_list', '_member_admin_pag_nonce' );
}
 ...
 * @since BuddyPress 2.4.0
 */
function bgci_bp_habits_front_template_part() {
	$located = bgci_bp_habits_get_front_template();
	...
	if ( false !== $located ) {
		...
		bgci_bp_habits_members_template_part();
	}
 ...
 *
 * @param  bgci_bp_habits_Group|null $group Optional. Falls back to current group if not passed.
 * @return string|bool                 Path to front template on success; boolean false on failure.
 */
function bgci_bp_habits_get_front_template( $group = null ) {
	if ( ! is_a( $group, 'bgci_bp_habits_Group' ) ) {
		$group = groups_get_current_group();
	}
	...
	if ( bgci_bp_habits_get_group_types() ) {
		$group_type = bgci_bp_habits_get_group_type( $group->id );
		if ( ! $group_type ) {
			$group_type = 'none';
		}
	 ...
	 * @param object $group          Group object.
	 */
	return bp_locate_template( apply_filters( 'bgci_bp_habits_get_front_template', $template_names, $group ), false, true );
}
 ...
 * @since BuddyPress 2.0.0
 */
function bgci_bp_habits_members_template_part() {
	?>
	<div class="item-list-tabs" id="subnav" aria-label="<?php esc_attr_e( 'Group secondary navigation', 'bp-group-check-in' ); ?>" role="navigation">
		<ul>
			<li class="groups-members-search" role="search">
				<?php bp_directory_members_search_form(); ?>
			</li>

			<?php bgci_bp_habits_members_filter(); ?>
			<?php

			/**
			 * Fires at the end of the group members search unordered list.
			 *
			 * Part of bgci_bp_habits_members_template_part().
			 *
			 * @since BuddyPress 1.5.0
			 */
 ...
 * @since BuddyPress 2.0.0
 */
function bgci_bp_habits_members_filter() {
	?>
	<li id="group_members-order-select" class="last filter">
		<label for="group_members-order-by"><?php _e( 'Order By:', 'bp-group-check-in' ); ?></label>
		<select id="group_members-order-by">
			<option value="last_joined"><?php _e( 'Newest', 'bp-group-check-in' ); ?></option>
			<option value="first_joined"><?php _e( 'Oldest', 'bp-group-check-in' ); ?></option>

			<?php if ( bp_is_active( 'activity' ) ) : ?>
				<option value="group_activity"><?php _e( 'Group Activity', 'bp-group-check-in' ); ?></option>
			<?php endif; ?>

			<option value="alphabetical"><?php _e( 'Alphabetical', 'bp-group-check-in' ); ?></option>
			 ...
			 * @since BuddyPress 2.0.0
			 */
			do_action( 'bgci_bp_habits_members_order_options' );
			?>
...
function bp_group_creation_tabs() {
	...
	foreach ( (array) $bp->groups->group_creation_steps as $slug => $step ) {
		...
		if ( bp_get_groups_current_create_step() == $slug ) :
			...
			if ( $is_enabled ) :
				?>
			<a href="<?php bgci_bp_habits_directory_permalink(); ?>create/step/<?php echo $slug; ?>/">
															<?php
else :
	?>
...
function bp_get_new_group_avatar( $args = '' ) {
	...
	$r = bp_parse_args(
		...
		array(
			...
			'alt'    => __( 'Group photo', 'bp-group-check-in' ),
		),
 ...
 * @since BuddyPress 1.6.0
 */
function bgci_bp_habits_current_create_step() {
...
function bp_directory_groups_search_form() {
	...
	$search_form_html = '<form action="" method="get" id="search-groups-form">
		...
		<input type="submit" id="groups_search_submit" name="groups_search_submit" value="' . __( 'Search', 'bp-group-check-in' ) . '" />
	</form>';
 ...
 * @todo Deprecate?
 */
function bgci_bp_habits_header_tabs() {
	...
	if ( ! bp_action_variable( 0 ) || bp_is_action_variable( 'recently-active', 0 ) ) :
		?>
        class="current"<?php endif; ?>><a href="<?php echo trailingslashit( $user_groups . '/my-groups/recently-active' ); ?>"><?php _e( 'Recently Active', 'bp-group-check-in' ); ?></a></li>
	<li
	<?php
	if ( bp_is_action_variable( 'recently-joined', 0 ) ) :
		?>
		class="current"<?php endif; ?>><a href="<?php echo trailingslashit( $user_groups . '/my-groups/recently-joined' ); ?>"><?php _e( 'Recently Joined', 'bp-group-check-in' ); ?></a></li>
	<li
	<?php
	if ( bp_is_action_variable( 'most-popular', 0 ) ) :
		?>
		class="current"<?php endif; ?>><a href="<?php echo trailingslashit( $user_groups . '/my-groups/most-popular' ); ?>"><?php _e( 'Most Popular', 'bp-group-check-in' ); ?></a></li>
	<li
	<?php
	if ( bp_is_action_variable( 'admin-of', 0 ) ) :
		?>
		class="current"<?php endif; ?>><a href="<?php echo trailingslashit( $user_groups . '/my-groups/admin-of' ); ?>"><?php _e( 'Organizer Of', 'bp-group-check-in' ); ?></a></li>
	<li
	<?php
	if ( bp_is_action_variable( 'mod-of', 0 ) ) :
		?>
		class="current"<?php endif; ?>><a href="<?php echo trailingslashit( $user_groups . '/my-groups/mod-of' ); ?>"><?php _e( 'Moderator Of', 'bp-group-check-in' ); ?></a></li>
	<li
	<?php
	if ( bp_is_action_variable( 'alphabetically' ) ) :
		?>
		class="current"<?php endif; ?>><a href="<?php echo trailingslashit( $user_groups . '/my-groups/alphabetically' ); ?>"><?php _e( 'Alphabetically', 'bp-group-check-in' ); ?></a></li>
 ...
 * @todo Deprecate?
 */
function bgci_bp_habits_filter_title() {
	$current_filter = bp_action_variable( 0 );

	switch ( $current_filter ) {
		case 'recently-active':
		default:
			_e( 'Recently Active', 'bp-group-check-in' );
			break;
		case 'recently-joined':
			_e( 'Recently Joined', 'bp-group-check-in' );
			break;
		case 'most-popular':
			_e( 'Most Popular', 'bp-group-check-in' );
			break;
		case 'admin-of':
			_e( 'Administrator Of', 'bp-group-check-in' );
			break;
		case 'mod-of':
			_e( 'Moderator Of', 'bp-group-check-in' );
			break;
		case 'alphabetically':
			_e( 'Alphabetically', 'bp-group-check-in' );
			break;
	}
	do_action( 'bgci_bp_habits_filter_title' );
}
...
function bp_get_current_group_directory_type_message() {
	$type_object = bgci_bp_habits_get_group_type_object( bp_get_current_group_directory_type() );

	$message = sprintf( __( 'Viewing all groups that are %s', 'bp-group-check-in' ), '<strong>' . $type_object->labels['name'] . '</strong>' );
...
function bp_get_group_current_avatar( $type = 'thumb' ) {
	...
	$group_avatar = bp_core_fetch_avatar(
		array(
			...
			'type'       => $type,
			'avatar_dir' => 'group-avatars',
			'alt'        => __( 'Group avatar', 'bp-group-check-in' ),
			'class'      => 'avatar',
		)
	);
...
function bp_group_has_membership_requests( $args = '' ) {
	...
	$requests_template = new bgci_bp_habits_Membership_Requests_Template( $r );
	 ...
	 * @param bgci_bp_habits_Membership_Requests_Template $requests_template Object holding the requests query results.
	 */
...
function bp_group_request_time_since_requested() {
	 ...
	 */
	echo apply_filters( 'bp_group_request_time_since_requested', sprintf( __( 'requested %s', 'bp-group-check-in' ), bp_core_time_since( $requests_template->request->date_modified ) ) );
}
...
function bp_get_group_requests_pagination_count() {
	...
	$total     = bp_core_number_format( $requests_template->total_request_count );
		...
		$message = sprintf( _n( 'Viewing 1 request', 'Viewing %1$s - %2$s of %3$s requests', $requests_template->total_request_count, 'bp-group-check-in' ), $from_num, $to_num, $total );
...
function bp_group_has_invites( $args = '' ) {
	...
	// Set the global (for use in bgci_bp_habits_Invite_Template::the_invite()).
	if ( empty( $group_id ) ) {
		$group_id = $r['group_id'];
	}
	...
	$invites_template = new bgci_bp_habits_Invite_Template( $r );
	 ...
	 *
	 * @param bool                      $value            Whether there are requests to display.
	 * @param bgci_bp_habits_Invite_Template $invites_template Object holding the invites query results.
	 */
...
function bp_get_group_invite_pagination_count() {
	...
	$message = sprintf( _n( 'Viewing 1 invitation', 'Viewing %1$s - %2$s of %3$s invitations', $invites_template->total_invite_count, 'bp-group-check-in' ), $from_num, $to_num, $total );
 ...
 * @since BuddyPress 1.5.0
 */
function bgci_bp_habits_activity_feed() {
	...
	<link rel="alternate" type="application/rss+xml" title="<?php bloginfo( 'name' ); ?> | <?php bp_current_group_name(); ?> | <?php _e( 'Group Activity RSS Feed', 'bp-group-check-in' ); ?>" href="<?php bp_group_activity_feed_link(); ?>" />

	<?php
}
add_action( 'bp_head', 'bgci_bp_habits_activity_feed' );
 ...
 * @return string|null
 */
function bgci_bp_habits_action_link( $action = '', $query_args = '', $nonce = false ) {
 ...
 */
function bgci_bp_habits_profile_stats( $args = '' ) {
	echo bgci_bp_habits_get_profile_stats( $args );
}
add_action( 'bp_members_admin_user_stats', 'bgci_bp_habits_profile_stats', 8, 1 );
 ...
 */
function bgci_bp_habits_get_profile_stats( $args = '' ) {
	...
	if ( empty( $r['output'] ) ) {
		...
		if ( ! empty( $r['user_id'] ) ) {
			...
			$r['output'] = $r['before'] . sprintf( _n( '%s group', '%s groups', $r['groups'], 'bp-group-check-in' ), '<strong>' . $r['groups'] . '</strong>' ) . $r['after'];
		}
	}
	 ...
	 */
	return apply_filters( 'bgci_bp_habits_get_profile_stats', $r['output'], $r );
}
...
function bp_get_article_prefix( $string ) {
	...
	if ( ! in_array( get_locale(), $supported_languages ) ) {
		return apply_filters( 'bp_get_article_prefix', __( 'a', 'bp-group-check-in' ) . ' ' . $string, $string );
	}

	$vowel_array = array( 'a', 'e', 'i', 'o', 'u' );  // array of vowel
	$prefix      = in_array( strtolower( substr( $string, 0, 1 ) ), $vowel_array ) ? __( 'an', 'bp-group-check-in' ) : __( 'a', 'bp-group-check-in' );
