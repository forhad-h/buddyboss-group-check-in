...
add_filter( 'bgci_bp_habits_get_directory_template', 'bp_add_template_locations' );
...
add_filter( 'bp_get_group_description', 'bgci_bp_habits_filter_kses', 1 );
add_filter( 'bp_get_group_description_excerpt', 'wp_filter_kses', 1 );
add_filter( 'groups_group_name_before_save', 'wp_filter_kses', 1 );
add_filter( 'groups_group_description_before_save', 'bgci_bp_habits_filter_kses', 1 );
...
add_filter( 'bp_activity_at_name_do_notifications', 'bgci_bp_habits_disable_at_mention_notification_for_non_public_groups', 10, 4 );
add_filter( 'bbp_forums_at_name_do_notifications', 'bgci_bp_habits_disable_at_mention_forums_notification_for_non_public_groups', 10, 4 );

// Default group avatar.
add_filter( 'bp_core_avatar_default', 'bgci_bp_habits_default_avatar', 10, 3 );
add_filter( 'bp_core_avatar_default_thumb', 'bgci_bp_habits_default_avatar', 10, 3 );

// Exclude Forums if group type hide.
add_filter( 'bbp_after_has_forums_parse_args', 'bgci_bp_habits_exclude_forums_by_group_type_args' );
// Exclude Forums if group type hide.
add_filter( 'bbp_after_has_topics_parse_args', 'bgci_bp_habits_exclude_forums_topics_by_group_type_args' );
// media scope filter.
add_filter( 'bp_media_set_groups_scope_args', 'bgci_bp_habits_filter_media_scope', 10, 2 );
add_filter( 'bp_document_set_document_groups_scope_args', 'bgci_bp_habits_filter_document_scope', 10, 2 );
add_filter( 'bp_document_set_folder_groups_scope_args', 'bgci_bp_habits_filter_folder_scope', 10, 2 );
 ...
 * @return string
 */
function bgci_bp_habits_filter_kses( $content = '' ) {
	 ...
	 */
	$tags = apply_filters( 'bgci_bp_habits_filter_kses', $allowed_tags );
 ...
 */
function bgci_bp_habits_maybe_load_mentions_scripts( $load_mentions, $mentions_enabled ) {
	if ( ! $mentions_enabled ) {
...
add_filter( 'bp_activity_maybe_load_mentions_scripts', 'bgci_bp_habits_maybe_load_mentions_scripts', 10, 2 );
 ...
 * @return bool
 */
function bgci_bp_habits_disable_at_mention_notification_for_non_public_groups( $send, $usernames, $user_id, BP_Activity_Activity $activity ) {
 ...
 */
function bgci_bp_habits_disable_at_mention_forums_notification_for_non_public_groups( $send, $usernames, $user_id, $forum_id ) {
	// Skip the check for administrators, who can get notifications from non-public groups.
	if ( bp_user_can( $user_id, 'bp_moderate' ) ) {
		return $send;
	}
 ...
 * @return string
 */
function bgci_bp_habits_default_avatar( $avatar, $params ) {
 ...
 * @return bool
 */
function bgci_bp_habits_user_can_filter( $retval, $user_id, $capability, $site_id, $args ) {
	...
	switch ( $capability ) {
		...
		case 'groups_access_group':
			...
			// If the check is for the logged-in user, use the bgci_bp_habits_Group property.
...
add_filter( 'bp_user_can', 'bgci_bp_habits_user_can_filter', 10, 5 );
 ...
 * @return bool
 */
function bgci_bp_habits_allow_mods_to_delete_activity( $can_delete, $activity ) {
...
add_filter( 'bp_activity_user_can_delete', 'bgci_bp_habits_allow_mods_to_delete_activity', 10, 2 );
 ...
 */
function bgci_bp_habits_exclude_forums_by_group_type_args( $args_forum ) {
	if ( bbp_is_forum_archive() || bbp_is_topic_archive() ) {
		$exclude_forum_ids = array();
		// Check group type enabled
		if ( true === bp_disable_group_type_creation() ) {
			// Get excluded group ids.
			$exclude_group_ids = array_unique( bgci_bp_habits_get_excluded_group_ids_by_type() );
 ...
 */
function bgci_bp_habits_exclude_forums_topics_by_group_type_args( $args_topic ) {
 ...
 * @return array
 */
function bgci_bp_habits_filter_media_scope( $retval = array(), $filter = array() ) {
 ...
 * @return array
 */
function bgci_bp_habits_filter_document_scope( $retval = array(), $filter = array() ) {
...
function bgci_bp_habits_filter_folder_scope( $retval = array(), $filter = array() ) {
