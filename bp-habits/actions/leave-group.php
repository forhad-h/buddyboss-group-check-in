...
function groups_action_leave_group() {
	...
	if ( groups_is_user_member( bp_loggedin_user_id(), bp_get_current_group_id() ) ) {
		...
		if ( 1 == count( $group_admins ) && $group_admins[0]->user_id == bp_loggedin_user_id() ) {
			...
			bp_core_add_message( __( 'There was an error leaving the group.', 'bp-group-check-in' ), 'error' );
		} else {
			bp_core_add_message( __( 'You left the group.', 'bp-group-check-in' ) );
		}
...
add_action( 'bgci_bp_habits_member_after_delete', 'groups_action_clean_up_invites_requests', 10, 2 );
