 ...
 */
function bgci_bp_habits_group_access_protection() {
	if ( ! bp_is_group() ) {
		return;
	}
	...
	if ( ! $user_has_access && $is_visible ) {
		...
		if ( bp_is_current_action( 'request-membership' ) && ! is_user_logged_in() ) {
			...
			$no_access_args = array(
				'message'  => __( 'You do not have access to this group.', 'bp-group-check-in' ),
	...
	if ( bp_is_current_action( 'admin' ) && ! bp_is_item_admin() ) {
		...
		$no_access_args  = array(
			'message'  => __( 'You are not an organizer or moderator of this group.', 'bp-group-check-in' ),
...
add_action( 'bp_actions', 'bgci_bp_habits_group_access_protection' );
