...
function groups_action_group_feed() {
	...
	buddypress()->activity->feed = new BP_Activity_Feed(
		array(
			...
			'title'         => sprintf( __( '%1$s | %2$s | Activity', 'bp-group-check-in' ), bp_get_site_name(), bp_get_current_group_name() ),

			'link'          => bp_get_group_permalink( $group ),
			'description'   => sprintf( __( 'Activity feed for the group, %s.', 'bp-group-check-in' ), bp_get_current_group_name() ),
			'activity_args' => array(
