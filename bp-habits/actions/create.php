...
function groups_action_create_group() {
	...
	if ( ! bp_user_can_create_groups() ) {
		bp_core_add_message( __( 'Sorry, you are not allowed to create groups.', 'bp-group-check-in' ), 'error' );
	...
	if ( bp_get_groups_current_create_step() && empty( $bp->groups->group_creation_steps[ bp_get_groups_current_create_step() ] ) ) {
		bp_core_add_message( __( 'There was an error saving group details. Please try again.', 'bp-group-check-in' ), 'error' );
	...
	if ( bp_get_new_group_id() ) {
		...
		if ( ! bp_is_group_creator( $bp->groups->current_group, bp_loggedin_user_id() ) ) {
			bp_core_add_message( __( 'Only the group organizer may continue editing this group.', 'bp-group-check-in' ), 'error' );
			bp_core_redirect( trailingslashit( bp_get_groups_directory_permalink() . 'create' ) );
		}
	}
	...
	if ( isset( $_POST['save'] ) ) {
		...
		if ( 'group-details' == bp_get_groups_current_create_step() ) {
			if ( empty( $_POST['group-name'] ) || ! strlen( trim( $_POST['group-name'] ) ) ) {
				bp_core_add_message( __( 'Please fill in all of the required fields', 'bp-group-check-in' ), 'error' );
		...
		if ( 'group-settings' == bp_get_groups_current_create_step() ) {
			...
			if ( ! empty( $_POST['group-types'] ) ) {
				bgci_bp_habits_set_group_type( $bp->groups->new_group_id, $_POST['group-types'] );
			}
	...
	if ( 'group-invites' === bp_get_groups_current_create_step() && ! empty( $_REQUEST['user_id'] ) && is_numeric( $_REQUEST['user_id'] ) ) {
		...
		$message = __( 'Invite successfully withdrawn', 'bp-group-check-in' );
		$error   = false;

		if ( ! groups_uninvite_user( (int) $_REQUEST['user_id'], $bp->groups->new_group_id ) ) {
			$message = __( 'There was an error withdrawing the invite', 'bp-group-check-in' );
			$error   = 'error';
		}
	...
	if ( 'group-avatar' == bp_get_groups_current_create_step() && isset( $_POST['upload'] ) ) {
		...
		if ( isset( $_POST['avatar-crop-submit'] ) && isset( $_POST['upload'] ) ) {
			...
			if ( ! bp_core_avatar_handle_crop( $args ) ) {
				bp_core_add_message( __( 'There was an error with the group profile photo, please try again.', 'bp-group-check-in' ), 'error' );
			} else {
				/**
				...
				bp_core_add_message( __( 'The group profile photo was uploaded successfully.', 'bp-group-check-in' ) );
			}
		}
	}
