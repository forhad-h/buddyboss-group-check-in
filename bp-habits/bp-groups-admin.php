bgci_bp_habits_admin_screen_options<?php
/**
...
if ( is_admin() && ! empty( $_REQUEST['page'] ) && 'bp-groups' == $_REQUEST['page'] ) {
	add_filter( 'set-screen-option', 'bgci_bp_habits_admin_screen_options', 10, 3 );
}
 ...
 * @since BuddyPress 1.7.0
 */
function bgci_bp_habits_add_admin_menu() {
	...
	if ( true === bp_disable_group_type_creation() ) {
		...
		$hooks[] = add_submenu_page(
			'buddyboss-platform',
			__( 'Groups', 'bp-group-check-in' ),
			__( 'Groups', 'bp-group-check-in' ),
			'bp_moderate',
			'bp-groups',
			'bgci_bp_habits_admin'
		);
	...
	foreach ( $hooks as $hook ) {
		...
		add_action( "load-$hook", 'bgci_bp_habits_admin_load' );
	}
}
add_action( bp_core_admin_hook(), 'bgci_bp_habits_add_admin_menu', 60 );
 ...
 */
function bgci_bp_habits_admin_menu_order( $custom_menus = array() ) {
	array_push( $custom_menus, 'bp-groups' );
	return $custom_menus;
}
add_filter( 'bp_admin_menu_order', 'bgci_bp_habits_admin_menu_order' );
 ...
 * @since BuddyPress 1.7.0
 *
 * @global bgci_bp_habits_List_Table $bgci_bp_habits_list_table Groups screen list table.
 */
function bgci_bp_habits_admin_load() {
	global $bgci_bp_habits_list_table;
	 ...
	 */
	do_action( 'bgci_bp_habits_admin_load', $doaction );
	...
	if ( 'do_delete' == $doaction && ! empty( $_GET['gid'] ) ) {
		...
		get_current_screen()->add_help_tab(
			array(
				...
				'title'   => __( 'Overview', 'bp-group-check-in' ),
				'content' =>
					'<p>' . __( 'This page is a convenient way to edit the details associated with one of your groups.', 'bp-group-check-in' ) . '</p>' .
					'<p>' . __( 'The Name and Description box is fixed in place, but you can reposition all the other boxes using drag and drop, and can minimize or expand them by clicking the title bar of each box. Use the Screen Options tab to hide or unhide, or to choose a 1- or 2-column layout for this screen.', 'bp-group-check-in' ) . '</p>',
			)
		);

		// Help panel - sidebar links.
		get_current_screen()->set_help_sidebar(
			'<p><strong>' . __( 'For more information:', 'bp-group-check-in' ) . '</strong></p>' .
			'<p>' . __( '<a href="https://www.buddyboss.com/resources/">Documentation</a>', 'bp-group-check-in' ) . '</p>'
		);

		// Register metaboxes for the edit screen.
		add_meta_box( 'submitdiv', __( 'Save', 'bp-group-check-in' ), 'bgci_bp_habits_admin_edit_metabox_status', get_current_screen()->id, 'side', 'high' );
		add_meta_box( 'bp_group_settings', __( 'Settings', 'bp-group-check-in' ), 'bgci_bp_habits_admin_edit_metabox_settings', get_current_screen()->id, 'side', 'core' );
		add_meta_box( 'bp_group_add_members', __( 'Add New Members', 'bp-group-check-in' ), 'bgci_bp_habits_admin_edit_metabox_add_new_members', get_current_screen()->id, 'normal', 'core' );
		add_meta_box( 'bp_group_members', __( 'Manage Members', 'bp-group-check-in' ), 'bgci_bp_habits_admin_edit_metabox_members', get_current_screen()->id, 'normal', 'core' );

		// Group Type metabox. Only added if group types have been registered.
		$group_types = bgci_bp_habits_get_group_types();
		if ( ! empty( $group_types ) ) {
			add_meta_box(
				'bgci_bp_habits_admin_group_type',
				__( 'Group Type', 'bp-group-check-in' ),
				'bgci_bp_habits_admin_edit_metabox_group_type',
		...
		if ( bp_enable_group_hierarchies() ) {
			add_meta_box(
				'bgci_bp_habits_admin_group_parent',
				__( 'Group Parent', 'bp-group-check-in' ),
				'bgci_bp_habits_admin_edit_metabox_group_parent',
				get_current_screen()->id,
				'side',
				'core'
			);
		 ...
		 * @since BuddyPress 1.7.0
		 */
		do_action( 'bgci_bp_habits_admin_meta_boxes' );
		...
		$bgci_bp_habits_list_table = new bgci_bp_habits_List_Table();

		// The per_page screen option.
		add_screen_option( 'per_page', array( 'label' => __( 'Groups', 'bp-group-check-in' ) ) );
		...
		get_current_screen()->add_help_tab(
			array(
				...
				'title'   => __( 'Overview', 'bp-group-check-in' ),
				'content' =>
					'<p>' . __( 'You can manage groups much like you can manage comments and other content. This screen is customizable in the same ways as other management screens, and you can act on groups by using the on-hover action links or the Bulk Actions.', 'bp-group-check-in' ) . '</p>',
			)
		);
		...
		get_current_screen()->add_help_tab(
			array(
				...
				'title'   => __( 'Group Actions', 'bp-group-check-in' ),
				'content' =>
					'<p>' . __( 'Clicking "Visit" will take you to the group\'s public page. Use this link to see what the group looks like on the front end of your site.', 'bp-group-check-in' ) . '</p>' .
					'<p>' . __( 'Clicking "Edit" will take you to a Dashboard panel where you can manage various details about the group, such as its name and description, its members, and other settings.', 'bp-group-check-in' ) . '</p>' .
					'<p>' . __( 'If you click "Delete" under a specific group, or select a number of groups and then choose Delete from the Bulk Actions menu, you will be led to a page where you\'ll be asked to confirm the permanent deletion of the group(s).', 'bp-group-check-in' ) . '</p>',
			)
		);
		...
		get_current_screen()->set_screen_reader_content(
			array(
				...
				'heading_pagination' => __( 'Groups list navigation', 'bp-group-check-in' ),
			)
		);
	}
	...
	wp_enqueue_script( 'bgci_bp_habits_admin_js', $bp->plugin_url . "bp-groups/admin/js/admin{$min}.js", array( 'jquery', 'wp-ajax-response', 'jquery-ui-autocomplete' ), bp_get_version(), true );
	wp_localize_script(
		'bgci_bp_habits_admin_js',
		'BP_Group_Admin',
		array(
			'add_member_placeholder' => __( 'Start typing a username to add a new member.', 'bp-group-check-in' ),
			'warn_on_leave'          => __( 'If you leave this page, you will lose any unsaved changes you have made to the group.', 'bp-group-check-in' ),
		)
	);
	wp_enqueue_style( 'bgci_bp_habits_admin_css', $bp->plugin_url . "bp-groups/admin/css/admin{$min}.css", array(), bp_get_version() );

	wp_style_add_data( 'bgci_bp_habits_admin_css', 'rtl', true );
	if ( $min ) {
		wp_style_add_data( 'bgci_bp_habits_admin_css', 'suffix', $min );
	}
 ...
 */
function bgci_bgci_bp_habits_admin_screen_options( $value, $option, $new_value ) {
	if ( 'buddyboss_page_bgci_bp_habits_per_page' != $option && 'buddyboss_page_bgci_bp_habits_network_per_page' != $option ) {
		return $value;
	}
 ...
 * @since BuddyPress 1.7.0
 */
function bgci_bp_habits_admin() {

	// Added navigation tab on top.
	if ( bp_core_get_groups_admin_tabs() ) { ?>
		<div class="wrap">
			<h2 class="nav-tab-wrapper"><?php bp_core_admin_groups_tabs( __( 'All Groups', 'bp-group-check-in' ) ); ?></h2>
		</div>
		<?php
	}
 ...
 * @since BuddyPress 1.7.0
 */
function bgci_bp_habits_admin_edit() {
	...
	if ( ! empty( $_REQUEST['no_admins'] ) || ! empty( $_REQUEST['error'] ) || ! empty( $_REQUEST['updated'] ) || ! empty( $_REQUEST['error_new'] ) || ! empty( $_REQUEST['success_new'] ) || ! empty( $_REQUEST['error_modified'] ) || ! empty( $_REQUEST['success_modified'] ) ) {
		...
		if ( ! empty( $no_admins ) ) {
			$messages[] = __( 'This group must have at least one organizer.', 'bp-group-check-in' );
		}

		if ( ! empty( $errors ) ) {
			if ( $errors < 0 ) {
				$messages[] = __( 'Group name, slug, and description are all required fields.', 'bp-group-check-in' );
			} else {
				$messages[] = __( 'An error occurred when trying to update your group details.', 'bp-group-check-in' );
			}
		} elseif ( ! empty( $updated ) ) {
			$messages[] = __( 'The group has been updated successfully.', 'bp-group-check-in' );
		}

		if ( ! empty( $error_new ) ) {
			$messages[] = sprintf( __( 'The following users could not be added to the group: %s', 'bp-group-check-in' ), '<em>' . esc_html( implode( ', ', $error_new ) ) . '</em>' );
		}

		if ( ! empty( $success_new ) ) {
			$messages[] = sprintf( __( 'The following users were successfully added to the group: %s', 'bp-group-check-in' ), '<em>' . esc_html( implode( ', ', $success_new ) ) . '</em>' );
		}

		if ( ! empty( $error_modified ) ) {
			$error_modified = bgci_bp_habits_admin_get_usernames_from_ids( $error_modified );
			$messages[]     = sprintf( __( 'An error occurred when trying to modify the following members: %s', 'bp-group-check-in' ), '<em>' . esc_html( implode( ', ', $error_modified ) ) . '</em>' );
		}

		if ( ! empty( $success_modified ) ) {
			$success_modified = bgci_bp_habits_admin_get_usernames_from_ids( $success_modified );
			$messages[]       = sprintf( __( 'The following members were successfully modified: %s', 'bp-group-check-in' ), '<em>' . esc_html( implode( ', ', $success_modified ) ) . '</em>' );
		}
	}
	 ...
	 * @since BuddyPress 1.7.0
	 *
	 * @param bgci_bp_habits_Group $this Instance of the current group being edited. Passed by reference.
	 */
	do_action_ref_array( 'bgci_bp_habits_admin_edit', array( &$group ) );
	?>

	<div class="wrap">
		<?php if ( version_compare( $GLOBALS['wp_version'], '4.8', '>=' ) ) : ?>

			<h1 class="wp-heading-inline"><?php _e( 'Edit Group', 'bp-group-check-in' ); ?></h1>
			...
			<?php if ( is_user_logged_in() && bp_user_can_create_groups() ) : ?>
				...
				?>
				"><?php _e( 'New Group', 'bp-group-check-in' ); ?></a>
			<?php endif; ?>
		...
		<?php else : ?>
			...
			<h1><?php _e( 'Edit Group', 'bp-group-check-in' ); ?>
				...
				<div id="poststuff">
					...
					<div id="post-body" class="metabox-holder columns-<?php echo 1 == get_current_screen()->get_columns() ? '1' : '2'; ?>">
						<div id="post-body-content">
							<div id="postdiv">
								<div id="bgci_bp_habits_name" class="postbox">
									<h2><?php _e( 'Name and Description', 'bp-group-check-in' ); ?></h2>
									<div class="inside">
										<label for="bp-groups-name" class="screen-reader-text">
										<?php
											/* translators: accessibility text */
											_e( 'Group Name', 'bp-group-check-in' );
										?>
										</label>
										...
										<div id="bp-groups-permalink-box">
											<strong><?php esc_html_e( 'Permalink:', 'bp-group-check-in' ); ?></strong>
											<span id="bp-groups-permalink">
												<?php bgci_bp_habits_directory_permalink(); ?> <input type="text" id="bp-groups-slug" name="bp-groups-slug" value="<?php bp_group_slug( $group ); ?>" autocomplete="off"> /
											</span>
											<a href="<?php echo bp_group_permalink( $group ); ?>" class="button button-small" id="bp-groups-visit-group"><?php esc_html_e( 'Visit Group', 'bp-group-check-in' ); ?></a>
										</div>

										<label for="bp-groups-description" class="screen-reader-text">
										<?php
											/* translators: accessibility text */
											_e( 'Group Description', 'bp-group-check-in' );
										?>
										</label>
										<?php
										wp_editor(
		...
		<?php else : ?>
			...
			<?php
				printf(
					...
					__( 'No group found with this ID.', 'bp-group-check-in' ),
					esc_url( bp_get_admin_url( 'admin.php?page=bp-groups' ) ),
					__( 'Go back and try again.', 'bp-group-check-in' )
				);
			?>
			</p>
 ...
 * @since BuddyBoss 1.0.0
 */
function bgci_bp_habits_admin_create() {
	...
	<div class="wrap">
		<?php if ( version_compare( $GLOBALS['wp_version'], '4.8', '>=' ) ) : ?>
			...
			<h1 class="wp-heading-inline"><?php _e( 'Create New Group', 'bp-group-check-in' ); ?></h1>

			<hr class="wp-header-end">

		<?php else : ?>

			<h1><?php _e( 'Create New Group', 'bp-group-check-in' ); ?> </h1>
				...
				<div id="post-body" class="metabox-holder columns-<?php echo 1 == get_current_screen()->get_columns() ? '1' : '2'; ?>">
					...
					<div id="postbox-container-1" class="postbox-container">
						<div id="submitdiv" class="postbox">
							<h2 class="hndle ui-sortable-handle"><span><?php esc_html_e( 'Publish', 'bp-group-check-in' ); ?></span></h2>
							<div class="inside">
								<div id="submitcomment" class="submitbox">
									<div id="major-publishing-actions">
										<div id="publishing-action">
											<input type="submit" name="save" id="save" class="button button-primary" value="<?php esc_attr_e( 'Publish', 'bp-group-check-in' ); ?>">			</div>
										<div class="clear"></div>
 ...
 * @since BuddyPress 1.7.0
 */
function bgci_bp_habits_admin_delete() {
	...
	<div class="wrap">
		<h1><?php _e( 'Delete Groups', 'bp-group-check-in' ); ?></h1>
		<p><?php _e( 'You are about to delete the following groups:', 'bp-group-check-in' ); ?></p>
			...
			<li>
				...
				<?php if ( bp_is_active( 'forums' ) && bbp_get_group_forum_ids( $group->id ) ) : ?>
					<label for="delete-group-forum-<?php echo $group->id; ?>" class="delete-forum-label">
						...
						<?php esc_html_e( 'Permanently delete the group discussion forum', 'bp-group-check-in' ); ?>
					</label>
				<?php endif; ?>
			</li>
		<?php endforeach; ?>
		...
		<p><strong><?php _e( 'This action cannot be undone.', 'bp-group-check-in' ); ?></strong></p>
		...
		<a
			...
			"
		>
			<?php _e( 'Delete Permanently', 'bp-group-check-in' ); ?>
		</a>
		<a class="button" href="<?php echo esc_attr( $base_url ); ?>"><?php _e( 'Cancel', 'bp-group-check-in' ); ?></a>
	</div>
 ...
 * @since BuddyPress 1.7.0
 *
 * @global bgci_bp_habits_List_Table $bgci_bp_habits_list_table Group screen list table.
 * @global string $plugin_page Currently viewed plugin page.
 */
function bgci_bp_habits_admin_index() {
	global $bgci_bp_habits_list_table, $plugin_page;
	...
	if ( ! empty( $_REQUEST['deleted'] ) ) {
		...
		if ( $deleted > 0 ) {
			$messages[] = sprintf( _n( '%s group has been permanently deleted.', '%s groups have been permanently deleted.', $deleted, 'bp-group-check-in' ), number_format_i18n( $deleted ) );
		}
	}

	// Prepare the group items for display.
	$bgci_bp_habits_list_table->prepare_items();
	 ...
	 */
	do_action( 'bgci_bp_habits_admin_index', $messages );
	?>

	<div class="wrap">
		<?php if ( version_compare( $GLOBALS['wp_version'], '4.8', '>=' ) ) : ?>

			<h1 class="wp-heading-inline"><?php _e( 'Groups', 'bp-group-check-in' ); ?></h1>
			...
			<?php if ( ! empty( $_REQUEST['s'] ) ) : ?>
				<span class="subtitle"><?php printf( __( 'Search results for "%s"', 'bp-group-check-in' ), wp_html_excerpt( esc_html( stripslashes( $_REQUEST['s'] ) ), 50 ) ); ?></span>
			<?php endif; ?>
		...
		<h1>
			<?php _e( 'Groups', 'bp-group-check-in' ); ?>
		...
		<?php $bgci_bp_habits_list_table->views(); ?>

		<form id="bp-groups-form" action="" method="get">
			<?php $bgci_bp_habits_list_table->search_box( __( 'Search all Groups', 'bp-group-check-in' ), 'bp-groups' ); ?>
			<input type="hidden" name="page" value="<?php echo esc_attr( $plugin_page ); ?>" />
			<?php $bgci_bp_habits_list_table->display(); ?>
		</form>
 ...
 */
function bgci_bp_habits_admin_edit_metabox_settings( $item ) {
	...
	<div class="bp-groups-settings-section" id="bp-groups-settings-section-status">
		<fieldset>
			<legend><?php _e( 'Group Privacy', 'bp-group-check-in' ); ?></legend>

			<label for="bp-group-status-public"><input type="radio" name="group-status" id="bp-group-status-public" value="public" <?php checked( $item->status, 'public' ); ?> /><?php _e( 'Public', 'bp-group-check-in' ); ?></label>
			<label for="bp-group-status-private"><input type="radio" name="group-status" id="bp-group-status-private" value="private" <?php checked( $item->status, 'private' ); ?> /><?php _e( 'Private', 'bp-group-check-in' ); ?></label>
			<label for="bp-group-status-hidden"><input type="radio" name="group-status" id="bp-group-status-hidden" value="hidden" <?php checked( $item->status, 'hidden' ); ?> /><?php _e( 'Hidden', 'bp-group-check-in' ); ?></label>
		</fieldset>
	</div>

	<div class="bp-groups-settings-section" id="bp-groups-settings-section-invite-status">
		<fieldset>
			<legend><?php _e( 'Who can invite others to join this group?', 'bp-group-check-in' ); ?></legend>

			<label for="bp-group-invite-status-members"><input type="radio" name="group-invite-status" id="bp-group-invite-status-members" value="members" <?php checked( $invite_status, 'members' ); ?> /><?php _e( 'All Members', 'bp-group-check-in' ); ?></label>
			<label for="bp-group-invite-status-mods"><input type="radio" name="group-invite-status" id="bp-group-invite-status-mods" value="mods" <?php checked( $invite_status, 'mods' ); ?> /><?php _e( 'Organizers and Moderators', 'bp-group-check-in' ); ?></label>
			<label for="bp-group-invite-status-admins"><input type="radio" name="group-invite-status" id="bp-group-invite-status-admins" value="admins" <?php checked( $invite_status, 'admins' ); ?> /><?php _e( 'Organizers', 'bp-group-check-in' ); ?></label>
		</fieldset>
	</div>

	<div class="bp-groups-settings-section" id="bp-groups-settings-section-activity-feed-status">
		<fieldset>
			<legend><?php _e( 'Who can post into this group?', 'bp-group-check-in' ); ?></legend>

			<label for="bp-group-activity-feed-status-members"><input type="radio" name="group-activity-feed-status" id="bp-group-activity-feed-status-members" value="members" <?php checked( $activity_feed_status, 'members' ); ?> /><?php _e( 'All group members', 'bp-group-check-in' ); ?></label>
			<label for="bp-group-activity-feed-status-mods"><input type="radio" name="group-activity-feed-status" id="bp-group-activity-feed-status-mods" value="mods" <?php checked( $activity_feed_status, 'mods' ); ?> /><?php _e( 'Organizers and Moderators only', 'bp-group-check-in' ); ?></label>
			<label for="bp-group-activity-feed-status-admins"><input type="radio" name="group-activity-feed-status" id="bp-group-activity-feed-status-admins" value="admins" <?php checked( $activity_feed_status, 'admins' ); ?> /><?php _e( 'Organizers only', 'bp-group-check-in' ); ?></label>
		</fieldset>
	</div>
	...
	<?php if ( bp_is_active( 'media' ) && bp_is_group_media_support_enabled() ) : ?>
		<div class="bp-groups-settings-section" id="bp-groups-settings-section-album-status">
			<fieldset>
				<legend><?php _e( 'Who can manage photos in this group?', 'bp-group-check-in' ); ?></legend>

				<label for="bp-group-media-status-members"><input type="radio" name="group-media-status" id="bp-group-media-status-members" value="members" <?php checked( $media_status, 'members' ); ?> /><?php _e( 'All Members', 'bp-group-check-in' ); ?></label>
				<label for="bp-group-media-status-mods"><input type="radio" name="group-media-status" id="bp-group-media-status-mods" value="mods" <?php checked( $media_status, 'mods' ); ?> /><?php _e( 'Organizers and Moderators', 'bp-group-check-in' ); ?></label>
				<label for="bp-group-media-status-admins"><input type="radio" name="group-media-status" id="bp-group-media-status-admins" value="admins" <?php checked( $media_status, 'admins' ); ?> /><?php _e( 'Organizers', 'bp-group-check-in' ); ?></label>
			</fieldset>
		</div>
	<?php endif; ?>
	...
	<?php if ( bp_is_active( 'media' ) && bp_is_group_albums_support_enabled() ) : ?>
		<div class="bp-groups-settings-section" id="bp-groups-settings-section-album-status">
			<fieldset>
				<legend><?php _e( 'Who can manage albums in this group?', 'bp-group-check-in' ); ?></legend>

				<label for="bp-group-album-status-members"><input type="radio" name="group-album-status" id="bp-group-album-status-members" value="members" <?php checked( $album_status, 'members' ); ?> /><?php _e( 'All Members', 'bp-group-check-in' ); ?></label>
				<label for="bp-group-album-status-mods"><input type="radio" name="group-album-status" id="bp-group-album-status-mods" value="mods" <?php checked( $album_status, 'mods' ); ?> /><?php _e( 'Organizers and Moderators', 'bp-group-check-in' ); ?></label>
				<label for="bp-group-album-status-admins"><input type="radio" name="group-album-status" id="bp-group-album-status-admins" value="admins" <?php checked( $album_status, 'admins' ); ?> /><?php _e( 'Organizers', 'bp-group-check-in' ); ?></label>
			</fieldset>
		</div>
	<?php endif; ?>
	...
	<?php if ( bp_is_active( 'media' ) && bp_is_group_document_support_enabled() ) : ?>
		<div class="bp-groups-settings-section" id="bp-groups-settings-section-document-status">
			<fieldset>
				<legend><?php _e( 'Who can manage documents in this group?', 'bp-group-check-in' ); ?></legend>

				<label for="bp-group-document-status-members"><input type="radio" name="group-document-status" id="bp-group-document-status-members" value="members" <?php checked( $document_status, 'members' ); ?> /><?php _e( 'All Members', 'bp-group-check-in' ); ?></label>
				<label for="bp-group-document-status-mods"><input type="radio" name="group-document-status" id="bp-group-document-status-mods" value="mods" <?php checked( $document_status, 'mods' ); ?> /><?php _e( 'Organizers and Moderators', 'bp-group-check-in' ); ?></label>
				<label for="bp-group-document-status-admins"><input type="radio" name="group-document-status" id="bp-group-document-status-admins" value="admins" <?php checked( $document_status, 'admins' ); ?> /><?php _e( 'Organizers', 'bp-group-check-in' ); ?></label>
			</fieldset>
		</div>
	<?php endif; ?>
	...
	<?php if ( bp_is_active( 'messages' ) && bp_disable_group_messages() ) : ?>
		<div class="bp-groups-settings-section" id="bp-groups-settings-section-message-status">
			<fieldset>
				<legend><?php _e( 'Who can manage group messages in this group?', 'bp-group-check-in' ); ?></legend>

				<label for="bp-group-message-status-members"><input type="radio" name="group-message-status" id="bp-group-message-status-members" value="members" <?php checked( $message_status, 'members' ); ?> /><?php _e( 'All Members', 'bp-group-check-in' ); ?></label>
				<label for="bp-group-message-status-mods"><input type="radio" name="group-message-status" id="bp-group-message-status-mods" value="mods" <?php checked( $message_status, 'mods' ); ?> /><?php _e( 'Organizers and Moderators', 'bp-group-check-in' ); ?></label>
				<label for="bp-group-message-status-admins"><input type="radio" name="group-message-status" id="bp-group-message-status-admins" value="admins" <?php checked( $message_status, 'admins' ); ?> /><?php _e( 'Organizers', 'bp-group-check-in' ); ?></label>
			</fieldset>
		</div>
	<?php endif; ?>
 ...
 * @param object $item Information about the current group.
 */
function bgci_bp_habits_admin_edit_metabox_group_parent( $item ) {
	...
	<div class="bp-groups-settings-section" id="bp-groups-settings-section-group-hierarchy">
		<select id="bp-groups-parent" name="bp-groups-parent" autocomplete="off">
			<option
				value="0" <?php selected( 0, $current_parent_group_id ); ?>><?php _e( 'Select Parent', 'bp-group-check-in' ); ?></option>
			<?php
 ...
 * @since BuddyPress 1.7.0
 *
 * @param bgci_bp_habits_Group $item The bgci_bp_habits_Group object for the current group.
 */
function bgci_bp_habits_admin_edit_metabox_add_new_members( $item ) {
	?>

	<label for="bp-groups-new-members" class="screen-reader-text">
	<?php
		/* translators: accessibility text */
		_e( 'Add new members', 'bp-group-check-in' );
	?>
	</label>
	<input name="bp-groups-new-members" type="text" id="bp-groups-new-members" class="bp-suggest-user" placeholder="<?php esc_attr_e( 'Enter a comma-separated list of user logins.', 'bp-group-check-in' ); ?>" />
 ...
 * @param bgci_bp_habits_Group $item The bgci_bp_habits_Group object for the current group.
 */
function bgci_bp_habits_admin_edit_metabox_members( $item ) {
	...
	foreach ( $members as $type => &$member_type_users ) {
		...
		$member_type_query = new BP_Group_Member_Query(
			array(
				 ...
				 */
				'per_page'   => apply_filters( 'bgci_bp_habits_admin_members_type_per_page', 10, $type ),
				'page'       => $current_type_page,
			)
		);

		$member_type_users   = $member_type_query->results;
		$pagination[ $type ] = bgci_bp_habits_admin_create_pagination_links( $member_type_query, $type );
	}
	...
	foreach ( $members as $member_type => $type_users ) :
		...
		<div class="bp-groups-member-type" id="bp-groups-member-type-<?php echo esc_attr( $member_type ); ?>">
			...
			switch ( $member_type ) :
				case 'admin':
							  esc_html_e( 'Organizers', 'bp-group-check-in' );
					break;
				case 'mod':
							  esc_html_e( 'Moderators', 'bp-group-check-in' );
					break;
				case 'member':
							  esc_html_e( 'Members', 'bp-group-check-in' );
					break;
				case 'banned':
							  esc_html_e( 'Banned Members', 'bp-group-check-in' );
					break;
			endswitch;
			?>
			</h3>
		...
		<?php if ( ! empty( $type_users ) ) : ?>
			...
			<table class="widefat bp-group-members">
				<thead>
					<tr>
						<th scope="col" class="uid-column"><?php _e( 'ID', 'bp-group-check-in' ); ?></th>
						<th scope="col" class="uname-column"><?php _e( 'Name', 'bp-group-check-in' ); ?></th>
						<th scope="col" class="urole-column"><?php _e( 'Role', 'bp-group-check-in' ); ?></th>
					</tr>
				</thead>
				...
				<?php foreach ( $type_users as $type_user ) : ?>
					<tr>
						...
						<td class="urole-column">
							<label for="bp-groups-role-<?php echo esc_attr( $type_user->ID ); ?>" class="screen-reader-text">
																  <?php
																	/* translators: accessibility text */
																	_e( 'Select group role for member', 'bp-group-check-in' );
																	?>
							</label>
							<select class="bp-groups-role" id="bp-groups-role-<?php echo esc_attr( $type_user->ID ); ?>" name="bp-groups-role[<?php echo esc_attr( $type_user->ID ); ?>]">
								<optgroup label="<?php esc_attr_e( 'Roles', 'bp-group-check-in' ); ?>">
									<option class="admin"  value="admin"  <?php selected( 'admin', $member_type ); ?>><?php esc_html_e( 'Organizer', 'bp-group-check-in' ); ?></option>
									<option class="mod"    value="mod"    <?php selected( 'mod', $member_type ); ?>><?php esc_html_e( 'Moderator', 'bp-group-check-in' ); ?></option>
									<option class="member" value="member" <?php selected( 'member', $member_type ); ?>><?php esc_html_e( 'Member', 'bp-group-check-in' ); ?></option>
									<?php if ( 'banned' === $member_type ) : ?>
									<option class="banned" value="banned" <?php selected( 'banned', $member_type ); ?>><?php esc_html_e( 'Banned', 'bp-group-check-in' ); ?></option>
									<?php endif; ?>
								</optgroup>
								<optgroup label="<?php esc_attr_e( 'Actions', 'bp-group-check-in' ); ?>">
									<option class="remove" value="remove"><?php esc_html_e( 'Remove', 'bp-group-check-in' ); ?></option>
									<?php if ( 'banned' !== $member_type ) : ?>
										<option class="banned" value="banned"><?php esc_html_e( 'Ban', 'bp-group-check-in' ); ?></option>
									<?php endif; ?>
								</optgroup>
							</select>
					...
					<?php if ( has_filter( 'bgci_bp_habits_admin_manage_member_row' ) ) : ?>
						<tr>
							<td colspan="3">
								<?php
								 ...
								 * @param bgci_bp_habits_Group $item Object for the current group.
								 */
								do_action( 'bgci_bp_habits_admin_manage_member_row', $type_user->ID, $item );
								?>
							</td>
						</tr>
					<?php endif; ?>
		...
		<?php else : ?>
			...
			<p class="bp-groups-no-members description"><?php esc_html_e( 'No members of this type', 'bp-group-check-in' ); ?></p>
 ...
 */
function bgci_bp_habits_admin_edit_metabox_status( $item ) {
	...
	<div id="submitcomment" class="submitbox">
		<div id="major-publishing-actions">
			<div id="delete-action">
				<a class="submitdelete deletion" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'action', 'delete', $base_url ), 'bp-groups-delete' ) ); ?>"><?php _e( 'Delete Group', 'bp-group-check-in' ); ?></a>
			</div>

			<div id="publishing-action">
				<?php submit_button( __( 'Save Changes', 'bp-group-check-in' ), 'primary', 'save', false ); ?>
			</div>
			<div class="clear"></div>
		</div><!-- #major-publishing-actions -->
	</div><!-- #submitcomment -->
 ...
 *
 * @param bgci_bp_habits_Group|null $group The bgci_bp_habits_Group object corresponding to the group being edited.
 */
function bgci_bp_habits_admin_edit_metabox_group_type( bgci_bp_habits_Group $group = null ) {
	...
	$types         = bgci_bp_habits_get_group_types( array(), 'objects' );
	$current_types = (array) bgci_bp_habits_get_group_type( $group->id, false );
	$backend_only  = bgci_bp_habits_get_group_types( array( 'show_in_create_screen' => false ) );
	?>
	...
	<div class="bp-groups-settings-section" id="bp-groups-settings-section-group-type">
		<select id="bp-groups-group-type" name="bp-groups-group-type[]" autocomplete="off">
			<option value="" <?php selected( '', $current_types[0] ); ?>><?php _e( 'Select Group Type', 'bp-group-check-in' ); ?></option>
			<?php
 ...
 */
function bgci_bp_habits_process_group_type_update( $group_id ) {
	 ...
	 */
	if ( bgci_bp_habits_set_group_type( $group_id, $group_types ) ) {
		// @todo Success messages can't be posted because other stuff happens on the page load.
	}
}
add_action( 'bp_group_admin_edit_after', 'bgci_bp_habits_process_group_type_update' );
 ...
 */
function bgci_bp_habits_admin_create_pagination_links( BP_Group_Member_Query $query, $member_type ) {
	$pagination = '';
	 ...
	 */
	$per_page = apply_filters( 'bgci_bp_habits_admin_members_type_per_page', 10, $member_type );
	...
	$pag_links = paginate_links(
		array(
			...
			'format'    => '',
			'prev_text' => __( '&laquo;', 'bp-group-check-in' ),
			'next_text' => __( '&raquo;', 'bp-group-check-in' ),
	...
	$viewing_text = sprintf(
		_n( 'Viewing 1 member', 'Viewing %1$s - %2$s of %3$s members', $query->total_users, 'bp-group-check-in' ),
 ...
 */
function bgci_bp_habits_admin_get_usernames_from_ids( $user_ids = array() ) {
 ...
 * @since BuddyPress 1.7.0
 */
function bgci_bp_habits_admin_autocomplete_handler() {
	...
	if ( $suggestions && ! is_wp_error( $suggestions ) ) {
		foreach ( $suggestions as $user ) {
			...
			$matches[] = array(
				...
				'label' => sprintf( __( '%1$s (%2$s)', 'bp-group-check-in' ), $user->name, $user->ID ),
				'value' => $user->ID,
			);
		}
	}
...
add_action( 'wp_ajax_bp_group_admin_member_autocomplete', 'bgci_bp_habits_admin_autocomplete_handler' );
 ...
 * @param string $doaction Current $_GET action being performed in admin screen.
 */
function bgci_bp_habits_admin_process_group_type_bulk_changes( $doaction ) {
	...
	if ( 'remove_group_type' !== $new_type && null === bgci_bp_habits_get_group_type_object( $new_type ) ) {
		$error = true;
	} else {
		...
		foreach ( (array) $_REQUEST['gid'] as $group_id ) {
			...
			$group_type = bgci_bp_habits_get_group_type( $group_id );

			if ( 'remove_group_type' === $new_type ) {
				// Remove the current group type, if there's one to remove.
				if ( $group_type ) {
					$removed = bgci_bp_habits_remove_group_type( $group_id, $group_type );
				...
				if ( $new_type !== $group_type ) {
					$set = bgci_bp_habits_set_group_type( $group_id, $new_type );
...
add_action( 'bgci_bp_habits_admin_load', 'bgci_bp_habits_admin_process_group_type_bulk_changes' );
 ...
 * @since BuddyPress 2.7.0
 */
function bgci_bp_habits_admin_groups_type_change_notice() {
	...
	if ( $updated && in_array( $updated, array( 'group-type-change-error', 'group-type-change-success' ), true ) ) {
		...
		if ( 'group-type-change-error' === $updated ) {
			$notice = __( 'There was an error while changing group type. Please try again.', 'bp-group-check-in' );
			$type   = 'error';
		} else {
			$notice = __( 'Group type was changed successfully.', 'bp-group-check-in' );
			$type   = 'updated';
		}
...
add_action( bp_core_admin_hook(), 'bgci_bp_habits_admin_groups_type_change_notice' );
...
function bp_register_group_type_sections_filters_actions() {
	...
	if ( true === bp_disable_group_type_creation() ) {
		...
		add_action( 'add_meta_boxes_' . bgci_bp_habits_get_group_type_post_type(), 'bp_group_type_custom_meta_boxes' );

		// add column
		add_filter( 'manage_' . bgci_bp_habits_get_group_type_post_type() . '_posts_columns', 'bp_group_type_add_column' );

		// action for adding a sortable column name.
		add_action( 'manage_' . bgci_bp_habits_get_group_type_post_type() . '_posts_custom_column', 'bp_group_type_show_data', 10, 2 );

		// sortable columns
		add_filter( 'manage_edit-' . bgci_bp_habits_get_group_type_post_type() . '_sortable_columns', 'bp_group_type_add_sortable_columns' );
...
function bp_group_type_custom_meta_boxes() {
	$screen = get_current_screen();
	add_meta_box( 'bp-group-type-label-box', __( 'Labels', 'bp-group-check-in' ), 'bp_group_type_labels_meta_box', null, 'normal', 'high' );
	add_meta_box( 'bp-group-type-permissions', __( 'Permissions', 'bp-group-check-in' ), 'bp_group_type_permissions_meta_box', null, 'normal', 'high' );
	if ( 'add' != $screen->action ) {
		add_meta_box( 'bp-group-type-short-code', __( 'Shortcode', 'bp-group-check-in' ), 'bp_group_shortcode_meta_box', null, 'normal', 'high' );
	}

	remove_meta_box( 'slugdiv', bgci_bp_habits_get_group_type_post_type(), 'normal' );
}
...
function bp_group_type_labels_meta_box( $post ) {
		...
		<thead>
			<tr>
				<th scope="col" colspan="2">
					<?php _e( 'Group Type', 'bp-group-check-in' ); ?>
				</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<th>
					<?php _e( 'Plural Label', 'bp-group-check-in' ); ?>
				</th>
				<td>
					<input type="text" class="bp-group-type-label-name" name="bp-group-type[label_name]" placeholder="<?php _e( 'e.g. Teams', 'bp-group-check-in' ); ?>"  value="<?php echo esc_attr( $label_name ); ?>" style="width: 100%" />
				</td>
			</tr>
			<tr>
				<th>
					<?php _e( 'Singular Label', 'bp-group-check-in' ); ?>
				</th>
				<td>
					<input type="text" class="bp-group-type-singular-name" name="bp-group-type[label_singular_name]" placeholder="<?php _e( 'e.g. Team', 'bp-group-check-in' ); ?>" value="<?php echo esc_attr( $label_singular_name ); ?>" style="width: 100%" />
				</td>
			</tr>
		</tbody>
	</table>
	...
	<h3><?php _e( 'Group Roles', 'bp-group-check-in' ); ?></h3>
	<p><?php _e( 'Rename the group member roles for groups of this type (optional).', 'bp-group-check-in' ); ?></p>

	<table class="widefat bp-postbox-table">
		<thead>
			<tr>
				<th scope="col" colspan="2">
					<?php _e( 'Organizers', 'bp-group-check-in' ); ?>
				</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				...
				<td>
					<input type="text" name="bp-group-type-role[organizer_plural_label_name]" placeholder="<?php _e( 'e.g. Organizers', 'bp-group-check-in' ); ?>"  value="<?php echo esc_attr( $organizer_plural ); ?>" style="width: 100%;" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="text" name="bp-group-type-role[organizer_singular_label_name]" placeholder="<?php _e( 'e.g. Organizer', 'bp-group-check-in' ); ?>" value="<?php echo esc_attr( $organizer_singular ); ?>" style="width: 100%;" />
				</td>
			</tr>
		</tbody>
	</table>
	...
	<table class="widefat bp-postbox-table">
		<thead>
			<tr>
				<th scope="col" colspan="2">
					<?php _e( 'Moderators', 'bp-group-check-in' ); ?>
				</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				...
				<td>
					<input type="text" name="bp-group-type-role[moderator_plural_label_name]" placeholder="<?php _e( 'e.g. Moderators', 'bp-group-check-in' ); ?>"  value="<?php echo esc_attr( $moderator_plural ); ?>" style="width: 100%;" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="text" name="bp-group-type-role[moderator_singular_label_name]" placeholder="<?php _e( 'e.g. Moderator', 'bp-group-check-in' ); ?>" value="<?php echo esc_attr( $moderator_singular ); ?>" style="width: 100%;" />
				</td>
			</tr>
		</tbody>
	</table>
	...
	<table class="widefat bp-postbox-table">
		<thead>
			<tr>
				<th scope="col" colspan="2">
					<?php _e( 'Members', 'bp-group-check-in' ); ?>
				</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				...
				<td>
					<input type="text" name="bp-group-type-role[member_plural_label_name]" placeholder="<?php _e( 'e.g. Members', 'bp-group-check-in' ); ?>"  value="<?php echo esc_attr( $members_plural ); ?>" style="width: 100%;" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="text" name="bp-group-type-role[member_singular_label_name]" placeholder="<?php _e( 'e.g. Member', 'bp-group-check-in' ); ?>" value="<?php echo esc_attr( $members_singular ); ?>" style="width: 100%;" />
				</td>
			</tr>
		</tbody>
	</table>
...
function bp_group_type_permissions_meta_box( $post ) {
	...
	<table class="widefat bp-postbox-table">
		<thead>
			<tr>
				<th scope="col" colspan="2">
					<?php _e( 'Groups Directory', 'bp-group-check-in' ); ?>
				</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td colspan="2">
					...
					<?php _e( 'Display this group type in "Types" filter in Groups Directory', 'bp-group-check-in' ); ?>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					...
					<?php _e( 'Hide all groups of this type from Groups Directory', 'bp-group-check-in' ); ?>
				</td>
			</tr>
		</tbody>
	</table>
	...
	<table class="widefat bp-postbox-table">
		<thead>
			<tr>
				<th scope="col" colspan="2">
					<?php _e( 'Group Type Invites', 'bp-group-check-in' ); ?>
				</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td colspan="2">
					...
					<?php _e( 'If a member is already in a group of this type, they cannot be sent an invite to join another group of this type.', 'bp-group-check-in' ); ?>
				</td>
			</tr>
		</tbody>
	</table>
	...
	if ( true === bp_member_type_enable_disable() ) {
		...
		<table class="widefat bp-postbox-table">
			<thead>
				<tr>
					<th scope="col" colspan="2">
						<?php _e( 'Profile Type Invites', 'bp-group-check-in' ); ?>
					</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td colspan="2">
						<?php _e( 'Only members of the selected profile types may be sent requests to join this group. (Leave blank for unrestricted invites)', 'bp-group-check-in' ); ?>
					</td>
				</tr>
		...
		<table class="widefat bp-postbox-table">
			<thead>
				<tr>
					<th scope="col" colspan="2">
						<?php _e( 'Profile Type Joining', 'bp-group-check-in' ); ?>
					</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td colspan="2">
						<?php _e( 'Members of the selected profile types can always join groups of this type, even if the group is private.', 'bp-group-check-in' ); ?>
					</td>
				</tr>
...
function bp_group_shortcode_meta_box( $post ) {
	...
	?>
		...
		<p><?php _e( 'To display all groups with this group type on a dedicated page, add this shortcode to any WordPress page.', 'bp-group-check-in' ); ?></p>
		<code id="group-type-shortcode"><?php echo '[group type="' . $key . '"]'; ?></code>
		<button class="copy-to-clipboard button"  data-clipboard-target="#group-type-shortcode"><?php _e( 'Copy to clipboard', 'bp-group-check-in' ); ?></button>
...
function bp_group_type_add_column( $columns ) {
	...
	$columns['title']         = __( 'Group Type', 'bp-group-check-in' );
	$columns['group_type']    = __( 'Label', 'bp-group-check-in' );
	$columns['enable_filter'] = __( 'Groups Filter', 'bp-group-check-in' );
	$columns['enable_remove'] = __( 'Groups Directory', 'bp-group-check-in' );
	$columns['total_groups']  = __( 'Groups', 'bp-group-check-in' );
	...
	switch ( $column ) {
		...
		case 'enable_filter':
			if ( get_post_meta( $post_id, '_bp_group_type_enable_filter', true ) ) {
				_e( 'Show', 'bp-group-check-in' );
			} else {
				_e( 'Hide', 'bp-group-check-in' );
			}
		...
		case 'total_groups':
			...
			printf(
				__( '<a href="%1$s">%2$s</a>', 'bp-group-check-in' ),
...
function bp_group_type_sort_items( $qv ) {
	...
	if ( ! isset( $qv['post_type'] ) || $qv['post_type'] != bgci_bp_habits_get_group_type_post_type() ) {
		return $qv;
	}
...
function bp_group_type_hide_quick_edit( $actions, $post ) {
	...
	if ( bgci_bp_habits_get_group_type_post_type() === $post->post_type ) {
 ...
 * @since BuddyBoss 1.0.0
 */
function bgci_bp_habits_admin_group_type_listing_add_groups_tab() {
	global $pagenow ,$post;
	...
	if ( true === bp_disable_group_type_creation() ) {
		...
		if ( ( isset( $GLOBALS['wp_list_table']->screen->post_type ) && $GLOBALS['wp_list_table']->screen->post_type == 'bp-group-type' && $pagenow == 'edit.php' ) || ( isset( $post->post_type ) && $post->post_type == 'bp-group-type' && $pagenow == 'edit.php' ) || ( isset( $post->post_type ) && $post->post_type == 'bp-group-type' && $pagenow == 'post-new.php' ) || ( isset( $post->post_type ) && $post->post_type == 'bp-group-type' && $pagenow == 'post.php' ) ) {
			?>
			<div class="wrap">
				<h2 class="nav-tab-wrapper"><?php bp_core_admin_groups_tabs( __( 'Group Types', 'bp-group-check-in' ) ); ?></h2>
			</div>
			<?php
		}
	}
}
add_action( 'admin_notices', 'bgci_bp_habits_admin_group_type_listing_add_groups_tab' );
