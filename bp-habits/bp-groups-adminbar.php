 ...
 */
function bgci_bp_habits_group_admin_menu() {
	global $wp_admin_bar;
	$bp = buddypress();
	...
	$wp_admin_bar->add_menu(
		array(
			...
			'title' => __( 'Edit Group', 'bp-group-check-in' ),
	...
	foreach ( $secondary_nav_items as $menu ) {
		...
		if ( $menu->show_in_admin_bar ) {
			$title = sprintf( __( 'Edit Group %s', 'bp-group-check-in' ), $menu->name );

			// Title is specific for delete.
			if ( 'delete-group' == $menu->slug ) {
				$title = sprintf( __( '%s Group', 'bp-group-check-in' ), $menu->name );
					...
					if ( bbp_is_group_forums_active() ) {
						$wp_admin_bar->add_menu(
							array(
								...
								'title'  => __( 'Edit Group Discussion', 'bp-group-check-in' ),
...
add_action( 'admin_bar_menu', 'bgci_bp_habits_group_admin_menu', 99 );
 ...
 */
function bgci_bp_habits_remove_edit_page_menu() {
	if ( bp_is_group() ) {
		remove_action( 'admin_bar_menu', 'wp_admin_bar_edit_menu', 80 );
	}
}
add_action( 'add_admin_bar_menus', 'bgci_bp_habits_remove_edit_page_menu' );
